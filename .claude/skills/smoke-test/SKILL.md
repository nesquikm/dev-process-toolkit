...
#### Fixture group 8 — STE-285 hooks runtime regression (Linear + Jira)

Symmetric to fixture group 7 (STE-225 TDD orchestrator forks runtime); covers the STE-285 hooks pipeline end-to-end at runtime per STE-286. Exercises install + per-scenario refusal probes for the 4 opt-in Process-category hooks seeded by STE-285 (`pre-commit-gate-check`, `pre-commit-tdd-orchestrator`, `pre-pr-spec-review`, `pre-spec-write-brainstorm-reminder`).

**Diagnostic shape:** any per-scenario regression in fixture group 8 appends a `STE-285 runtime regression: <fixture-name>` line to the findings file, mirroring fixture group 7's `STE-225 runtime regression: ...` shape (per STE-231 AC.5 precedent). A clean run instead appends `STE-285 runtime check: PASS` to the per-tracker summary.

**Install verification (AC-STE-286.3):** after Phase 2 step 3's canonical `/setup` returns, the smoke driver re-invokes `claude -p /dev-process-toolkit:setup --hooks=all` as a separate `claude -p` child against the test project (Phase 2 step 3a per FR). This materializes the 4 opt-in Process-category hooks into `<test-project>/.claude/settings.json` without re-running the full canonical chain (the preselect-flag path bypasses `AskUserQuestion` and calls `installHooks(...)` directly with the resolved `HOOK_REGISTRATIONS` keys). On return, fixture group 8 reads `<test-project>/.claude/settings.json` and asserts the 4 hook entries land with the correct event/matcher pairs per `HOOK_REGISTRATIONS`:

- Exactly **3 entries under `PreToolUse: Bash`** matcher — one per pre-commit/pre-PR hook (`pre-commit-gate-check`, `pre-commit-tdd-orchestrator`, `pre-pr-spec-review`); these fire on `Bash` tool calls so the matcher field is the literal token `"matcher": "Bash"`.
- Exactly **1 entry under `UserPromptSubmit: *`** matcher — the `pre-spec-write-brainstorm-reminder` advisory; this fires on every user prompt regardless of tool so the matcher field is the literal token `"matcher": "*"`.

The install-verification half therefore reduces to a shape check on `<test-project>/.claude/settings.json`. Three grep probes pin the assertion:

```
grep -c -F '"matcher": "Bash"' <test-project>/.claude/settings.json   # expect ≥ 1 (canonical: 1 matcher block carrying the 3 PreToolUse:Bash hooks, OR 3 separate blocks)
grep -c -F '"matcher": "*"'   <test-project>/.claude/settings.json   # expect ≥ 1 (canonical: 1 matcher block carrying the 1 UserPromptSubmit:* hook)
grep -c -F '${CLAUDE_PLUGIN_ROOT}/templates/hooks/process/' <test-project>/.claude/settings.json   # expect ≥ 4 — one entry per seeded hook; confirms portable token shape (STE-288)
```

- `grep -c -F '${CLAUDE_PLUGIN_ROOT}/templates/hooks/process/' <test-project>/.claude/settings.json` ≥ 4 — one entry per seeded hook; confirms the portable `${CLAUDE_PLUGIN_ROOT}`-rooted command shape (STE-288). A literal absolute path (e.g., `/Users/<u>/workspace/...`) in any of the 4 entries ⇒ portability regression: the seeded `command` was materialized for the smoke harness's machine instead of the variable-expansion shape Claude Code resolves at invocation time on the installer's machine.

All three grep tokens MUST appear; missing any ⇒ install-verification regression. The driver additionally parses the file as JSON and counts entries per (event, matcher) bucket to confirm the exact 3 + 1 split — the bare grep ≥ 1 lower bound is the floor probe; the JSON-parse step is the precise probe. Per-scenario regression here appends `STE-285 runtime regression: hooks-install-shape` to the findings file with the observed counts inlined.

**Runtime refusal probe (AC-STE-286.4):** after the install-verification half passes, fixture group 8 drives the 4 opt-in hooks through a per-scenario runtime probe. The 4 scenarios — `pre-commit-gate-check`, `pre-commit-tdd-orchestrator`, `pre-pr-spec-review`, `pre-spec-write-brainstorm-reminder` — each exercise the single canonical refusal action for the matching hook (commit / commit-with-FR / `gh pr create` invocation / `/spec-write` invocation respectively). Per scenario the driver rotates `$CLAUDE_SESSION_FILE` to a fresh empty-JSONL fixture at `/tmp/dpt-smoke-empty-session-<scenario>.jsonl` (one harmless non-Skill entry, satisfying the seeded hooks' `[ -f $file ]` fail-open guard), then spawns one **standalone `bash` subprocess** that invokes the seeded hook script under `plugins/dev-process-toolkit/templates/hooks/process/<scenario>.sh` directly, with `CLAUDE_SESSION_FILE=/tmp/dpt-smoke-empty-session-<scenario>.jsonl` set inline on the subprocess env. Each scenario's combined stdout+stderr lands at `/tmp/dpt-smoke-<tracker>-hooks-runtime-<scenario>.log` (one log file per scenario, 4 files total per leg). The 4 scenarios run sequentially, not in parallel — a per-scenario regression appends `STE-285 runtime regression: hooks-runtime-<scenario>` to the findings file with the offending log path inlined.

This is the AC-STE-286.7-accepted **fallback** spawn shape — 4 standalone `bash` subprocesses invoking the seeded hook scripts directly, NOT a single shared `claude -p` child with `$CLAUDE_SESSION_FILE` rotated between sequenced scenarios. The fallback is mandatory because the Claude Code harness clobbers any wrapper-shell `$CLAUDE_SESSION_FILE` with its own canonical session log path before spawning PreToolUse / UserPromptSubmit subprocesses, defeating parent-shell env rotation; see `specs/frs/STE-286.md` `## Notes` § `$CLAUDE_SESSION_FILE empirical finding (recorded 2026-05-13 during /implement Phase 1 of M72)` for the canonical empirical record. The 4-scenario structure, the per-scenario empty-JSONL fixture, and the `/tmp/dpt-smoke-<tracker>-hooks-runtime-<scenario>.log` capture pattern are preserved verbatim across the fallback — only the spawn surface (`bash` subprocess vs. `claude -p` child) differs.

**Per-scenario NFR-10 assertions (AC-STE-286.5):** for each of the 4 scenarios, after the standalone `bash` subprocess returns, fixture group 8 runs one `grep -F` probe over the per-scenario `/tmp/dpt-smoke-<tracker>-hooks-runtime-<scenario>.log` file to assert the NFR-10 refusal/reminder shape. Three of the 4 scenarios are **hard refusals** (exit ≠ 0, stderr `Refusing:`); the 4th is **advisory** (exit = 0, stderr `Reminder:`). Each scenario carries a remediation token of the shape `dev-process-toolkit:<skill>` naming the skill whose invocation would have satisfied the hook, plus a `hook=<name>` token naming the seeded hook that fired. The 4 assertions are independent — a regression in one does not short-circuit the others; per-scenario regression appends `STE-285 runtime regression: hooks-runtime-<scenario>-assertion` to the findings file with the offending log path + the missing token list inlined.

- **Scenario 1 — `pre-commit-gate-check`:** the standalone `bash` subprocess simulates a `git commit` attempt with no prior `Skill(/dev-process-toolkit:gate-check)` entry in the rotated `$CLAUDE_SESSION_FILE`. Expected exit ≠ 0 (hard refusal); expected stderr substrings (all three required, asserted via `grep -F` on `/tmp/dpt-smoke-<tracker>-hooks-runtime-pre-commit-gate-check.log`): `Refusing:`, `dev-process-toolkit:gate-check`, `hook=pre-commit-gate-check`. Missing any one of the three ⇒ regression.
- **Scenario 2 — `pre-commit-tdd-orchestrator`:** the standalone `bash` subprocess stages a synthetic `specs/frs/<stub-id>.md` file and simulates a `git commit` attempt with no prior `Skill(/dev-process-toolkit:tdd)` entry in the rotated `$CLAUDE_SESSION_FILE`. Expected exit ≠ 0 (hard refusal); expected stderr substrings (all three required, asserted via `grep -F` on `/tmp/dpt-smoke-<tracker>-hooks-runtime-pre-commit-tdd-orchestrator.log`): `Refusing:`, `dev-process-toolkit:tdd`, `hook=pre-commit-tdd-orchestrator`. Missing any one of the three ⇒ regression.
- **Scenario 3 — `pre-pr-spec-review`:** the standalone `bash` subprocess simulates a `gh pr create` attempt with no prior `Skill(/dev-process-toolkit:spec-review)` entry in the rotated `$CLAUDE_SESSION_FILE`. Expected exit ≠ 0 (hard refusal); expected stderr substrings (all three required, asserted via `grep -F` on `/tmp/dpt-smoke-<tracker>-hooks-runtime-pre-pr-spec-review.log`): `Refusing:`, `dev-process-toolkit:spec-review`, `hook=pre-pr-spec-review`. Missing any one of the three ⇒ regression.
- **Scenario 4 — `pre-spec-write-brainstorm-reminder`:** the standalone `bash` subprocess simulates a `/dev-process-toolkit:spec-write` invocation with no prior `Skill(/dev-process-toolkit:brainstorm)` entry in the rotated `$CLAUDE_SESSION_FILE` and **no `--tracker` arg** on the simulated invocation. Expected exit = 0 (advisory only — the reminder fires but the hook does not block); expected stderr substrings (all three required, asserted via `grep -F` on `/tmp/dpt-smoke-<tracker>-hooks-runtime-pre-spec-write-brainstorm-reminder.log`): `Reminder:`, `dev-process-toolkit:brainstorm`, `hook=pre-spec-write-brainstorm-reminder`. Missing any one of the three ⇒ regression.

The 4 `grep -F` probes (one per scenario) are the canonical NFR-10 conformance check for fixture group 8; the `Refusing:` / `Reminder:` token split mirrors the seeded hook scripts' refusal-category contract (hard-refuse for the 3 pre-commit/pre-PR scenarios, advisory-only for the `/spec-write` reminder). The remediation-token shape `dev-process-toolkit:<skill>` is fixed by NFR-10 (the canonical remediation hint format); the `hook=<name>` token is the seeded hook's self-identification, load-bearing because it lets the operator distinguish which of the 4 hooks fired when multiple scenarios share a logfile prefix during debug runs.

**Findings file shape (AC-STE-286.6):** fixture group 8's per-scenario regressions and clean-run summary land in the fixture-group-8 findings file at `/tmp/dpt-smoke-<tracker>-findings-<date>.md` (per-tracker, per-date — one file per tracker leg per smoke run; the path is fixture-group-8-scoped so a regression here does not collide with the broader Phase 3 findings file, which uses the legacy `/tmp/dpt-smoke-findings-<date>-<tracker>.md` ordering). The diagnostic shape follows the STE-231 AC.5 precedent — fixture group 8's canonical-shape line names the system-under-test (STE-285, the seeded-hooks FR), not the test FR (STE-286). Two append paths apply:

- **On any scenario regression** (install-verification shape probe, runtime-refusal probe, or per-scenario NFR-10 assertion — any of the failure modes catalogued above): fixture group 8 appends a single line of the canonical shape `STE-285 runtime regression: <fixture-name>` to `/tmp/dpt-smoke-<tracker>-findings-<date>.md`, where `<fixture-name>` is the per-failure-mode token already named above (`hooks-install-shape`, `hooks-runtime-<scenario>`, or `hooks-runtime-<scenario>-assertion`). Multiple regressions in a single leg append multiple lines, one per regression, in the order they fire — the file is append-only across the 4 scenarios + 1 install probe (up to 9 lines maximum per leg: 1 install + 4 runtime + 4 assertion). The `STE-285 runtime regression:` prefix is verbatim (literal angle-bracket placeholder for `<fixture-name>` — the actual line interpolates the fixture name on emit).
- **On a clean run** (install-verification + all 4 scenarios + all 4 NFR-10 assertions pass): fixture group 8 instead appends the line `STE-285 runtime check: PASS` (verbatim) to the per-tracker summary block at the bottom of `/tmp/dpt-smoke-<tracker>-findings-<date>.md`. This is the affirmative health signal — its absence on a leg where the other fixture groups' `STE-<sut> runtime check: PASS` lines do appear is itself a regression indicator (suggests fixture group 8 never ran, or aborted mid-leg before emit).

Per the STE-231 precedent (fixture groups 4–7's `STE-<sut> runtime regression: <fixture-name>` shape), the regression diagnostic names the SUT first so triage points to the broken seeded-hooks implementation (STE-285), not the test infrastructure (STE-286). A grep over the findings file post-run (`grep -F 'STE-285 runtime regression:' /tmp/dpt-smoke-<tracker>-findings-<date>.md`) yields zero matches on a clean leg and one-or-more matches on a regressed leg; the complementary `grep -F 'STE-285 runtime check: PASS' /tmp/dpt-smoke-<tracker>-findings-<date>.md` yields exactly one match on a clean leg and zero on a regressed leg.

See `specs/frs/STE-286.md` (and the FR's `## Notes` empirical-finding subsection) for the full driver shape and the `$CLAUDE_SESSION_FILE` override fallback rationale (per-scenario standalone `bash` subprocesses).
