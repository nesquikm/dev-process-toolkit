...
**Two clauses per sub-fixture, and the identifier clause is deliberately NOT the discriminator.** Probe #26 fires in all three sub-fixtures, so it is named in all three captures; the identifier clause only proves the probe surfaced. It is the SUBJECT clause that has to discriminate, and each one is verified to exit 0 on its own capture and NON-ZERO on both siblings.

**The identifier clause names the PROBE, never a rendering of it (STE-564).** It used to be an `any-of` over two row shapes — `**#26 ` and `**Probe #26 ` — because one run rendered the row three ways. That was a correct reading of the captures in front of it and it still died: STE-532/533 standardised probe-row rendering, and both shapes fell to ZERO occurrences across five 2026-09-05 captures while probe #26 stayed healthy and kept reporting file, line, expectation and actual. Four arms across two groups stopped matching on the same day and not one of them went red — the fourth occurrence of the class.

The diagnosis came from a CONTRAST rather than from a failure: sub-fixture 3c keys on the probe NAME instead of a decoration and passed on the same runtime that failed the others. So the repair is not a newer marker, which would only move the defect to a fresh address; it is to assert the SUBJECT. The arm is `any-of-token` over probe #26's two rendered id spellings, exactly the two-spelling discipline 3c already uses.

**The reason the arms were decorated in the first place is preserved, at the matching layer where it belongs.** STE-488 measured that an undelimited key is substring-satisfiable by a sibling row — `identity_mode_conditional` is a proper substring of `plan_identity_mode_conditional` — and the leading `**` was chosen because it was the only leading delimiter that survived these spans being EXECUTED, where a backtick is command substitution. `any-of-token` supplies that delimiter without borrowing one from the rendering: a key matches only where it is not adjacent to an identifier character, so the shorter probe id no longer matches inside the longer one. The hazard was always a matching problem, and this is its matching-level answer.
...
**The frontmatter is the LEG'S OWN mode shape, not `mode: none`'s (STE-564, F3).** This template used to carry `id:` and no `tracker:` block — the `mode: none` identity shape — on a group that runs on the LINEAR leg only. Under a tracker, probe #13's bimodal invariant demands the exact opposite: a `tracker:` block present, the minted `id:` absent. So the staged FR was off-shape for the only leg that stages it, probe #26's subject never received a well-formed input, and the group's real assertion ran VACUOUS while its identifier clause still reported the probe surfacing. Substitute the run's own tracker prefix for `<TKR>` and its own ACTIVE milestone token for `<MILESTONE>` — an FR bound to a milestone with no live plan file hard-fails probe #27 on its own account, which is the same attributability loss one door along.

```markdown
---
title: Smoke fixture — Notes-section capability scanner
milestone: <MILESTONE>
status: active
archived_at: null
tracker:
  linear: <TKR>-FIX-A
created_at: <run start, ISO-8601>
---
...
- Assert (probe identifier): ``bun "${CAP_ASSERT}" any-of-token /tmp/dpt-smoke-<tracker>-ste221-positive.log tracker-project-milestone-attached tracker_project_milestone_attached`` exit 0 — probe #26 surfaced, named by its OWN identifier under either rendered spelling.
- Assert (subject): ``bun "${CAP_ASSERT}" present-token /tmp/dpt-smoke-<tracker>-ste221-positive.log milestone_attach_skipped_adapter_limit`` exit 0 — probe #26 READ the `## Notes` token this sub-fixture staged and reported it back. This is the group's real subject, and it is the assertion the mode-none-shaped fixture was silently skipping: the key is assistant-0 in BOTH sibling captures, so it cannot pass for any other reason.
...
- Assert (probe identifier): ``bun "${CAP_ASSERT}" any-of-token /tmp/dpt-smoke-<tracker>-ste221-control.log tracker-project-milestone-attached tracker_project_milestone_attached`` exit 0.
- Assert (subject): ``bun "${CAP_ASSERT}" absent-token /tmp/dpt-smoke-<tracker>-ste221-control.log milestone_attach_skipped_adapter_limit milestone_attach_unavailable`` exit 0 — paired with the identifier clause above, this IS the control's subject: probe #26 surfaced and read back NEITHER capability key, which is the only thing distinguishing 2b from both siblings. Verified in both directions on the committed captures — it exits 0 on the control and NON-ZERO on 2a (canonical key present) and on 2c (alias present). **The bare aggregate `GATE FAILED` is retired and must not be restored (AC-STE-489.4):** it is present in ALL THREE captures (12 / 14 / 12 raw occurrences), including the two in which probe #26 did not fail, so it could not have gone red for this sub-fixture's subject however well or badly probe #26 behaved. It passed only because probe #26's row happened to be independently readable — which is not a thing it checks. **The row-decorated form that replaced it is retired for the same reason, one layer along (STE-564).** It read `**#26 \`tracker-project-milestone-attached\` — GATE FAILED`, and STE-532/533 standardised probe-row rendering so that decoration is no longer emitted — measured at ZERO occurrences across five 2026-09-05 captures while probe #26 stayed healthy. An arm pinned to how a row is drawn dies the day anyone standardises the drawing, and says nothing about the probe.
- Cleanup.
...
- Assert (probe identifier): ``bun "${CAP_ASSERT}" any-of-token /tmp/dpt-smoke-<tracker>-ste221-alias.log tracker-project-milestone-attached tracker_project_milestone_attached`` exit 0 — the same arm as its siblings, because the probe's name does not change between renderings while its row decoration does.
- Assert (subject): ``bun "${CAP_ASSERT}" present-token /tmp/dpt-smoke-<tracker>-ste221-alias.log milestone_attach_unavailable`` exit 0 — the deprecated alias was read from `## Notes` and the missing binding downgraded (deprecation window honored per STE-214 AC.5). **Keying this on the literal `ADVISORY` is NOT the repair:** this capture carries zero `ADVISORY`, assistant AND raw — its downgrade renders as `(capability-gap downgrade)` plus the alias key — so swapping the broken regex for that literal would leave 2c red for a second, different wrong reason. When STE-198's rollover note flips, this sub-fixture re-keys onto probe #26's own failing row, the shape 2b asserts, and never onto the bare aggregate.
- Cleanup.
...
- The per-group block the renderer below produces. This bullet names only that the block is appended — how it is spelled, how many lines it carries, and which form is refused are defined once in § Phase 3 — Capture and are deliberately not restated here.
...
- Assert (probe identifier): `bun "${CAP_ASSERT}" any-of-token /tmp/dpt-smoke-<tracker>-ste450-tracker-block.log identity_mode_conditional identity-mode-conditional` exit 0 — probe #13 surfaced, named by its OWN identifier under either rendered spelling.

  **Why a BARE `identity_mode_conditional` arm is a false green, and why the answer is the MATCHER rather than a decoration (STE-488, re-sited by STE-564).** The obvious arm is the probe key itself, and under plain substring matching it is a **false green**: that key is a SUBSTRING of `plan_identity_mode_conditional`, which is sub-fixture 9b's probe. Measured — the bare arm exits **0 against 9b's own capture**, where probe #13's enforcing arm never fired at all (it PASSED there; the hit belongs to probe #73). A run in which #13 never fired would satisfy 9a purely because #73 did. STE-488 fixed that with a LEADING delimiter, and the only one available was the `**` row prefix: a trailing delimiter cannot fix a substring relation (`plan_identity_mode_conditional:` also ends in `identity_mode_conditional:`), and the delimiter could not be the backtick the runtime renders, because these spans are EXECUTED and a backtick is command substitution in bash. That reasoning was correct and the arm STILL died — STE-532/533 standardised probe-row rendering, and both row shapes (`**#13 ` / `**✗ Probe #13 `, and 9b's `**#73 ` / `**✗ Probe #73 `) fell to ZERO occurrences across five 2026-09-05 captures while both probes stayed healthy and kept reporting file, line, expectation and actual. **The delimiter now comes from the matcher.** `any-of-token` matches a key only where it is not adjacent to an identifier character, so `identity_mode_conditional` does not match inside `plan_identity_mode_conditional` while the longer key still matches itself. The substring hazard was always a matching problem; borrowing a delimiter from the runtime's prose made the arm a hostage to how that prose is drawn.

  **Two arms, not one, and this is fixture 3c's lesson applied before it bites rather than after.** `/gate-check` renders a probe id in either the hyphenated or the underscored spelling depending on the leg, so both are named and neither is satisfied by the other probe's row. Verified in both directions on the committed captures: 9a's pair exits 0 on the tracker-block capture and non-zero on the plan-stem one, and 9b's mirror pair does the reverse — the same property the row prefix was protecting, now protected by something the runtime cannot restyle.

  **The residual, stated rather than glossed.** These arms prove probe #13 was NAMED, not that it FAILED — a run rendering a passing #13 row would satisfy this clause. That is why it is paired with the verdict clause below rather than standing alone, and it is the same reason a bare `#13` is rejected as an arm: on its own it is satisfiable by a run in which nothing fired.
...
- Assert (probe identifier): `bun "${CAP_ASSERT}" any-of-token /tmp/dpt-smoke-<tracker>-ste450-plan-stem.log plan_identity_mode_conditional plan-identity-mode-conditional` exit 0 — probe #73 surfaced, named by its OWN identifier under either rendered spelling.

  Name-keyed and two-armed for the same reasons as 9a, and deliberately symmetric even though this arm is not itself substring-satisfiable — nothing in the runtime's vocabulary contains `plan_identity_mode_conditional` as a proper superstring, and the bare form was measured to exit 1 against 9a's capture, correctly. Keeping the shape uniform is the point: whoever adds the next probe id here does not have to re-check it for the substring hazard, because `any-of-token` supplies the boundary for every arm rather than for the ones someone remembered. Per-instance vigilance is what failed three times on fixture 3c and a fourth time on this row decoration.
...
If sub-fixture 12a passes on a leg, append `STE-467 runtime check: PASS` to the run summary line; any failure appends `STE-467 runtime check: FAIL`. This group names only the line it contributes — how that line is spelled, and what a group that never executed appends, are defined once in § Phase 3 — Capture, and what each outcome MEANS is defined once in § Phase 2.X summary line; neither is restated here. Tracker-independent by construction — the disposition is decided by the manifest's presence on disk, never by a tracker surface, so the group is never N/A on any registered leg.
...
- Assert `verifyDeliverStageCapture(capturePath, evidence, spawn)` (`adapters/_shared/src/deliver_stage_capture.ts`) returns `ok: true` with an empty `reasons` array for the capture. All three arguments are passed: the third is the stage's spawn expectation — the chain the stage ran and the handle its ownership check resolved — without which a fence carrying a fabricated spawn receipt grades clean. The predicate demands report prose, then **exactly one** ```` ```deliver-stage-result ```` fence, all eight sections (`stage`, `milestone`, `status`, `summary`, `gate`, `drive`, `e2e`, `follow_ups`) in the fixed order, within the 26-line cap, with concrete scalars.
...
**Source:** `/tmp/dpt-smoke-<tracker>-ste533-stage-report.txt` — the closing report of the last adopting stage the chain ran (`/implement` on every leg). No phase captures that text as a file of its own, so **the driver writes** it, the way group 14 writes its own subject: run `extractAssistantText` (`adapters/_shared/src/smoke_child_capture.ts`) over `/tmp/dpt-smoke-<tracker>-implement.log` (already captured during Phase 2 step 3), take the assistant text from the last `stage-status-block` fence opener's paragraph onward through the closing marker, and write that to the path above before the assertions run. A leg whose `/implement` capture carries no closing report at all renders NOT-REACHED, never PASS — and a group whose subject nothing writes is the defect this line exists to prevent.

**Assertions:**

- Run `bun "${PLUGIN_DIR}/adapters/_shared/src/stage_block_adoption.ts" --report /tmp/dpt-smoke-<tracker>-ste533-stage-report.txt` — the executable front door for `verifyStageReportAdoption` (`adapters/_shared/src/stage_block_adoption.ts`). Exit **0** is the pass, and it proves exactly this much: the capture carries exactly one ```` ```stage-status-block ```` fence, at most the stated number of prose lines above it, nothing after it but the cap-exempt sections the stage owes, and no canonical capability token loose in the prose outside the fence. That last clause is a refusal, not a presence check — `locateCapabilityTokens` splits the tokens it FINDS into `inBlock` and `outsideBlock` and refuses the second set, so a report carrying none of them at all still exits 0. Exit **1** is a fail and every refusal is printed in the house `Remedy:`/`Context:` shape; exit **2** is a usage error and is neither.
...
**Fixture-roster spelling — the one accepted form (STE-491).** The Phase 2.X fixture-group block goes into the findings file and into the run summary in exactly one spelling: the head line, then one `STE-<sut> runtime check: <PASS|FAIL|NOT-REACHED|N/A>` line for **every** fixture group the canonical roster carries — one line per group, on every leg, with no exceptions. Read the count from the roster, never from a number written down here. The block is produced, not typed: `bun "${PLUGIN_DIR}/adapters/_shared/src/smoke_fixture_groups.ts" render --leg <the resolved leg> --passed "<group numbers>" --failed "<group numbers>"` renders it for the linear, jira and tracker-less legs alike, and its stdout is what the findings file carries verbatim. § Phase 2.X summary line defines what each of the four outcomes means; this section defines how they are written.

A compacted hand-tally — the one-line `STE-226 PASS · STE-214 N/A · STE-215 PASS · …` shape a leg produces when it tallies the roster itself instead of running the module — is **not an accepted alternative**, on any leg, in the findings file or anywhere else. It carries the same verdicts and is invisible to a search for the documented spelling, so it returns zero matches — byte-indistinguishable from a leg that never reported the fixture groups at all, which is precisely the silence Phase 2.X exists to remove. A roster that is not the module's own stdout is treated as a roster that was never reported.
