...
- **AC-STE-448.8 — the plan's minted identity, and its self-derived filename.** The plan lands at `../dpt-test-project-none/specs/plan/M_<TAIL>.md` (or `specs/plan/archive/M_<TAIL>.md`) carrying the **same** `id:` value **verbatim** — byte-identical to the FR's, not merely the same tail — and its own filename stem equals `M_` + `id.slice(23, 29)`. The second half is the load-bearing one and is why this row is not a duplicate of the row above: the tail is 6 of the id's 26 characters, so a plan could carry a correct `id:` under a filename derived from something else entirely (a counter, another mint, a stale rename) and every id-only check would still pass. Recompute the stem from the id read out of the file and compare; do not read the stem and check the id contains it.
- **AC-STE-448.9 — the release proof, and exactly half of it.** After the archive commit, `../dpt-test-project-none/.dpt/locks/<id>` is **ABSENT** — `<id>` being the full `fr_`-prefixed value read from the FR's frontmatter, never the 6-char tail, because `LocalProvider.claimLock` keys the lock on the full minted id. On a tracker leg the equivalent evidence is the ticket reaching `Done`; here it is the lock file being gone.
