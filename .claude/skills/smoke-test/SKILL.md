...
- Assert `verifyDeliverStageCapture` (`adapters/_shared/src/deliver_stage_capture.ts`) returns `ok: true` with an empty `reasons` array for the capture. The predicate demands report prose, then **exactly one** ```` ```deliver-stage-result ```` fence, all eight sections (`stage`, `milestone`, `status`, `summary`, `gate`, `drive`, `e2e`, `follow_ups`) in the fixed order, within the 26-line cap, with concrete scalars.
