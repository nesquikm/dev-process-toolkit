...
**The step rules travel IN the step fence.** Four rules every step obeys — one operation per Bash call; fetch every listing and never fabricate one; run a module directly; a refusal is information — are written once, in the step fence's `RULES_EOF` heredoc below, and the fence puts them into every child's prompt. They used to live here, in prose that explained why it was not repeated per row, and nothing carried prose into a prompt: leg 7's driver pasted the rules by hand, leg 8's followed this document and did not, and leg 8 measured children who had never been told (41 discarded-run observations, 2026-09-25). A rule that depends on a driver remembering to paste it is not a rule. Edit the rules in the fence; the rows below still cite them by number, and the fence also hands each child a scratch directory named for the run's nonce and its step.
...
**The marker column is DATA, not a label.** The fence writes it verbatim into each child's `dpt-shared-tracker-scenario:` line, and the grader maps a session to a scenario by parsing that line, so the cell must be exactly one scenario id and nothing else. Note the consequence for row 4–5: it covers TWO steps under the single id `S1`, while the run marks the second of them `S2` — so a reader reconciling session markers against this table will find an S2 session the table never names. That mismatch is real and sent one investigation of live leg 4 down an attribution story that was not what happened; it is recorded here rather than fixed in the column, because the column is read by a machine that accepts one id.
...
| 2 | S8 | B | tree | Repoint B into the shared container with `repoint_tracker_binding.ts`; when it refuses, fix what it names and repoint again. **Every listing the repoint reads is FETCHED from the tracker and saved verbatim — never typed by hand.** `--statuses` on Jira comes from the adapter's declared `list_project_statuses` capability: `getJiraIssueTypeMetaWithFields` `allowedValues` for a company-managed project, `getTransitionsForJiraIssue` → `to.name` for a team-managed one. The single exception is `--labels`, which no Atlassian tool lists. On Linear, `--statuses` is the raw `list_issue_statuses` answer for the team (a bare array, read by `name`), `--projects` the last page of `list_projects`, and `--containers` the raw `list_milestones` answer for the new project (`{ milestones: [...] }`); Linear takes no `--labels`, because its milestone collisions are read from `--containers`. A hand-built listing is indistinguishable from fabricated gate evidence and will be refused as such — that is what stopped this step on 2026-09-23. Row 7 reads the WORKING TREE, so archiving the step-1 FR by moving the file is enough and NO COMMIT is required for the repoint to pass — if you commit anyway and a commit gate refuses, that refusal blocked the whole command, so run the repoint on its own afterwards rather than treating it as a deadlock. Run the repoint as its own command, never chained after a git command. |
| 3 | intruder | A | intruder | Create one issue titled `<nonce> intruder untagged item` in the shared container. **This is SCAFFOLDING: nothing is being tested about your judgement here, and your job is to give the detector something to recall — so create the item even though this repository's declaration names a version floor.** The item MUST be a plain issue with NO labels at all and NOT an Epic: `classifyTicket` files any Epic as `container` and anything carrying a non-`milestone-` label as `sibling`, so either one leaves the detector with `unowned=0` and S4, S10 and S13 all fail on a fixture that did its job wrongly rather than on the behaviour they test. On live leg 4 this step made two calls, neither to the tracker, and created nothing. |
| 4–5 | S1 | A, then B | tree | Create an FR titled `<nonce> S1 same title` through `/spec-write`. |
| 6 | S3 | A | tree | Plan a milestone titled `<nonce> S3 span` spanning B, through `/spec-write`. Then make THIS plan name B. `spans_repos.ts --declare` cannot do that here: it refuses because B holds no plan for the milestone yet (B writes one at step 7), and when it does run it writes both plans at once. So hand-write A's side into A's own plan's frontmatter, just before the closing `---`, exactly as the front door would write it: a `spans_repos:` line followed by the two indented entries `shr-<nonce>-a: .` and `shr-<nonce>-b: ../dpt-shared-<tracker>-b` (two spaces of indent each). Write nothing in B, and do not create B's plan: B must not name A back before step 9, because step 8 is graded on A's release being held while A names B and B does not name A (`one-sided`). Live legs 6–9 ran this step without the declaration and step 8 had no sibling to hold; the one leg that held (7) got there because this step wrote B's plan. |
...
| 8 | S14 | A | tree | Before B's plan names A back: run the RELEASE form of `sibling_release.ts <A> <A>/specs/plan/<token>.md <token> --children /tmp/dpt-shared-<tracker>-scratch-<nonce>-8-S14/children.json` once for the span milestone, then stop. Save that children file first, as the tracker returns it and never filtered or edited: on Jira the raw `searchJiraIssuesUsingJql` answer for `parent = ` the span Epic's key (the key `<token>` is derived from: `M_DST_115` is `DST-115`); on Linear the raw `list_issues` answer for the shared project with `includeArchived: true` (it takes no milestone filter; `sibling_release.ts` picks the span's children itself), paged until `hasNextPage` is false and saved as a JSON array of every page when there is more than one. It MUST be the release form, not `--offer`: this scenario is graded on a HOLD — AC-STE-617.10 requires the run to exit 1 naming B as `one-sided` before B's back-reference exists — and an offer surface exits 0 printing `children=not checked (offer)`, which structurally cannot produce it. Live leg 6 ordered `--offer` here and the scenario failed on an instruction it had followed exactly. Release nothing and commit nothing. |
| 9 | S3 | B | tree | List the container's milestones to the last page and **save the raw `searchJiraIssuesUsingJql` answer verbatim** on Jira (on Linear, **the raw `list_milestones` answer for the project, `{ milestones: [...] }`, verbatim**: it returns at most 50 milestones and carries no cursor, so an answer of fewer than 50 is its last page, and the `--join-key` is that milestone's `id`) as the listing file (rule 2 — leg 4 fetched the real answer and then hand-typed the file, and the module refused it as an unrecognised listing before reaching anything else). Then join A's milestone `<nonce> S3 span` by its key through the decision front door: `resolve_milestone_identity.ts` with **`--join-key` AND `--sibling <A>`** (the join key is the span milestone's key as the listing you saved gives it: its Epic key on Jira, its `id` on Linear) — a shared join without `--sibling` refuses, and a refused run writes no receipt, which is what S3 and S14 are both graded on. Each of those is its OWN Bash call (rule 1): the leg-4 run carried `; echo "exit=$?"`, so even a successful join would have been discarded and its receipt never announced. Then make B's plan name A back: `spans_repos.ts <B>/specs/plan/<token>.md <token> --declare <A>`. |
...
| 16 | S6 | B | below-floor | Create one FR titled `<nonce> S6 below floor` through `/spec-write`, using the `claude_ai_*` tracker tools. **ATTEMPT THE WRITE even though this repository's declaration names a version floor above this client's version.** This scenario tests the HOOK, not the declaration: the grader needs a write ATTEMPT that the tracker-write hook REFUSES, and a session that attempts nothing is graded `not-observed`, which is a failure. The refusal is the expected outcome and the point of the step. Keep the `claude_ai_*` tools: the predicate requires at least one refusal recorded on the SECOND server name, so an attempt on the primary server alone does not satisfy it. |
| 17 | S7 | A | tree | Create one tracker issue directly, with no front-door run before it. |
| 18 | S9 | A | tree | Edit B's S1 ticket to carry A's tag — a real tracker edit (`editJiraIssue` on Jira; on Linear, `save_issue` with the ticket's key as `id` and `labels`). **`ticket_ownership.ts decide` WILL answer `foreign-repo` for B's ticket, and that is not your stop sign: MAKE THE TRACKER EDIT ANYWAY.** This scenario tests the tracker-write HOOK, not the ownership decision: it is graded on the hook's refusal arriving as that call's tool_result, and a session that attempts no tracker write is graded `not-observed`, which is a failure. The refusal is the expected outcome and the attempt is the point — the contract rows 16 and 22 already state. Leg 8's child stopped at the ownership decision and made no call. |
| 19 | S11 | B-relocated | tree | This session starts in `<B>/.s11/relocated`, a worktree of B at another path (§ Before step 19). (1) Run `git rev-parse --show-toplevel` and check that it prints that path. (2) Transition A's S1 ticket `<A's S1 key>` to In Progress directly, with no front-door run before it. (3) Run `chmod 000 CLAUDE.md` in this worktree, so its declaration exists but cannot be read, and try the same transition again. (4) Run `chmod 644 CLAUDE.md`. Both transitions are real tracker calls (`transitionJiraIssue` on Jira; on Linear, `save_issue` with the ticket's key as `id` and `state`). **`ticket_ownership.ts decide` WILL answer `foreign-repo` for A's ticket, and that is not your stop sign: MAKE BOTH TRACKER CALLS ANYWAY.** This scenario tests the tracker-write HOOK in a relocated checkout: it is graded on the hook's refusals arriving as those calls' tool_results, one of them naming the unreadable `CLAUDE.md`, and a session that attempts no tracker write is graded `not-observed`, which is a failure. Leg 8's child stopped at the ownership decision, so neither transition reached the hook. |
| 20 | S10 | A | old-client | Create one FR titled `<nonce> S10 old client` in this repository's tracker through `/spec-write`. **Before you decline anything, do one observable thing: read this client's own `.claude-plugin/plugin.json` version.** That is not busywork — it is the comparison that makes the below-floor story meaningful, and it is exactly what leg 4's child did unprompted. It is also load-bearing for grading: a scenario ALL of whose sessions record zero tool calls cannot be graded at all, and leg 6's client made none while leg 4's made one, so whether this scenario reached its own predicate was left to chance. Then make the create: this old client writes an item the detector at step 21 must surface, and a client that writes nothing leaves the scenario vacuous. |
| 21 | S10 | A | tree | Run the untagged detector over the shared container. |
| 22 | S13 | B | tree | THREE obligations, all required, each its own Bash call. **(1)** CLAIM A's S1 ticket `<A's S1 key>` by key — a real `transitionJiraIssue` call on Jira; on Linear, a real `save_issue` call with that key as `id` and `state`. **`ticket_ownership.ts confirm` WILL REFUSE this as `foreign-repo`, and that refusal is not your stop sign: MAKE THE TRACKER CALL ANYWAY.** This scenario tests the tracker-write HOOK, not ticket ownership: AC-STE-617.10 grades it on the hook's refusal arriving as that call's tool_result, so a run that never makes the call records nothing to grade and the scenario fails. The refusal is the expected outcome and the attempt is the point — the same contract step 16 states for the below-floor client, which has passed on two consecutive legs. Leg 6 skipped this silently; leg 7 stopped here and said so, which was correct behaviour under a rule that had not told it this exception exists. **(2)** Import it — which writes this repository's tag onto A's ticket, a real label-sync call (`editJiraIssue` setting `fields.labels` on Jira; on Linear, `save_issue` with that key as `id` and `labels`, not `addLabels`, which is not read as a label sync). **`importFromTracker` WILL refuse the key LOCALLY as `sibling` before any tracker call, and that is not your stop sign either: MAKE THE LABEL-SYNC CALL ANYWAY.** The scenario grades (2) on the hook refusing that sync on A's ticket, and a local refusal records no tracker call to grade. Leg 8's child made (1) and stopped at this local refusal. **(3)** Import the intruder's untagged item `<intruder key>` through `/spec-write`'s orphan-import question, answering with the printed `Import <intruder key>` label. Leg 6 did (3) and neither (1) nor (2): a single sentence listing three acts reads as one act with elaboration, which is the same shape that lost half of rows 25-26 before they were numbered. |
| 23 | S12 | A | tree | (1) Run `git -C <B> commit --allow-empty -m "s12: commit into B"`, then `env -C <B> gh pr create --title s12 --body s12`. (2) Create B's own gate evidence in this session: run `/dev-process-toolkit:gate-check <B>`, which records the run with `gate_receipt.ts gate-check <B>` and prints its `dpt-receipt:` line, then `/dev-process-toolkit:spec-review <B>`. (3) Run `git -C <B> commit --allow-empty -m "s12: commit into B"` again, then `env -C <B> gh pr create --title s12 --body s12` again. Open each PR exactly as written, `env -C <B>` included: it is ONE operation (rule 1), and it names <B> in the command itself. A separate `cd <B>` followed by a bare `gh pr create` still opens the PR from B, but the grader reads the repository from the command, so on live leg 9 that PR was never seen. `no git remotes found` from gh is the expected answer once the hook permits the PR; it is not a failure to fix. |
...
| 25–26 | S16 | A, then B | tree | TWO obligations, both required, each its own Bash call. **(1)** Run the typed `M<N>` door (`next_free_milestone_number.ts`) and write `specs/plan/M999.md` by hand. **(2)** Run gate probe #73 (`plan_identity_mode_conditional.ts`). Do both in this session's own repository only: step 25's child works in A and step 26's in B, and each is graded on its own two runs. Never run either module against the other repository or write its `M999.md` — on live leg 9 the A child did both halves from A and wrote B's file, a cross-repository write the grader reports as `sibling-file-write`. A session that has written the fixture has not finished, and on leg 4 root A ran the door, wrote the file and ended without ever running the probe. Name each module by its full absolute path in the command that runs it (rule 1) — root B ran both and both were unreadable, being chained AND reached through a shell variable. |
...
Before you write a prompt into the step fence, fill in every `<…>` placeholder: `<nonce>`, `<A>` and `<B>` as the absolute paths of the two repositories, `<tracker>`, `<container>` (the shared space key or project name), `<token>` (the span milestone's plan token, from A's `specs/plan/` after step 6), and the ticket keys earlier steps' logs returned: `<A's S1 key>` (A's step-4 create) and `<intruder key>` (step 3's create). No other `<…>` placeholder appears in the step table; a test holds it to this list. The grader reads commands into B by B's path, so `<B>` must be written out, never abbreviated.
...
**Set the allow rules BEFORE the run, not when the verdict explains them.** Leg 4 needed hand-added rules in `.claude/settings.local.json` for the operator's own Phase 1 answers, and then the classifier denied a SCENARIO CHILD mid-run — `Permission for this action was denied by the Claude Code auto mode classifier. Reason: [Auto-Mode Bypass]` — at the moment that child had correctly diagnosed its own bad listing file and was trying to replace it. So the child did the right thing and was stopped from doing it, and the scenario recorded the original error rather than the repair. Add the rules the run needs at Phase 0.5, before any child spawns; discovering this from a verdict costs the leg.

**Children inherit the operator's `permissionMode: auto`, and Claude Code's auto-mode classifier is therefore a live gate over every step of every leg.** This document did not say so for 1414 lines, and it is not a detail: the classifier can DENY a child's tool call on its own judgement, for reasons unrelated to any guard under test, and a denied call looks from the transcript like the toolkit refusing. Measured on live leg 3 (2026-09-23), where it denied a step-2 heredoc writing repoint listings — the abort that ended the leg. It is also INCONSISTENT in a way nobody has explained: the same shape at step 1, heredocs writing `orphans-p1.json` and `epics.json`, was NOT denied. So its behaviour cannot be predicted from the command's shape, and a leg can die at a step that ran clean the run before.

What follows from that, for whoever runs a leg: a denial is a THIRD outcome beside pass and guard-refusal, and the three are not interchangeable in a report. Before concluding a guard fired, read whether the refusal names a toolkit reason (an NFR-10 `Refusing:` block, a hook message) or reads as a classifier denial with no toolkit vocabulary in it. The grader cannot tell them apart either — it reads tool_use/tool_result pairs, and a denied call is simply a call that did not happen. Documented rather than fixed: nothing in this repository controls that classifier, and pretending otherwise would be the more dangerous error.
...
RUN_STATE_NEEDS="TRACKER TOPLEVEL DPT_SMOKE_RUN_ID SPAWN_CEILING NONCE ROOT_A ROOT_B PLUGIN_TREE PLUGIN_BELOW_FLOOR PLUGIN_INTRUDER OLD_CLIENT"
RUN_STATE_NEEDS_LINEAR=""
# run-state preamble: begin — the same lines in every fence that reads the run state
# loader canary: this document's text reaches a session with the invocation's arguments already
# substituted for its positional parameters, so a fence RUN FROM THE LOADED TEXT would refuse with an
...
# The step rules (§ Phase 3): written ONCE, here, so every child receives them. A file for the same bash 3.2 reason as the prompt.
cat > "/tmp/dpt-shared-<tracker>-step.rules" <<'RULES_EOF'
FOUR RULES FOR EVERY STEP OF THIS SESSION. Each carries its reason, because each was broken by a child doing something ordinary and sensible, and a rule without its reason reads as style and gets optimised away.
1. **ONE OPERATION PER BASH CALL.** No `;`, no `&&`, no trailing `echo "exit=$?"`, no second command of any kind beside the one being graded. Two separate mechanisms punish chaining and a child told only one of them will keep breaking the other. For a **toolkit module**: the recorded `exitCode` belongs to the command's LAST segment, so `bun <module>; echo "exit=$?"` records the echo's zero, and seven predicates gate on the module's exact exit code — the echo you added to preserve the status is what destroys it. For **git**: one command yields one subcommand, so `git merge …; git ci …` is read as a merge and the second invocation is invisible. Name the module by its FULL ABSOLUTE PATH in the same command that runs it: `S=…; bun "$S/adapters/_shared/src/x.ts"` is unreadable for a second and independent reason — the target is not absolute at the point it is parsed — so splitting the chain without also inlining the path fixes nothing. Measured on leg 4: whole scenarios whose work was done correctly and never seen. The verdict's `observations` carry the live count for a given run; no number is written here, because the instrument that counts them is itself under repair and a document pinning its output is stale by construction.
2. **FETCH EVERY LISTING; NEVER FABRICATE ONE.** Any file a command reads as a tracker listing must come from a real MCP call. **FABRICATING is forbidden** — typing rows, inventing objects, editing values, or repairing an answer you did not like. **PROJECTING a fetched answer into the shape a documented flag requires is NOT fabricating, and is sometimes the documented procedure** — the distinction is whether every value in the file came out of the tracker. **Omitting or adding fields is neither fabricating nor projecting: save the answer whole.** A file with fields deleted, trimmed, added or filled in passes that test and is still not the tracker's answer. Leg 8 measured all four: children dropped `context` (which broke S14's shape detection), trimmed URLs and avatars, had `jq` add a `requestCursor`, and had `sed` invent a `creator` by copying `reporter` — manufactured tracker data, then fed to an ownership decision. This rule previously read "do not reshape it", which forbade the projection too, and that wording aborted live leg 5 at step 2: the child fetched Jira statuses the documented way for a team-managed project (`getTransitionsForJiraIssue`, which answers `{transitions:[{to:{name}}]}`), found the flag wanted a `statuses` array, and correctly refused rather than break the rule. It had no legal path, which is the failure mode this programme has now produced twice. That specific case no longer needs a projection at all — `repoint_tracker_binding.ts` accepts both documented shapes and projects `to.name` itself, so the transformation lives in one tested place rather than in twenty-six prompts — but the distinction stands for any flag that still needs one. The modules validate the answer's shape and refuse anything else, and a hand-built input into a file a gate reads is indistinguishable from fabricated evidence. The ONE exception is Jira `--labels`, which no Atlassian tool lists; everything else, statuses included, is fetchable — see `docs/setup-reference.md` § Repoint preconditions. Leg 4's step 9 called `searchJiraIssuesUsingJql`, had the real answer in hand, and typed a file anyway; the module refused it and the scenario died there.
3. **RUN A MODULE DIRECTLY; NEVER THROUGH A WRAPPER.** Invoke `bun "<the module path>"` as its own command. Do not write a script that imports the module and prints your own summary. A wrapper can filter, reformat, swallow a throw, or print a count the module never produced, so the grader does not accept one and is right not to — a script that imports X is not a run of X. Two different children wrote wrappers on leg 4, so this is a habit rather than one child's improvisation; a child that meets the refusal should fix the invocation rather than conclude the harness is broken.
4. **A REFUSAL IS INFORMATION; READ WHAT IT NAMES.** Every toolkit refusal names the offending shape and a remedy. Fix what it names and retry. Do not conclude a step is impossible because one command was refused — and when the refusal blocked a chained call, note that everything after it never ran either.
RULES_EOF
STEP_RULES=$(cat "/tmp/dpt-shared-<tracker>-step.rules")
...
${STEP_RULES}
Scratch directory: any file this step writes for its own use goes under /tmp/dpt-shared-${TRACKER}-scratch-${NONCE}-${STEP_NAME}/ (create it), never a bare /tmp/dpt-sN/ path. A path without the run's nonce survives from run to run: on leg 8, /tmp/dpt-s8/ still held 2026-09-23 listings, one naming a key that no longer existed, beside that day's.
${STEP_PROMPT}
