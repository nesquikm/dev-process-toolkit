...
2. **FETCH EVERY LISTING; NEVER TYPE ONE.** Any file a command reads as a tracker listing must be the raw MCP answer, saved verbatim. Do not hand-build it, do not repair it by hand, do not reshape it. The modules validate the answer's shape and refuse anything else, and a hand-built input into a file a gate reads is indistinguishable from fabricated evidence. The ONE exception is Jira `--labels`, which no Atlassian tool lists; everything else, statuses included, is fetchable — see `docs/setup-reference.md` § Repoint preconditions. Leg 4's step 9 called `searchJiraIssuesUsingJql`, had the real answer in hand, and typed a file anyway; the module refused it and the scenario died there.
