...
| 22 | S13 | B | tree | THREE obligations, all required, each its own Bash call. **(1)** CLAIM A's S1 ticket `<A's S1 key>` by key — a real status transition; AC-STE-617.10 requires a recorded claim transition and leg 6 made no `transitionJiraIssue` call at all. **(2)** Import it. **(3)** Import the intruder's untagged item `<intruder key>` through `/spec-write`'s orphan-import question, answering with the printed `Import <KEY>` label. Leg 6 did (3) and neither (1) nor (2): a single sentence listing three acts reads as one act with elaboration, which is the same shape that lost half of rows 25-26 before they were numbered. |
