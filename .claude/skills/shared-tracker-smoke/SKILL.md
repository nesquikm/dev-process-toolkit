...
**Four rules every step obeys.** They are here rather than repeated per row because each was violated in several different rows of live leg 4, and a rule copied into four prompts goes stale in three of them. Every rule carries its reason: each was broken by a child doing something ordinary and sensible, so a rule without its reason reads as style and gets optimised away.
