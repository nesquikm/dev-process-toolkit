...
| 25–26 | S16 | A, then B | tree | TWO obligations, both required, each its own Bash call. **(1)** Run the typed `M<N>` door (`next_free_milestone_number.ts`) and write `specs/plan/M999.md` by hand. **(2)** Run gate probe #73 (`plan_identity_mode_conditional.ts`). The scenario is graded on BOTH module runs in BOTH roots; a session that has written the fixture has not finished, and on leg 4 root A ran the door, wrote the file and ended without ever running the probe. Name each module by its full absolute path in the command that runs it (rule 1) — root B ran both and both were unreadable, being chained AND reached through a shell variable. |
