...
│   ├── skills/                          # 18 user-invocable SKILL.md files
...
- **Canonical-key closed set (STE-114 AC-STE-114.1).** Top-level keys are limited to exactly `{mode, mcp_server, jira_ac_field, branch_template}`. Sub-section contents (`### Linear`, `### Jira`, etc.) are scoped out and free-form. Non-canonical top-level keys → `/gate-check` failure via the `task-tracking-canonical-keys` probe (gate-check #21). One-time migration helper for projects that picked up the drift before the constraint landed: `scripts/migrate-task-tracking-canonical.ts` (dry-run; outputs unified diff). Full rationale: `docs/patterns.md` § Schema L Canonical keys. **Shared-container declaration (sub-section keys, STE-602).** Two optional keys of the ACTIVE `### Jira` / `### Linear` sub-section, never top-level: `repo_tag: <lowercase-kebab>` (its presence IS the declaration that the container is shared; the tag must also sit in `default_labels`) and `min_dpt_version: <X.Y.Z>` (the lowest toolkit version allowed to write into the container). `readWorkspaceBinding` returns them with `shared: true` exactly when a non-empty `repo_tag` is declared, and refuses a malformed declaration with a typed NFR-10 error in seven classes: a non-kebab tag; a tag missing from `default_labels` (`RepoTagBindingError`); a non-strict floor; one key without the other; either key written twice; a floor below `FIRST_GATED_DPT_VERSION`; a CLAUDE.md that exists but cannot be read. Full contract: `docs/patterns.md` § Shared tracker container declaration. **Receipt store (STE-602, STE-607):** in a shared repository each deciding command (`create_idempotency_probe.ts decide`, `container_ownership.ts consent`, `ticket_ownership.ts confirm`) writes one JSON receipt under `.dpt/ledger/receipts/<session>/` (`receiptsDir`, keyed by `CLAUDE_CODE_SESSION_ID`, git-ignored) and prints `dpt-receipt: <absolute path> sha256:<digest of the file>`. The `pre-tracker-write-gate` PreToolUse hook honours a receipt only when that line — the only one — sits in the non-error result of a Bash call that is one plain `bun [run] <this plugin's deciding module> <its receipt-writing subcommand>` invocation (`decide` / `consent` / `confirm`; `--title-file <path>` carries a title a plain command cannot quote) and the file still hashes to the announced digest. A `create` receipt authorises exactly one create tool_use, allocated in transcript order with pending parallel calls counted. Contract: `docs/hooks-reference.md` § pre-tracker-write-gate.
...
`id:` is **mode-conditional**, mirroring the FR-side bimodal identity invariant. In `mode: none` a **minted** plan carries the full `fr_<26-char ULID>` that `Provider.mintId()` returned, verbatim — the plan's `M_<short-ULID>` filename is the 6-char tail of that value, so the key is what makes the identity reconstructable. In any tracker mode the key is **absent** (the tracker assigns identity). Enforced bidirectionally at gate time by probe #73 `plan_identity_mode_conditional`, which additionally asserts a recorded `id:` derives its own filename. Epic-keyed plans carried over by a tracker → `mode: none` transition are grandfathered unconditionally and never required to carry the key. Sequential `M<N>` plans are grandfathered by **git-introduction provenance**, not by filename shape: a plan whose introducing commit predates the mint epoch is legacy and stays silent, one introduced at or after it reads as a mis-named new plan and fails, and one git cannot date raises an advisory. The same provenance machinery runs a **second arm under `mode: jira`**, keyed on the mode string rather than on a not-tracker-less boolean and dated against its own later epoch (`JIRA_EPIC_EPOCH`, the release that made sequential Jira plans legacy once milestones became Epic-keyed); `mode: linear` runs neither arm, since sequential numbering is Linear's specified behaviour, and the tracker-mode `id:`-absence rule fires under every tracker mode alongside it. Adopting minted ids still needs no migration for genuinely old plans; the frontmatter opt-out `kind: legacy` (alongside the existing `kind: scaffolding`) silences the check permanently for a plan the operator declares old.
...
2. **Canonical regeneration** (`/docs --full`, invoked by `/ship-milestone` after its `/docs --commit` call — the flags are mutually exclusive, so the ship step is two sequential invocations). Rewrites the entire `docs/` tree under the Diátaxis layout (Schema B anchors). Packages-mode reference content is driven by `SignatureGroundTruth` (Schema Z); user-facing content is LLM-generated prose + mermaid. Both `--full` and `--commit` require explicit human approval on the unified diff before any file write.
...
| `## /setup audit` section in CLAUDE.md | Created by `appendAuditEntry` (`adapters/_shared/src/setup/audit_log.ts`) on every Schema L resolution at steps 7b/7c/7d — user-supplied or default-applied — driven by step 8a's deterministic post-condition. Bullet shape: `- <ISO-date> step:<N> (<field>) value:<JSON-quoted> reason:"<reason>"` (STE-108 AC-STE-108.7 append-only; STE-153 AC-STE-153.1 unconditional synthesis). | #19 `setup-audit-section-presence` |
...
| Bun zero-match placeholder test | Bun-stack only; written into `src/`, co-located with the source it shields, under the canonical layout (the legacy `tests/`-mirror layout puts it under the mirror root instead — exact filename in `docs/setup-reference.md`). Carries the marker comment `// generated by /dev-process-toolkit:setup — Bun zero-match workaround (see examples/bun-typescript.md)` until a real `*.test.ts` ships. Prevents `bun test`'s zero-match-exit-1 (STE-113 AC-STE-113.3). | #20 `bun-zero-match-placeholder` |
...
| `skills/implement/SKILL.md` or `skills/gate-check/SKILL.md` approaching NFR-1 cap | Medium | Tier 1 `wc -l` checks each release; standing overflow rule extracts to companion doc |
