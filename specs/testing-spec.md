...
| Line count | `wc -l` per SKILL.md | Every skill ≤ 358 lines (NFR-1) |
| Frontmatter | YAML parse | Valid `name`, `description` fields; `argument-hint` where the skill takes arguments (conditional on the surface, not a fixed list: `/pr` carried none until it gained `--draft`, and now carries one) |
...
| v2-minimal regression (all 27 skills) | `tests/fixtures/v2-minimal/` | `…/snapshots/<skill>.snap` |
