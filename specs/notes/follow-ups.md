...
## M_4df444 (v2.84.0): what the milestone created, and what it found

### Created by this milestone

- **A smoke run's session debris is found by identity and removed by exact path (STE-593).** `smoke_session_cleanup.ts` plans one run's debris across ten stores from the run's recorded session ids and a time window, leaves live and kept sessions alone, deletes nothing without `--delete`, re-validates every path at delete time and exits non-zero naming any survivor. The four stores that carry no session id (config temps, plugin orphans, cmux temps, zcompdump locks) are planned only inside a window, and a config temp only when its writer's pid is dead.
- **A spawn script cannot be fed to bash through stdin (STE-595).** A repo-level `PreToolUse` hook in the tracked `.claude/settings.json` refuses a `bash`/`sh`/`zsh` that reads a backgrounding or unseen script from stdin. Every spawn site states the run-from-a-file rule, and every backgrounding spawn fence ends with a live-child count that aborts on a mismatch.
- **Every spawned smoke session carries a recorded id (STE-594).** Each `claude -p` spawn mints a session id, records it in a per-run ledger under `.dpt/ledger/` before it spawns, and passes `--session-id`. The loop's Termination removes a leg's recorded sessions only when that leg's verdict artifact reads `pass`.

### Found by this milestone

- **F1. The transcript-less evidence class over-admits across a long window.** AC-STE-593.1 admits a session whose only evidence is an empty `session-env` dir. That class was sized for a runaway's window (2026-09-11, about 40 minutes). Over the three-week manual window of the 2026-09-15 sweep it admitted `41fa26f3` (born 2026-09-01T17:35, no transcript, telemetry, MCP log or registry entry), which nothing ties to a smoke run. The delete would have been one empty directory, so it is harmless, but it breaks "only the run's own". Possible fix: outside a short window, require positive smoke evidence for a transcript-less session (an MCP log whose cwd is a run cwd, or `sdk-cli` non-interactive telemetry). The operator's `[b]` choice kept it out of that sweep. AC semantics were not changed in this milestone.
- **F2. The run-from-a-file rule reopens STE-185's content-swap window.** STE-185 delivers prompt bodies on stdin so no prompt file sits on disk. Writing a prompt-bearing spawn fence to a file for `bash <file>` puts the prompt heredocs and the auto-approve marker back on disk, and bash reads a script file as it executes, so a swap mid-run changes the lines still to come. Mitigation: write the fence with `mktemp` outside the working tree and run it in the same Bash call (the detector already allows that shape).
- **F3. `Bash(bash:*)` is broad by operator choice.** The operator chose it at 14:00 +04 on 2026-09-15 over a path-scoped `Bash(bash /tmp/dpt-:*)`, knowing it also allows `bash -c '<anything>'` on both the tracked list and the `/smoke-test` scaffold. Recorded so a later reader does not mistake it for an oversight.
- **F4. The smoke live-child identity check assumes `claude` execs.** The `/smoke-test` count accepts a pid whose `comm` is `claude` or `*/claude`. Under cmux the `claude` on PATH is a shim that execs `cmux-claude-wrapper`; if any wrapper layer forks instead, `$!` names the wrapper, the count reads `live=0` and aborts, and the reap skips the real children before Phase 5's `rm -rf`. Verify at the AC-STE-594.9 / AC-STE-595.8 live run.
- **F5. Phase A's identity check is weaker than the smoke check.** It compares a recorded pid's `comm` with the forking shell's own (`bash`), so any bash process that inherits a recycled pid passes. It meets the AC wording.
- **F6. A per-leg abort names the teardown rather than running it.** On a live-child mismatch the Phase A fence names § Per-leg abort teardown and exits 1; the driver then runs that recipe. It was not copied inline because the recipe is written once on purpose.
- **F7. A failed ledger append does not stop its spawn.** Every spawn's ledger append runs without a guard, so a refused append still launches a child the ledger does not name. That leaves a session behind rather than deleting anything, but it contradicts STE-594's note that recording precedes spawning. A plain `|| exit` would put an exit between one spawn and the fence's live-child count, which STE-595 pins against, so the fix needs a small design (skip that spawn and count it as not launched), not a patch.
- **F8. The run id lives in `/tmp`, the ledger does not.** The ledger survives a `/tmp` wipe; the file that names the run, `/tmp/dpt-conformance-loop-${DATE}.run`, does not. If it is gone at Termination, the kept-leg manual command prints with no `--session` and plans nothing, and the ledger file is orphaned. Fails safe.
- **F9. Verdict freshness is per run, not per iteration.** Termination judges an artifact against the run-id file's mtime. In a multi-iteration run, an earlier iteration's `pass` would still read fresh if a later iteration's leg died before rewriting it. RC collection aborts that iteration first (a missing or stale verdict reconciles non-zero), so Termination should never see it; recorded with that reasoning.
- **F10. Standalone `/smoke-test` scatters one run across ledger files.** Each spawn fence mints its own run id when no loop exported one, because every Bash call is a fresh shell.
- **F11. Phase B's `autofix` sessions are recorded but never cleaned or listed.** List them with `bun <plugin dir>/adapters/_shared/src/smoke_run_ledger.ts sessions --run <run> --leg autofix` and pass those ids to the manual cleanup command.
- **F12. The smoke-test appends hardcode `--project-root /Users/ns/workspace/dev-process-toolkit`.** Phase A and Termination resolve the ledger from the current directory, so a loop run from a worktree would write grandchild rows where Termination never reads them. Sessions are left behind; nothing is deleted wrongly.
- **F13. The older loop fences still rely on field splitting.** § Per-leg abort teardown (`for SEL in ${SELECTED_LEGS}`, `for SEL in ${SPAWNED_LEGS}`) and pre-flight (h)'s remedy predate this milestone and, run inline under zsh, iterate once with every leg in one word. This milestone fixed the same pattern only in the new Termination fence (split through `$( … )`, delete arguments built as positional parameters), with a zsh-run test.
- **F14. The `/smoke-test` live-child count covers a whole fence, not each spawn.** The docs run `/setup` → `/spec-write` → `/implement` one at a time with polls between, but the count sits once at the fence's end, so run as documented only the last spawn is counted. A count after each spawn, like the retry example's, would fix it; it changes a shape STE-595's tests pin.
- **F15. The stdin-spawn detector over-refuses one harmless shape.** A single-quoted `'$(x &)'` inside a script fed to a shell through stdin is refused, because the lexer strips quotes before the substitution scan. That is the direction the rule prefers; the remedy is the same run-from-a-file form.
- **F16. Manual mode cannot see MCP-log residue whose transcripts are already gone.** After the 2026-09-15 sweep, 0 `projects/*dpt-test-project*` dirs remain, but their 27 twins under `~/Library/Caches/claude-cli-nodejs/` still held 377 MCP logs (3,023,379 bytes, 140 distinct first sessionIds, 2026-07-19T17:06Z to 2026-08-16T15:54Z, every cwd a dpt-test-project path, 0 overlapping the live registry; measured by the supervising session). They predate the sweep window, and rule (b) admits an MCP-log-only session only when every cwd is passed as `--run-cwd`, because the dpt-slug matching applies to projects dirs, not to MCP-log cwds or cache slug dirs. The survivor check re-lists planned paths only, so the residue reads as "survivors 0". Offered as a post-release dogfood run of the shipped CLI: dry run first, then the operator's go.
- **F17. The Phase A mismatch path's pidfile removal trips zsh's NOMATCH.** `/conformance-loop` removes the iteration's pidfiles with an unquoted `/tmp/dpt-conformance-loop-…-iter-*.pid` glob; under zsh, when no pidfile exists, the glob raises NOMATCH instead of expanding to nothing. It fires on the live-child mismatch path only, and only when no pidfile was written; `setopt local_options null_glob` (the idiom the smoke driver's self-check already uses) or a loop over the recorded pids would close it. Found by the supervising session's pre-review, 2026-09-15.
- **F18. The zsh smoke-fence test pins the appends, not the reap loop.** `tests/m_4df444-zsh-spawn-fences.test.ts` asserts, under zsh, that every `/smoke-test` append records its run, leg and parent, but its smoke case never reaches the mismatch branch, so the `for P in $(printf '%s\n' "${PIDS}")` reap-loop fix in the two `/smoke-test` count blocks is unpinned under zsh. A case that forces a mismatch (a directory where one pidfile belongs, as the STE-595 fence-run test does) would pin it.
- **F19. A live session with no `startedAt` gets no snapshot protection.** The shell-snapshot rule keeps every snapshot born at or after the earliest live session's `startedAt`; a live registry row without a numeric `startedAt` contributes nothing, so its idle session's snapshot can still be planned. Every row measured on 2026-09-15 carried one; a registry-shape change would silently reopen the gap. Falling back to the registry file's mtime, or refusing to plan snapshots while any live row lacks a start time, would close it.
- **F20. A live session's next token capture erases its own `claimed_by` stamps.** `writeSessionRows` (`adapters/_shared/src/token_usage.ts:63-81`) drops every ledger row recorded for the capturing session and appends rows freshly re-derived from its transcript, and those carry no `claimed_by`. A claim made while that session is still live, such as `/implement`'s in-session archival claiming its rows for an FR, is therefore erased by the same session's next Stop or SessionEnd capture; rows from other sessions keep theirs. That is why M_4df444's archived plan rollup shows skill labels instead of FR lines. Carrying `claimed_by` over from the dropped rows (matched on session and scope) would close it. Found by the ceremony worker at `/ship-milestone`, 2026-09-15, and confirmed against the source by the supervising session.
- **F21. A passed leg's id-less leftovers survive the automatic cleanup by design.** Termination deletes only a passed leg's recorded session ids and passes no time window, so the shell-snapshot planner and the four id-less planners plan nothing and whole-dir removal stays manual-only: each passed leg's shell snapshots, any orphans it births and its emptied `dpt-test-project-*` project dirs remain (a few small files per run; the test-project names are fixed per leg, so the dirs do not pile up). On 2026-09-15 the operator chose to narrow AC-STE-594.9 to the session-keyed stores rather than widen Termination, because matching by time is riskier (see F23). A periodic manual-mode sweep takes the rest. Found by the pre-PR `/spec-review`, 2026-09-15.
- **F22. The token-ledger compare-and-swap narrows the STE-344 lost-update gap; it does not close it.** The real concurrent writer, `writeSessionRows`, rewrites the whole file through temp and rename (`token_usage.ts:90`). A hook rename landing between the cleanup's compare and its rename is lost, and a hook whose read precedes the cleanup's rename and whose rename follows it brings the dropped run rows back. The retry test models the intruder as an append, not as the rename-shaped writer it really is. A shared lock, or a capture that appends instead of rewriting, would close it. Found by the pre-PR `/spec-review`, 2026-09-15.
- **F23. The plugin-orphan planner consults no liveness probe.** Unlike the other id-less planners, `planPluginOrphans` plans a `temp_git_*` clone or marketplace `.bak` that no plugin state file names without asking whether any process holds it, so inside a manual window a live session's in-flight clone is plannable and passes delete-time re-validation. It meets AC-STE-593.2; the v2.84.0 CHANGELOG line was corrected before the push so it no longer claims a liveness check. An `isHeld` probe on the candidate would close it. Found by the pre-PR `/spec-review`, 2026-09-15.
- **F24. The cmux temp-file name shape is a guess.** `CMUX_TMP` (`claude-hook-sessions.json(.<n>)?.tmp`) was never measured: on 2026-09-15 `~/.cmuxterm` held only `claude-hook-sessions.json` and its `.lock`, and the 09-15 cleanup manifest recorded no cmux names. If the guess is wrong, the `cmux_tmp` store always reports 0, as if clean. It fails safe; measure the real name the next time a cmux hook leaves one behind. Found by the pre-PR `/spec-review`, 2026-09-15, and already confirmed by the operator as a deviation at the commit gate.
- **F25. Two traceability rows list test helpers as implementation.** The `specs/requirements.md` rows for AC-STE-594 and AC-STE-595 name `tests/_ste594_harness.ts`, `tests/_spawn_fences.ts`, `tests/_stdin_spawn_hook.ts` and three test fixtures in the implementation column. The implementation column should name only shipped code. Found by the pre-PR `/spec-review`, 2026-09-15.
- **F26. CLAUDE.md's Structure block predates the repo-level hook.** It names `.claude/skills/` but neither `.claude/hooks/` nor the hooks block now in the tracked `.claude/settings.json`, and its plugin `hooks/` line reads as the complete list of blocking PreToolUse gates. One line under Structure would fix it. Found by the pre-PR `/spec-review`, 2026-09-15.
