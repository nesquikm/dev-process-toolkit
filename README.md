...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v1.29.0 — "Runbook"** (M28 closes the M26 dogfood failure mode — `/implement`'s abstract `Provider.claimLock` / `Provider.releaseLock` references read as "skip tracker writes." Single FR (STE-101): `docs/implement-tracker-mode.md` gains two adapter-agnostic runbook sections (Claim, Release) with concrete `mcp__<tracker>__get_issue` / `transition_status` sequences, four-way decision routing (`claimed` / `already-ours` / `taken-elsewhere` / `already-released`), and silent-no-op-trap post-write verification. `skills/implement/SKILL.md` replaces the Provider API references with runbook pointers, adds a new Phase 1-exit self-check (step 0.e "Claim verification" — re-fetch + assert `status_mapping[in_progress]` + `assignee == currentUser` + NFR-10 hard-refusal on mismatch), and rewrites the forbidden rule into operational form. New `tests/implement-tracker-claim-runbook.test.ts` (25 prose-shape assertions); shared `tests/_skill-md.ts` helper. Test count: 702 → 727.).
