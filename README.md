...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.90.0 — "Proven Live"** (M_2306b6, the shared-tracker programme is proven live. Two real repositories bound to one real Jira space and one real Linear project run every live scenario through real sessions: same-title creates, milestone mint and join, the sibling-busy release hold, the below-floor and old clients, the hook-less intruder, the repoint, claim and import ownership, and the commit and pull-request guards across repositories. A program grades each run from the recorded tool calls, tracker answers and files on disk, never from what a session says it did, and the test suite re-grades both committed runs against the code being released, so this release could not ship until both passed. An offline suite does the same against tracker doubles that honour their own queries, and a mutation matrix breaks each guard in turn to show it can fail. Getting there took fifteen live legs; the defects they found, in the grader, the smoke's own instructions and one hook that spent a receipt on a write it had refused, are fixed here)
