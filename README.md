...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v1.44.0 — "Tripwire"** (M44 extends `.claude/skills/smoke-test/SKILL.md` with a `--tracker linear|jira` flag + `--jira-project <KEY>` argument so the Tier 5 dogfood driver covers both adapter modes. Pre-flights #7 (`atlassianUserInfo` OAuth probe) and #8 (`getVisibleJiraProjects` Space probe) gate the Jira path; Phase 1 step 4/6 and Phase 5 teardown each carry parallel sub-branches; teardown uses `JQL: project = <KEY> AND labels = "dpt-smoke" AND created >= "<run-start>"` + `transitionJiraIssue` to Done (Atlassian MCP exposes no delete tool — the Space is reused across runs and only run-window labeled items get closed). `UpsertMetadataInput` widens with `labels?: string[]`; `/spec-write` forwards `defaultLabels` from `readWorkspaceBinding(...)` into both adapters (Linear → `save_issue.labels`, Jira → `createJiraIssue.additional_fields.labels`). Smoke #6 against DST provided retroactive validation for M43's AC-STE-154.9 steps 2–6; smoke #7 confirmed byte-for-byte Linear back-compat. Test count: 1215 → 1217 (+2).
