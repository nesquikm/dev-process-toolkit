...
| `/tdd`            | RED → GREEN → VERIFY cycle                                                                                                                                                                                                                                         | `<FR-id>`                                                                  |
...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.28.0 — "Refused"** (M81 — closes the regression class where `/spec-write` draft + branch gates and `/setup` Socratic first-turn auto-applied under `claude -p` non-tty stdin when the `<dpt:auto-approve>v1</dpt:auto-approve>` marker was absent. STE-313 lands a consolidated single-call arbiter `evaluateGateMarkerRefusal` collapsing the four-state `(marker × stdin)` matrix into `apply` / `prompt` / `RequiresInputRefusedError` (NFR-10 shape, naming the gate site), three verbatim conformance-loop iter-1 regression fixtures + a replay test loop, two new /gate-check probes #55 `not_a_trigger_anchor_present` + #56 `marker_helper_invoked_per_gate` (both severity: error), and NOT-a-trigger anchors in `## Rules` of both /spec-write and /setup SKILL.md. Tech Design vector (1) PreToolUse hook deferred to a follow-up FR — the existing two-step `check_marker_runtime` + `requireOrRefuse` pattern produces byte-identical refusal. 50 new tests; total 2131. Forward-only per `project_no_users_yet`.)
