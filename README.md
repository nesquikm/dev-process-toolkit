...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.17.0 — "Socratic"** (M66 — closes the autonomous-mode contract on the symmetric Socratic-loop side and the byte-checkable capability-row emission side. STE-237: universal `AskUserQuestion` mandate + first-turn contract — `Write` / `Edit` / `NotebookEdit` forbidden before the first `AskUserQuestion` `tool_use` OR `RequiresInputRefusedError` raise; new pure-I/O helper `assertFirstTurnShape(transcript)` is the single arbiter; new `/gate-check` probe #43 `socratic_loop_uses_ask_user_question` enforces every Pattern-26-tagged skill body cites `AskUserQuestion` + `docs/auto-mode-protocol.md`; new `/smoke-test` Phase 8 spawns `claude -p` children with autonomous-mode reminder + no marker and asserts the first-turn contract; `## /setup audit` row format gains `loop_entered: true|false` column for two-axis loop visibility with STE-232's `imputed:`. STE-238: literal-token `MUST emit \`<key>\`` directives at every `/spec-write` emission site (draft / commit / branch-gate / spec-research); new static-map entry `branch_gate_skipped_already_non_main`; new `/gate-check` probe #44 `closing_summary_capability_keys` greps for the literal directives; new `/smoke-test` Phase 9 lenient-assertion fixtures verify runtime token emission; vendor-neutral `renderProbeSkipReason(...)` helper forbids the F9 `"require Linear MCP"` paraphrase. Triggered by the magpie incident (gist `https://gist.github.com/nesquikm/2904e50c7213b6aa392b998d4137f609`) + `/conformance-loop` iteration 1 (F1+F3+F4+F8+F9). Test count: 620.
