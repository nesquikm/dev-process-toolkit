...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.26.0 — "Mapped"** (M79 — promotes status mapping from per-adapter strict equality to a per-project YAML config with a tolerance wrapper that prompts the operator on non-key encounters instead of failing the gate. STE-302 ships a new loader module + `specs/tracker-config.yaml` schema with the four-role canonical enum `{initial, in_progress, in_review, done}` and a three-way `statusToRole` return (mapped / `null` known-non-key / `"unknown"` drift). STE-303 wires `/setup` Step 7f to write the config via MCP fetch + LLM-judgment proposal + unified-diff approve/edit/cancel gate; idempotent re-entry is the canonical resync path. STE-304 introduces `withTolerance(provider, specsDir)` at the Provider boundary — composes with `LocalProvider` / `TrackerProvider` identically, routes non-key encounters through a 3-option `AskUserQuestion` (force/skip/cancel) + `Other` fallback, refuses non-tty with `RequiresInputRefusedError` (no auto-apply — no safe default), and emits five capability rows. STE-305 relaxes `/gate-check` probes #8 + #14 to route through the wrapper while preserving STE-151 carve-outs verbatim and emitting ADVISORY (not GATE FAILED) under non-tty CI. New gate probes #52 `tracker_config_shape` + #53 `tracker_tolerance_wrapper_present`. Dogfooded: this repo's own `specs/tracker-config.yaml` ships with the release. Test count: 2006. Forward-only per `project_no_users_yet`.
