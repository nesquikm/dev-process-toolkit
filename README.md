...
- **Deterministic quality gates** — 77 numbered `/gate-check` probes (typecheck + lint + test + spec/plan/frontmatter/branch hygiene) override LLM judgment
...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.67.0 — "Refutable"** (M127, fixture falsifiability: seven findings from the 2026-08-16 conformance run, all of them assertion defects in the maintainer's own harness rather than regressions in any system under test — a shared falsifiability harness that executes each repaired assertion's own bytes against a capture with its subject present and again with it removed, refusing a rewording as evidence and scoring counts so a mutation that never applied cannot read as a pass; a hand-off count that read zero on every run because it anchored to lines a streaming capture does not have; a fixture that had drifted twice by taking one run's rendering of an LLM-composed verdict block as a contract; a healthy run failed for a sampler that missed its window; per-run evidence files that overwrote one another and leaked into the toolkit repo; assertions demanding outcomes the system is documented never to produce; and a wildcard that matched through encoded newlines, scoring one spurious pass and one false failure in the same run.)
