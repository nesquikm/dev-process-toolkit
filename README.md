...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.8.0 — "Sweep"** (Smoke findings sweep — 6 FRs closing 2026-05-04 Linear smoke #9/#10 + Jira Run A/B findings. STE-213 symmetrizes `/spec-write` Auto-mode/`-p` default-apply across the FR-draft + commit gates. STE-214 widens `/gate-check` probe #26 to recognize the full milestone-attach capability-key set. STE-215 adds a `/implement` Phase 4b' cross-cutting-spec propagation hook + sibling `/gate-check` probe `cross-cutting-spec-stale-file-refs`. STE-216 adds `/simplify` dirty-tree advisory closing the 4×-recurring silent-dirty-tree footgun. STE-217 adds `/gate-check` probe #28 negative-assertion recognizer (4 patterns) so verify lines that deliberately reference deleted paths stop false-positive-flagging. STE-218 cleans the `.claude/skills/smoke-test/SKILL.md` body — drops the runs table + 9 inline `F-DR<N>`/`run #<N>` references in favor of self-contained one-liner rationale. One new `/gate-check` probe (#37). Test count: 1590 (+74).
