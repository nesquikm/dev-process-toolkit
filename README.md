...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v1.19.0 — "FR Identity Stability"** (retires the scan-and-increment `FR-<N>` identifier in favor of collision-proof AC prefixes — the bound tracker ID in tracker mode (`AC-STE-50.1`), or the last 6 chars of the FR's ULID random portion in `mode: none` (`AC-VDTAF4.1`); new `acPrefix()` helper + `ShortUlidCollisionError` + `ac_lint` module wired into `importFromTracker` and `/gate-check`; one-off in-repo rewrite purges every legacy `FR-<N>` heading, `{#FR-N}` anchor, cross-reference, and `AC-<N>.<M>` prefix across specs + docs + skills + tests; the `findFRByFRCode` resolver branch (formerly FR-69) and the `kind: "fr-code"` route are deleted; STE-50..STE-52, 3 FRs; `mode: none` regression preserved).
