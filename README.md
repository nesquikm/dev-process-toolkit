...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.81.0 — "Assent"** (M_840a06, four outward writes to a tracker that has no delete tool stop firing on their own judgement. The setting is two repositories considering a shared tracker project — whether they should share one is an open question this release deliberately does not decide — where each of these is undoable rather than merely untidy. The reconciliation preamble imported every ticket with no local spec file, writing an FR file and then pushing a rewritten description back — justified by a guard the cited code does not contain, so on a shared board it would mint local files for a sibling repo's work and rewrite that repo's tickets. Orphans are never auto-imported now; consent routes through the same arbiter every other clarifying question uses, with no default the auto-approve marker can apply. The create-idempotency probe could not match at all: measured live, an exact-summary JQL join is ACCEPTED by Jira and answers zero rows, a silent always-miss rather than an error, so the probe never found the ticket it was built to find and the retry path ran it three times before creating a duplicate. Linear failed the other way, its query being a ranked relevance search over title or description, which mis-binds an FR onto an unrelated ticket instead of duplicating. All four sites — including the copy inside the capped skill file, the one that actually executes — now narrow by container and repo tag and let a client-side normalized compare decide, and a match wearing a sibling repo's tag stops the run rather than being reused or created beside. That stop became a graded outcome: the key is registered with its emit directive in the same change, because the registry is bidirectional and either half alone reds the gate. The milestone allocation guard gained the order a shared board needs — exactly one repo mints a container, every other derives its id from the key already there, by key and never by name — naming an asymmetry sitting verbatim in the tree, where the path that only reads refuses without an enumerator while the path that creates degrades to an unconditional create. That order is prose on the caller, not a guard in the mint helper, which is untouched here and left to a later milestone. And the flag every surface named for re-pointing a binding finally dispatches: the alternative those surfaces offered refuses a same-mode no-op and skips the range containing the tracker-config write, so the remedy for a failed status fetch named an instrument that skipped the step that failed. Seven numbered preconditions now say what must be true first, including the one an operator would otherwise walk into — the flag re-runs that write from the newly-bound container alone, so it can clobber a hand-written union and strand every archived FR still bound to the old container. Whether two containers' status names actually collide is empirical per repo pair, so the docs order a check and assert no answer)
