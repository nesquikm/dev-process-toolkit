...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.78.0 — "Pulse"** (M140, external link liveness: a spec could cite an external resource that had since died and nothing would notice — the reference scanner dropped any token carrying a URL scheme, so an external link was not merely unchecked, it was invisible. Links are now classified once, by the author, as required or informational, checked at the moment they are added, and recorded in the spec with the verdict and the time of the check. A new gate probe grades those records: a dead link something depends on fails, while a dead informational link, a stale record and an unrecorded courtesy link report without failing. The probe makes no requests of its own — it reads the verdict the author wrote, because a probe that fetched would fail differently depending on where it ran, and a machine with no egress would be indistinguishable from one facing dead links.)
