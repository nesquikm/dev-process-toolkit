...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.88.0 — "Honest Join"** (M_685ff6, creating a milestone in a tracker project two repositories share now runs one executed decision before anything is written: it reads the container listing, names its act in the approval question (create, or join by key or by title), never joins a closed Epic by title, and the mint that follows refuses to perform any other act. A joined Jira Epic keeps its labels, because the label write is computed as a read-merge and the tracker-write gate refuses a write that drops one. In tracker mode a new milestone is always tracker-keyed. Joining a milestone writes the span into both plans, so the sibling release gate can see it, and that gate now reads the sibling repository's whole git state and refuses unless it can prove the sibling idle, naming the state that holds the release on every surface that offers it. A feature request's milestone container is resolved before its ticket is created, so a stranded binding refuses instead of leaving an orphan ticket. Re-pointing a repository at another tracker project is one command that checks every precondition, rewrites only the project line, and names the branches still bound to the old project; probe #25 catches a flip that skipped it. What this release does not buy is protection for a client below the declared version floor: it still joins by title, and only the floor and the stop paragraph stand in its way)
