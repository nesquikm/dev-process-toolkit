...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.80.6 — "Purview"** (M_a8e09a, gates that mind their own subject: four surfaces that asserted something wider than the thing they owned, and in three of the four the over-claim is what kept a real defect invisible. A live-tree dogfood graded the whole plan/ship coherence report flat, but that probe reports three different conditions and only one of them was that suite's subject. One of the passengers — an archived plan that has not shipped yet — is legitimate transient state between the archival commit and the release commit, so the assert turned `bun test` red for the entire archive-then-ship window, which is precisely the window the ship ceremony's own pre-flight refuses to ship through: the gate could not be cleared without shipping and could not be shipped without clearing. The violation now carries a typed kind and only the dogfood narrows; detection does not move, and both surviving detectors are asserted here rather than assumed. `/pr` sent every ticket to the review lane without reading where it already was, which drags a finished ticket backwards on any project whose review lane sits behind its done lane, and the next gate run reads that as the author's drift — the transition is decided from the observed status now, and every skip is reported in words. The rule deciding pull-request titles named a release branch this repository has never had and credited a local hook with validating a subject authored on the host; both are corrected against what was measured, and no title convention is sanctioned or forbidden by the change. Two sibling suites gated their every-gate roll-ups on a typed `3` while the reader that derives the set shipped unconsulted beside them — measured, a real fourth blocking gate left both green — and they grade against the derivation now, with their hand-kept arrays deliberately untouched, because two entries are bound by index to specific skill files and the derived set is name-sorted. The reviews then turned on this milestone's own work and found it reproducing its own defect twice: a guard whose regex could not match the form it forbade, and a replacement clause asserting this repository's host settings as fact inside a skill that ships into everyone else's. Both were caught before the pull request, which is a first)
