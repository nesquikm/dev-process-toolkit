...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.22.0 — "Mirror"** (M72 — STE-285 hooks E2E smoke coverage. STE-286 adds `/smoke-test` Phase 2.X fixture group 8 — *STE-285 hooks runtime regression* — closing the symmetric coverage gap left by M71. Fixture group 8 mirrors fixture group 7's shape for the symmetric contract: install verification via `/setup --hooks=all` (re-asserting the canonical 3 PreToolUse:Bash + 1 UserPromptSubmit:* split) + 4 per-scenario NFR-10 refusal probes for the 4 seeded Process-category hooks. Empirical Phase 1 finding confirmed Claude Code's harness clobbers wrapper-shell `$CLAUDE_SESSION_FILE` on subprocess spawn — runtime probe uses the AC-STE-286.7-accepted fallback (per-scenario standalone `bash` subprocesses with `CLAUDE_SESSION_FILE` set inline). Bundles a small `--hooks=<value>` non-interactive preselect flag (`parsePreselectFlag` parser) so the smoke driver can install hooks without hitting the multi-select default-off menu. Test count: 2327.
