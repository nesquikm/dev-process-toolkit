...
| `/spec-write`     | Guide through writing spec files (requirements, technical, testing, plan)                                                                                                                                                                                          | `[--no-tech] [--fetch \| --no-fetch] [requirements \| technical \| testing \| plan \| all]` |
...
| `/implement`      | End-to-end feature implementation with TDD and bounded three-stage self-review (Stage A spec compliance → Stage B two-pass delegated review: Pass 1 spec compliance, Pass 2 code quality, fail-fast → Stage C hardening)                                           | `<milestone, task description, issue number, "next", or "all"> [--code-only]` |
| `/tdd`            | RED → GREEN → REFACTOR → AUDIT cycle                                                                                                                                                                                                                                         | `<FR-id>`                                                                  |
| `/gate-check`     | Deterministic quality gates (typecheck + lint + test)                                                                                                                                                                                                              | `[path to the checkout to grade] [--fix to auto-fix lint issues]`          |
...
| `/spec-archive`   | Manually archive a FR (by ULID or tracker ref) or milestone (`M<N>` group) via `git mv` into `specs/frs/archive/` / `specs/plan/archive/` with diff approval; runs a post-archive drift check (Pass A grep + Pass B semantic scan) on finish (FR-17, FR-21, FR-45) | `<ULID, tracker ID, tracker URL, or M<N>> [--plan-only] [--parked] \| --backfill-milestone-labels [--apply \| --dry-run]` |
...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.89.0 — "Provenance"** (M_85e846, the commit and pull-request guards now judge an action by the repository it actually lands in, and accept only evidence produced for that repository. A commit behind a wrapper, a full path, a nested shell, an if block, an alias, or a merge or cherry-pick that writes a commit is recognised as a commit, and every way of changing directory moves the commit with it: cd options, builtin cd, pushd and popd, home expansion, variables the command binds itself, and the directories it computes for itself. Pull-request creation is read through that same recogniser rather than a regex anchored at the start of the command, so a request opened after a cd, inside a subshell, behind a repository flag or chained after a push is seen at last, while `gh pr create --help`, which opens nothing, no longer is. Each gate skill leaves a receipt inside the checkout it ran for, and a receipt counts only while a non-error Skill call vouches for it, so a file written by hand is not evidence. What this release does not buy is a fresh gate: a receipt from a run earlier in the session still counts after the code has changed, and a session started before this release keeps the old hooks until it is restarted)
...
- [`plugins/dev-process-toolkit/docs/hooks-reference.md`](plugins/dev-process-toolkit/docs/hooks-reference.md) — The manual for the bundled hooks: what each one does, the blocking `PreToolUse` gates that refuse `git commit` and `gh pr create` until the Skill they name has run in the session, the receipt-demanding `pre-tracker-write-gate` that refuses shared-container tracker writes no deciding command approved, their exact refusal text, and the override path
