...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v1.30.0 — "Loudly"** (M29 closes the silent-feature-drop class that the v1.29.0 dogfood pair surfaced on a fresh Bun project. Eight FRs (STE-106..111, STE-113, STE-114) — `/setup` write-or-abort semantics with `mergeAllowList` helper + `templates/permissions.json` per-stack table, always-emit `## Docs` section, verification ≠ Q&A discipline with `appendAuditEntry` + toolkit marker, bootstrap commit phase, Bun zero-match-exit-1 placeholder + `examples/bun-typescript.md`, Schema L canonical-key closed set + dry-run migration script, `/spec-write` mode-conditional `id:` frontmatter + probe-13 severity warn → error, `/spec-archive` traceability-link rewrite at archive time. 7 new `/gate-check` probes (#17–#23) + 1 severity flip; every regression class is now caught at gate-time. Test count: 727 → 815.).
