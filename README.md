...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.6.0 — "Sensitivity"** (Smoke #10 follow-up — 3 FRs from the 2026-05-04 Linear + Jira smoke runs against v2.5.0. STE-194 calibrates `/gate-check` probe #26 (`tracker-project-milestone-attached`) to read the FR's `## Notes` body for the canonical `milestone_attach_unavailable` capability key — token + null binding routes the missing-binding outcome to ADVISORY instead of GATE FAILED, closing the smoke-driver false-positive surface where Linear-mode runs created tickets in projects with zero milestones by construction. STE-195 documents the stream-idle retry-with-rollback sub-flow for `/smoke-test` Phase 2 prompt-bearing children — one retry after `git clean -fdq -e .claude -e .mcp.json && git checkout -- .` in the test project's cwd, double-timeout aborts via NFR-10 canonical refusal. STE-196 closes the four-run-old `/simplify` SKILL.md no-op-gate-skip carve-out documentation gap with a dedicated `## When this is a no-op {#when-this-is-a-no-op}` section. Test count: 1452 (+5).
