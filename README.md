...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.10.0 — "Convergence"** (M57 — single-FR milestone shipping the `/conformance-loop` project-local skill. Replaces the manual two-terminal `/smoke-test` workflow with one parent-driven invocation that runs both Linear and Jira `/smoke-test` children in parallel via Bash subprocess + `wait`, aggregates the per-tracker findings into one cross-tracker-deduplicated report (exact-match on `STE-<N> runtime regression: <fixture>` + ≥80% normalized-body fuzzy overlap), and — only when `--auto-fix` is explicitly opted in — sequentially dispatches `/spec-write` + `/implement` per high-severity finding under `--max-iterations` + no-progress safety rails. Default `capture-only` mode preserves `/smoke-test`'s "Capture, don't fix" rule unchanged for raw invocations; `--auto-fix` is the formally-sanctioned exception (cross-referenced from `/smoke-test` SKILL.md). Three new `conformance_loop_terminated_{green,exhausted,no_progress}` capability keys for the closing summary. Project-local skill (`.claude/skills/conformance-loop/SKILL.md`) — downstream plugin users never see it; the override is scoped to the dogfooded toolkit repo only. Test count: 1631 (+34).
