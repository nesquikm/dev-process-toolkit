...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.80.2 — "Datum"** (M_8f8e25, pins that punish the right direction: a guard should not fail when the thing it guards gets better. The reachability probe rewarded giving a module a front door by turning the gate red, and its remedy — lower a pinned count — was stated in three places that had to agree byte-for-byte, so the sanctioned direction cost three coordinated edits and reddened a file the lowering never named. A fourth surface required the duplication outright, and the one test named for the pin compared two frozen literals to each other and could not fail on it. The count now has one home, a ledger of every move it has made with the commit and reason behind each, and a ceremony the gate executes rather than describes: a raise is refused, a reasonless move is refused. The two scanners M143 shipped are now numbered probes with front doors, so a clause dropped from a driven-path surface reds the gate rather than one test file — and registering them moved the pin not at all, which is the fix proving itself. Found on the way: nine live pins across eight files asserted a skill-length cap the NFR does not set, a cascade table renumbered a shipped probe through a dual-purpose constant, and two of three staleness halves were frozen at a number that could no longer fail)
