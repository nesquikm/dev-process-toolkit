...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.80.0 — "Onward"** (M143, chains that continue: an orchestrated run stopped at every phase boundary. Each stage closed with a report block mandated as the last thing in it, which returns control under a fork and ends the whole run inline — the same instruction, safe in one composition and fatal in another. Four skills also ended by offering to run the step the orchestrator had already named, and on the chain that closes a milestone the offer made the close ceremony run twice. Stages can now read whether an orchestrator is driving them, from a signal distinct from the headless marker, because a spawned worker is driven and fully interactive at once; a driven stage emits no terminal block and drops its claim on the next step, while a standalone run still asks every question it asked before. The prompts are registered so a standing authorization can reach them — and opening a pull request gained the irreversible guard it never had, without which registering the ceremony PR prompt would have made publishing relay-free.)
