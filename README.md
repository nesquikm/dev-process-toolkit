...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.5.0 — "Heredoc"** (Smoke #9 / Jira-run-2 follow-up — 7 FRs from the 2026-05-01 Linear + Jira smoke runs against v2.4.0. STE-185 retires `/tmp/dpt-smoke-prompt-*.txt` and lifts heredoc-on-stdin to first-class for prompt-bearing children, closing the cross-tracker content-swap window that triggered the Jira-run-2 corruption recovery. STE-186 widens Phase 0.5 cleanup glob to `/tmp/dpt-smoke-mcp-config-*.json` with audit-trail invariant. STE-187 auto-disambiguates same-day Linear project names via `-v<N>` suffix. STE-188 pipes `< /dev/null` on non-prompt Phase 2 child spawns; partitions cleanly with STE-185. STE-189 codifies `/setup`'s scaffold-deliverable inventory + restrictive-prompt rule. STE-190 adds explicit ADF-escape-tolerance tests + canonical parser module for Jira `pull_acs` description-body path (9 unit cases). STE-191 surfaces matching Space KEY in pre-flight #8 single-hit name-match refusals. Test count: 1447 (+9).
