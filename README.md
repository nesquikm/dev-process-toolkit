...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.25.0 — "Vendored"** (M78 — single feat-class FR adding a dependency-docs catalog so consumer projects (typically a mobile app with several private packages) can fold sibling-package docs into design context during `/brainstorm` and `/spec-write`. STE-301 ships a new git-tracked manifest at `specs/deps.yaml`, a `/deps` user-invocable management skill (add/edit/delete/list/sync with Socratic prompts, shell-injection-safe `git clone -- "<origin>" "<path>"`, Diátaxis layout validation, auto-detect-existing-checkout-with-matching-origin reuse, and a `ref drift: HEAD=<sha>` advisory in `sync` for pinned-ref divergence), a read-only `deps-researcher` Haiku subagent (Read/Grep/Glob only) invoked via a paired `/deps-research` forked skill on the `NON_COMMIT_PRODUCING_SKILLS` allowlist, a closed-schema `parseDepsResearchBlock` parser (banner + 3 canonical sections + optional `## Missing deps` + ≤ 25-line cap; pattern clone of STE-230's `parseSpecResearchBlock`), and injection seeds in `/brainstorm` Step 1.5b + `/spec-write` § 0b step 2.5b mirroring the STE-230 injection points. New `/gate-check` probe #51 (`deps_researcher_subagent_invariants`, severity: error) byte-checks subagent + child-skill frontmatter invariants; three new static capability keys (`deps_research_invoked` / `_no_matches` / `_shape_violation`) registered in `CANONICAL_CAPABILITY_KEYS` with MUST-emit literal-token directives. CLAUDE.md counts: 18 → 20 skills, 5 → 6 agents. Test count: 1850. Forward-only per `project_no_users_yet`.
