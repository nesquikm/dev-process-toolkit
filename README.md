...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.87.0 — "Owned"** (M_947c79, a repository that shares one Jira project or one Linear project with other repositories now declares it in one place: a `repo_tag` and a `min_dpt_version` in the tracker sub-section, read by code and refused by name when malformed. Setup writes that declaration and a plain-language stop paragraph that older clients, which cannot run the new guards, still read in every session. Creating a ticket, listing tracker orphans and importing or claiming a ticket are each decided by a command that reads the declaration and records its decision as a session receipt. The create check confirms the tracker answered the question it was asked. The orphan list offers only this repository's tickets and names who filed the unlabelled ones. A ticket from another project or repository is refused before anything is written. A new PreToolUse gate refuses shared-container tracker writes that no deciding command approved in the session, and writes from a toolkit older than the declared floor. Repositories that declare nothing see no change at all. What this release does not buy is protection against a writer that never declares: an undeclared repository is not refused, only reported through the untagged tickets it leaves behind)
