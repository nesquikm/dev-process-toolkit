...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.22.1 — "Portable"** (M73 — STE-288 hook install path portability fix. `additionFor` and `readInstalledHookNames` in `plugins/dev-process-toolkit/skills/setup/install_hooks.ts` now emit/match the literal `${CLAUDE_PLUGIN_ROOT}/templates/hooks/process/<name>.sh` token instead of an absolute install-time path, so the same `.claude/settings.json` works across dev clones and marketplace installs (STE-285 M71 design intent). Two-layer regression guard: unit tests pin byte-equality on the literal string + fixture group 8 install-verification greps the runtime-emitted JSON shape. Hoisted the shared literal into a module-level `HOOK_ARGS_PREFIX` constant to prevent write/read drift. Forward-only — zero shipped marketplace users, no migration logic. Test count: 2335.
