...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.53.0 — "Legible"** (M110 follow-up: the v2.52.0 re-run confirmed the "Gatekeeper" magpie fence works — `/spec-write` refused the first-turn tracker-create instead of writing — but Phase 8 couldn't render a *correct* refusal as a pass: under `claude -p` a refusal is prose, not a structured event, so it read as `vacuous`, indistinguishable from doing nothing. `RequiresInputRefusedError` now carries a canonical marker the stream parser maps to a recognizable refusal, so a surfaced refusal renders `ok-refused`; the tty probe is corrected to treat a heredoc stdin (`isTTY` undefined) as non-tty. Prior M110 releases: v2.52.0 "Gatekeeper" (first-turn tracker-create fence, probe #72) and v2.51.0 "Crucible" (milestone + gist-publish gates on the marker/refusal contract, liveness detector, socratic vacuous outcome + path-scoping, Linear AC-fence `href` widening; probes #70/#71).)
