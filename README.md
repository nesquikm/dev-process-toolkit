...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v1.37.0 — "Conventional"** (M36 ships a single user-facing FR: the cutover from the bespoke `M<N>: vX.Y.Z "Codename" — title (STE-<N>)` commit-message form used for M1–M35 to [Conventional Commits v1.0.0](https://www.conventionalcommits.org/en/v1.0.0/), enforced by a local `commit-msg` hook installed by `/setup`. Hybrid posture: pure POSIX-shell hook by default (zero-dep, works on every adopting project including Flutter / Python examples); opt-in commitlint variant via `/setup --commitlint` for projects with Node/Bun tooling. Skill prose updated across `/implement`, `/ship-milestone`, `/pr`; new `## Commit Convention` section in the CLAUDE.md template + this repo's root CLAUDE.md. Dogfooded — this repo's M36 implementation commit (`feat(commits): adopt Conventional Commits v1.0.0`) and the M36 ship commit (`chore(release): v1.37.0`) are the first two canonical CC commits on `main`, both validated by the same hook before they could land. M1–M35 commit history is intentionally pre-CC and not rewritten. Single-FR milestone, minor bump (new public surface). Test count: 1049 → 1071 (+22 fixture-driven shell-hook test cases plus an updated assertion in `ship-milestone-shape.test.ts`).).
