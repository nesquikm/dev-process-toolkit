...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.67.1 — "Verdict"** (M128, driver verdict integrity: the two driver-level findings from the 2026-08-16 conformance run, both in the maintainer's own harness rather than in any system under test — a design collision in which an earlier release made each leg's exit code carry its verdict while the collection gate, written before that, aborted the whole iteration on any non-zero code, so the loop could not emit its own report on exactly the runs that find something; the gate now reads the verdict artifact at that one code, because a declared-findings leg and a leg whose grandchild chain never completed reconcile to the same number and only the artifact separates them, while every other code, and a missing or unreadable artifact behind that one, still abort; and a roster whose documented per-group spelling one leg compacted onto a single line, returning zero matches for the documented form and so reading identically to a leg that reported nothing at all.)
