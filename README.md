...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.18.0 — "Hush"** (M67 — round 3 of the autonomous-mode contract hardening sequence. STE-251 closes the two surfaces `/conformance-loop` iter-1 caught after the v2.17.0 ship: F1 (Pattern 26 entry-side cross-tracker drift — `/spec-write` violates on Linear, `/setup` violates on Jira) and F2 (marker-absent silent-commit path under `claude -p` non-tty stdin — model rationalizes past dismissed `AskUserQuestion` responses and lands real commits via "safe defaults"). Source-level "FIRST ACTION (under non-interactive stdin)" lead-clause prepend on all four in-scope skill bodies forbidding `Write` / `Edit` / `NotebookEdit` before the first `AskUserQuestion` `tool_use` OR `RequiresInputRefusedError` raise; `requireOrRefuse(...)` gains a non-tty branch that throws `RequiresInputRefusedError` deterministically when stdin is non-tty AND the gate's required value is missing; new `/gate-check` probe `socratic_first_turn_post_hoc_drift` flags post-`/spec-write` commits whose body lacks both audit-row marker and refusal NFR-10 block. Detection-only scope; non-interactive only — interactive (tty) sessions are byte-identical to v2.17.0. F1 reproducer fixture relocated under `tests/fixtures/socratic-first-turn/regression/` to survive per-date overwrites; new replay test asserts forward-compat for `assertFirstTurnShape`'s violation contract. Test count: 652.
