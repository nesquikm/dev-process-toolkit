...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v1.33.0 — "Guardrails"** (M31 converts three discretionary checks — memory rules that only helped the current session — into hard gates that survive across LLM sessions and users. STE-117 ships `### Linear` / `### Jira` Schema L sub-sections + adapter-contract widening + `/setup` workspace-binding prompt + new `/gate-check` probe #25 `task-tracking-workspace-binding-present` (closes the silent-landing trap from M30). STE-118 ships an idempotent `attachProjectMilestone` helper that auto-binds tickets to their `M<N> — <title>` milestone object on Linear, invoked from `/spec-write` post-create and `/implement` Phase 1 entry, plus new probe #26 `tracker-project-milestone-attached` enforcing presence + name match (em-dash drift visible). STE-119 ships a mode-agnostic `nextFreeMilestoneNumber` helper running a three-way scan (active plans / archived plans / CHANGELOG) and a new probe #27 `frontmatter-milestone-not-archived` to catch post-edit collisions where an active FR's `milestone:` points at an archived plan file. Test count: 861 → 937 (+76).).
