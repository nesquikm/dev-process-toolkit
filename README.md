...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.23.0 — "Reconciled"** (M70 — state-emission convergence + post-iter-1 contract closures. STE-284 closes the 2026-05-13 partial-scan trap: new `reconcileTrackerLocal()` helper at `adapters/_shared/src/reconcile_tracker_local.ts` is the single SoT consumed by both `/spec-write` § 0.5 preamble (auto-imports tracker→local orphans, guarded by `existsSync` per STE-135) and `/gate-check` probe #49 (`tracker_local_reconciliation_drift`, severity warning → error on hard FR-id collisions). `Provider` interface evolves: new `mode` / `listMilestones` / `listActiveFRs` methods on `LocalProvider` and `TrackerProvider`; `nextFreeMilestoneNumber` extended with optional `provider` param and a `tracker` source. STE-119's tracker-exclusion decision (M31) explicitly superseded. STE-294 extends the STE-220 → STE-226 → STE-237 → STE-262 → STE-270 marker-trigger triple-layer defense: `FORBIDDEN_PHRASES` now flags `standing instruction` + `default-applied per standing` paraphrases of the autonomous-mode reminder; `/spec-write` SKILL.md carries the canonical NOT-a-trigger anchor sentence byte-repeated at 3 gate sites (§ 0b step 4, § 7a, ## Rules); a new regression fixture under `tests/fixtures/socratic-first-turn/regression/` plus a runtime test asserts `RequiresInputRefusedError` on the iter-1 § F1 capture. STE-295 bundles 5 iter-1 follow-ups: hook carve-out so spec-only commits skip the `/tdd` requirement; `stripLinearACFences` extended to the `mcp__linear__get_issue` read path via a new wrapper at `adapters/linear/src/get_issue.ts`; smoke-test SKILL.md master-merge step + severity-format canonical normalization (`**Severity:** <level>`); `/gate-check` probe #37 fence-only scope documented. Test count: 2445. Forward-only per `project_no_users_yet`.
