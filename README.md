...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.80.3 — "Anchor"** (M_f040b5, what conformance found: nine findings from a run whose two legs both finished 6/6 green. Every failure sat in the harness's own fixtures or in the loop driver — except one, and that one reaches consumers. On a Linear project using the default branch template, the acceptance-criteria push shipped in M114 is unreachable without a human: the template renders the milestone token, tracker-first minting makes that token opaque, and so the branch carries no ticket id for the resolver to find. A green gate, a ticket moved to Done, and four criteria that never toggled. The resolver now reads the milestone off the branch and the ticket off the single FR bound to it, refusing whenever more than one qualifies, and the template and the resolver are asserted to agree from both sides rather than one. The second finding is a class on its fourth appearance: a predecessor standardised how a probe row is drawn, and six fixture arms keyed to the old decoration stopped matching that day without a single red — the probes were healthy throughout, and only the assertions had died. The diagnosis came from a contrast rather than a failure, since the one sibling keyed on a probe NAME kept passing. The arms now name their probes, the delimiter they had borrowed from the runtime's prose comes from a token-bounded matcher instead, and a derived sweep grades arms while still permitting the paragraphs that quote what they retired. The third: the loop driver counted its legs by asking the shell to split an unquoted variable, which zsh does not do, so it aborted a completed two-leg run at its final gate after two hours forty. It failed closed and could never fake a green, which is exactly why it survived — and the suite stayed green because it tested those fences under the one shell where the bug is invisible. Giving the harness its second shell immediately found a second guard that fails OPEN there. Found on the way: two shipped checks that graded a spelling rather than a subject, this milestone's own class arriving inside its own repair)
