...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.84.1 — "Unbundled"** (M_862e04, one approval now authorises one act. A release skill's content approval asked once and took three actions on the answer, the last of which opened a pull request; it now asks only to apply and commit, the push and the pull request have gained an approval of their own that runs before either happens, and the rule stating the split names the push-and-open step and the merge step by their own acts instead of calling one by the other's name. Carried with it is the enabling fix the milestone surfaced: the sweep guarding cross-repo routing asserted that no plan on disk declares a target repo, forbidding the very capability it guards, so the first real declaring plan turned the suite red; it now partitions on the declaration, admits a declaring plan and routes it as declared, while the undeclared invariant it was built to protect survives unchanged. What this release does not prove is written down rather than implied by a green gate: the sibling repository carries no test runner, so its half is verified by reading and ships under its own pull request there, and this repository's plan keeps one task open until that pull request exists)
