...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.84.0 — "Leave No Trace"** (M_4df444, a smoke or conformance run now cleans up after itself. Every child session starts with an id its parent mints and records in a per-run ledger under .dpt/ before it spawns. When the loop ends, a leg whose verdict artifact reads pass has exactly those sessions removed, while a failed or aborted leg keeps everything for triage. One cleanup module finds a run's debris across ten stores by identity and a time window. It deletes nothing without --delete, removes only exact paths it re-validates at delete time, and names any survivor; its first real sweep took the 27 old smoke project dirs and left none. A repo-level hook refuses the stdin-fed spawn fence behind the 2026-09-11 runaway before it starts, and every backgrounded spawn block counts its live children and aborts on a mismatch. What this release does not prove is written down rather than implied by a green gate: the ledger and the hook are verified only at the next live run, and the skip baseline for its trunk commit was never measured)
