...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v1.28.0 — "Subtraction"** (M26 deletes shipping surface that exists to protect a user base that does not exist. Eight FRs across three phases: Phase A deletes Linear V1 SSE migration code/docs (STE-93), the `strip_ulid` migration tool (STE-94), and orphan MCP fixtures + dead frontmatter exports (STE-95); Phase B rewrites transition-state framing into final-state framing — drops v1→v2 lenient-on-migrated-plans clauses + STE-60→61 transition-window prose (STE-96), sweeps "pre-M12 / pre-M14 / pre-M18" framing across 26 occurrences in `docs/` and `skills/` (STE-97), reframes `/setup --migrate` as "mode switching" not "migration" (STE-98), and rewrites NFR-2 as a live forward-compat constraint instead of v1.7.0/v1.8.0 backward-compat (STE-99); Phase C prefixes `${CLAUDE_PLUGIN_ROOT}/` to two consumer-installable `bun run adapters/...` docs and extends `tests/skill-path-portability.test.ts` to scan `docs/**/*.md` (STE-100). Net ~600 LOC code deleted + ~700 lines doc prose deleted-or-rewritten. First M-milestone where total test count goes DOWN (704 → 702) — intentional dead-surface reduction.).
