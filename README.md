...
- **Deterministic quality gates** — 79 numbered `/gate-check` probes (typecheck + lint + test + spec/plan/frontmatter/branch hygiene) override LLM judgment
...
| `/deliver`        | Full delivery pipeline orchestrator — inline `/brainstorm` + `/spec-write`, then one fresh visible spawned worker per milestone running `/implement` → `/ship-milestone` → `/pr` serially, with relayed approval gates and `merge_policy`-routed PR closure       | `[feature request or idea, or a milestone or FR identity]`                 |
...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.68.0 — "Compact"** (M129, deliver pipeline contract repair and resume: the stage hand-off contract now reaches the worker through the kickoff text on the operative surface and is verified against a captured stage report rather than the orchestrator's own prose — a contract with no producer had passed every check that only ever read the consumer describing itself; approval gates gain three classes with a standing authorization that covers the mechanical one and can never reach merge, push, deploy or publish; merge policy can be tightened by saying so but never loosened, because no tightening transition targets `auto`; a milestone can declare the repo it targets and route to the topology that repo supports; `/pr` can open a draft and refuses rather than silently downgrading; and `/deliver` now recognises a milestone or FR identity, refusing one it cannot deliver instead of brainstorming work that already shipped, or resuming it at the phase its on-disk state allows.)
