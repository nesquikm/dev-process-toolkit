...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v1.43.0 — "Sentinel"** (M43 lands one /smoke-test run #5 fix. STE-154 aligns `adapters/jira.md` with the live Atlassian Rovo MCP after the first Tier 5 manual conformance pass — collapses seven F1–F7 discrepancies into one doc-and-config FR. Most-severe finding (F7): live MCP tool names are camelCase + `Jira` infix (`getJiraIssue`, `editJiraIssue`, `transitionJiraIssue`, …), not the spec's snake_case. F3 extends the existing `jira_ac_field:` Schema L key with the `description` sentinel — when team-managed Kanban templates ship no AC custom field, `/setup` prompts the operator to record `jira_ac_field: description` so ACs live in the issue description body's `## Acceptance Criteria` section. F4 swaps the hard-coded `issuetype="Story"` for `Task` default + `jira_issue_type:` override. F1 adds a `to.statusCategory.key` fallback so default Kanban workflows work zero-config. F5 documents Space pre-creation + `getVisibleJiraProjects` visibility probe. All "Status: provisional" blocks replaced with one-line live-MCP provenance. Smoke #5 trace at `specs/notes/jira-smoke-5.md` captures step 1 live-verified; steps 2–6 partial. No new canonical Schema L key. Test count: 1196 → 1215 (+19).
