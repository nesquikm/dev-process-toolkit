...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.9.0 — "Evidence"** (M55 verification — 3 FRs closing the systemic gap M55's v2.8.0 smoke runs surfaced. STE-220 reopens STE-213's runtime path: co-locates the Auto-mode/`-p` default-apply carve-out at every `/spec-write` gate site (§ 0b step 4, § 4, § 7a) with an explicit detection contract (`Auto Mode Active` system-reminder + `claude -p` invocation flag, byte-repeated paste-not-paraphrase) — STE-213 left this implicit and v2.8.0 `claude -p /spec-write` runs hung at the FR-draft gate despite a green checkmark. STE-221 + STE-222 add defensive `/smoke-test` Phase 2.X regression fixtures for STE-214 (probe #26 ## Notes scanner) + STE-215 (cross-cutting spec-drift propagation) — three sub-fixtures each (positive + control + deprecated-alias / probe-side), with canonical `STE-<sut> runtime regression: <fixture>` diagnostics naming the system-under-test. Phase 2.X is shared infrastructure for future SKILL.md-prose fixes. Closes the "documented but unenforced" failure mode for the M55 cohort. Test count: 1597 (+7).
