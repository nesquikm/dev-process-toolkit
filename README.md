...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.11.0 — "Deterministic"** (M58 + M59 — two milestones shipped together, both replacing prose-judgment contracts with byte-checkable deterministic mechanisms. M58 introduces a multi-agent `/tdd` orchestrator that runs each TDD phase in an isolated subagent context via Skills `context: fork` + custom-subagent pairing (test-writer batches the AC list once per FR, implementer runs per AC, refactorer once at end after all GREEN), with a strict ` ```tdd-result ` fenced YAML hand-off contract parsed deterministically and bounded retries through a five-mode failure classifier. M59 closes STE-220's third attempt by replacing the prose-only Auto-mode/`-p` detection with an explicit `<dpt:auto-approve>v1</dpt:auto-approve>` pre-authorization marker that gates default-apply at every relevant SKILL.md gate — byte-checkable at runtime, no LLM inference. Two new `/gate-check` probes (#38 `auto_approve_marker_in_canonical_spawns`, #39 `tdd_orchestrator_integrity`) lock the structural invariants of both shapes. Test count: 1741.
