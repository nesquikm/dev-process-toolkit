...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.80.5 — "Herald"** (M_a41431, gates that announce themselves: three PreToolUse hooks refuse a commit, a pull request, or a commit staging a test, unless a specific Skill ran as a `tool_use` earlier in the same session. The predicate is a substring pair on one JSONL line, so it cannot see a Bash call, a passing suite, or any output — and nothing told an agent so until it was refused, mid-flow, after the work was done. The manual for all three already existed and was correct; a recursive grep for its filename across the docs tree, the skills tree, the templates, the README and CLAUDE.md returned nothing, which made this a reachability bug wearing a missing-doc costume. Six surfaces now name the gates, the README index lists the manual, and the manual names the surfaces it is reachable from. The four workflow chains a bootstrapped project is handed name the review step before the pull request, and the second copy of those chains — the one `/setup` actually surfaces — is compared against the first by extraction rather than pinned, so changing one and forgetting the other reds the run. Against a milestone whose subject is a shipped gate nobody announced, announcing two of three was the likeliest way to fail, so the third FR derives the blocking set from the hook entry points themselves and grades every surface against what it found: a fourth gate in the same shape reds the run that adds it rather than the run that trips it. The audits then turned on this milestone's own work and found six wrong claims — four in the manual being made reachable, including a trigger it described backwards and a refusal text the hook does not emit, and two in the consumer template. A reachable document that is wrong is worse than one nobody can find, so all six now match the code)
