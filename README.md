...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.16.0 — "Sentinel"** (M65 — closes the autonomous-mode contract on the refusal side: every `requires-input:` step now routes through a shared `requireOrRefuse(spec, key, sentinel)` helper at `adapters/_shared/src/requires_input.ts` (four-outcome matrix: user-supplied / pre-baked / default-applied / refused; throws `RequiresInputRefusedError` in NFR-10 canonical shape on refusal). The `## /setup audit` row format gains `imputed: true|false`, derived from `appendAuditRow`'s `source:` parameter so model-imputed values are structurally distinguishable from user-confirmed ones; legacy rows parse tolerantly via `parseAuditRow`. New `/gate-check` probe #42 `requires_input_sentinel_coverage` globs every plugin + project SKILL.md and asserts each `requires-input:` annotated skill cites both the helper and the canonical doc `docs/auto-mode-protocol.md`. `/setup` step 7b refactored as the regression anchor; `/smoke-test` Phase 0 documents the three-branch resolution. Test count: 2027.
