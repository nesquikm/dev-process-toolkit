...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v1.36.1 — "Spine"** (M35 ships a single user-facing FR: a corrective patch on top of v1.36.0 "Compass" that adds two-tier visual hierarchy to the README workflow diagram via mermaid `classDef` styling — five spine skills (`/setup`, `/brainstorm`, `/spec-write`, `/implement`, `/ship-milestone`) get stadium shape + bold green styling; nine secondary skills (`/tdd`, `/gate-check`, `/debug`, `/visual-check`, `/simplify`, `/spec-review`, `/spec-archive`, `/docs`, `/pr`) stay plain rectangles in muted grey — and rewrites the prose paragraph beneath the diagram to factually match `skills/implement/SKILL.md` (M34's prose claimed `/implement` orchestrates `/tdd`, `/gate-check`, `/docs --quick`, and `/pr`; in reality only `/docs --quick` is invoked, TDD + gate commands run inline, and the user opens the PR via `/pr` after `/implement`'s commit lands). Single-FR milestone, patch bump on top of v1.36.0; no new feature surface. Test count: 1049 → 1049.).
