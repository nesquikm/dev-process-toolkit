...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.79.0 — "Bearings"** (M142, stack-aware TDD hook: the commit guard that requires a test-driven run recognised one project layout, so a Dart, Python, Go or Kotlin commit carrying a source and its test classified as carrying nothing to guard — the toolkit ships example configurations for six stacks and the guard covered one. The predicates now follow the marker-based stack detection the toolkit already had, so one table answers both which runner and which layout, and the reference names the directories the code actually matches. The second half is what let the first survive: an unrecognised layout resolved to the same outcome as a commit with genuinely nothing to guard, so a guard that never looked read exactly like a guard that passed. There is now a fourth outcome that names the checkout it could not identify and why, advisory rather than blocking — a guard that refused on its own ignorance would be switched off, which costs more than the miss.)
