...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.19.0 — "Allowlist"** (M68 — /conformance-loop permission hardening. STE-252 replaces the blanket `--permission-mode bypassPermissions` posture across the `/conformance-loop` call tree with a tracked, content-rich `permissions.allow` allow-list in `.claude/settings.json`. The toolkit's "policy artifact" goes from constraint-by-presence to constraint-by-content: operators review the policy as a single-file PR diff; children spawn in default permission mode and honor the allow-list at command-pattern granularity (Bash patterns, Edit/Write/Read/Grep/Glob, MCP tool families). New `/conformance-loop` Phase 0 pre-flight asserts `.permissions.allow` is non-empty (NFR-10 refusal on miss); new `/gate-check` probe #46 `conformance-loop-bypass-removed` flags any future regression. The release also lands six baseline cleanups (tracked `.mcp.json` with Linear entry, `package.json` + `ts-morph` devDep, requirements.md version line bump, marker-probe carve-out for Phase 8 socratic-loop fixtures, gate-check + setup SKILL.md cap trims, NFR-1 cap bumped 300 → 350) so `/ship-milestone`'s baseline-test gate is clean. Test count: 2146.
