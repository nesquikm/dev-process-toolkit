...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.22.2 — "Bundled"** (M74 — STE-289 hooks bundling redo. M71/M72/M73's install-side hook mechanism was structurally impossible: the Claude Code harness only expands `${CLAUDE_PLUGIN_ROOT}` in plugin-bundled `<plugin>/hooks/hooks.json`, never in user `.claude/settings.json`. M74 ships the plugin-bundled `plugins/dev-process-toolkit/hooks/hooks.json` (harness auto-discovers + fires across every project where the plugin is enabled) and rips out the install-side surface: `install_hooks.ts` + 5 setup tests + 1 stale capability-rows test + sections 0c/6a from setup/SKILL.md + fixture group 8 from smoke-test/SKILL.md. Per-script hook tests and the 4 hook scripts under `templates/hooks/process/` survive — only registration moves. New `plugins/dev-process-toolkit/tests/bundled-hooks-shape.test.ts` is the byte-shape regression guard (25 tests). Reverses STE-262/STE-270/STE-276's prior rejection of bundled hooks.json — rejection grounds remain mitigated. Forward-only — zero shipped marketplace users, no migration logic. Manual cleanup for any prior `/setup --hooks` run: delete the 4 dev-process-toolkit hook entries from `.claude/settings.json`. Test count: 2296.
