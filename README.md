...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.7.0 — "Roundtrip"** (Smoke #11 follow-up — 9 FRs closing 26 verified bugs from the 2026-05-04 Dart-lib + Flutter-app smoke runs. STE-197 trims `plan.md.template` to a single milestone + YAML frontmatter and routes `/setup` bootstrap milestones through a `kind: scaffolding` marker. STE-198 splits the milestone-attach capability key and adds an auto-create branch for Linear projects with zero milestones. STE-199 distinguishes three tracker-availability states (deferred binding, mid-session probe, `--code-only` fallback) and enforces that Auto Mode does NOT relax `requires-input` (`--tracker=<mode>` pre-bake). STE-200 adds plan-only archival + slim closure for scaffolding milestones. STE-201 makes `/ship-milestone` pre-flight walk the `**Tasks:**` block (not just FR rows) with `[deferred]` marker support. STE-202 polishes the Feature workflow row, milestone resolver kind, branch_template trunk-OK allowlist, and `docs_default_applied` capability. STE-209 fixes setup-time canonical-shape hygiene (explicit-subcommand permissions, `.mcp.json` `type: http`, per-stack doctor invocation). STE-210 hardens Phase 4 archival (archive-fallback in `/ship-milestone`, `git mv` → flip → add ordering, `archiveFRWithFlip` helper). STE-211 preserves Linear AC-token round-trip via backtick-wrap on emit + strip on import. Five new `/gate-check` probes (#32–#36). Test count: 1516 (+64).
