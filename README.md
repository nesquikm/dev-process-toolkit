...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.82.0 — "Kinship"** (M_8f07e0, a milestone may now deliberately span two repositories, and the repo that finishes first is told it is waiting rather than told to ship. A plan names its homes under a new spanning map; the reader has exactly three arms — undeclared, declared, malformed and refused — and resolves each entry through the repo probe and the FR binding that already existed, so no fact gains a second home. The ship-ready predicate reads that map only for a plan that would otherwise be ship-ready: while a sibling still holds active FRs bound to the milestone, the close offer goes quiet, probe 75 carries an awaiting-sibling note naming the sibling and its count, and the resume chain orders no stage at all; a sibling whose path cannot be found degrades to a note and leaves the verdict alone. A target repo written as a list used to read as no declaration at all and route to whichever repo ran the command; it now refuses, names all three spellings the parser flattens into the same empty value, and points at the spanning key. The joining side got its safety catches: both mints matched an existing milestone by exact title, so a trailing space was enough to mint a duplicate on a board with no delete button — they now share one normalizer, one match joins, two or more refuse by name, and a join option refuses rather than creates. The minted Jira Epic finally carries its milestone label, because the listing leg that counts Epics sees only one whose summary leads with a token, and the label leg is the only route to a title-only mint. What this release does not do is written down in the plan rather than implied by a green gate: no smoke leg drives two roots, and a dated, re-derivable note says what closing that would cost)
