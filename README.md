...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.80.7 — "Corpus"** (M_ESMJFJ, GB-20, three defects reported from a consumer project, the first of them latently destructive. `/docs --full` gathered "every active spec … (skip `archive/`)", and a project that has shipped its backlog has archived every FR it ever completed — so that set is empty exactly when the project is most mature. This repository is itself the case in point, carrying zero active FRs against 484 archived and zero active plans against 148, which makes an all-archived tree the normal steady state of a mature project rather than an edge case, and every such tree was exposed. Regeneration from that empty corpus did not error: it produced a plausible diff across the whole canonical tree, with the FR-derived narrative silently dropped or invented, presented behind an approval prompt too large to read — the gate was the surface the damage arrived through rather than a defence against it. Both available remedies now ship together, because either alone leaves a hole: the archives are read alongside the active specs, so an all-archived project regenerates from its real history, and a genuinely spec-less one is refused outright instead of handed a diff. The second defect was legibility — a run that deliberately changed nothing and a run whose docs leg failed both exited zero, along with a successful write, both decline paths and the zero-flag usage path, four outcomes behind one status. Every terminal path now states which one it reached on its own last line, and both consumers read that line instead of the exit code, which stopped `/implement` reporting a documentation fragment it had deliberately never written. The third was a publish that never ran: `/report-issue` mandated a `-s` secret flag that `gh` does not have and exits 1 on. It is dropped rather than replaced — secret is already the default — and the substitution a hurried reader would reach for is named and forbidden where it would be made, because `-p` is the only visibility flag left in the help output and would have published a curated bug report, settings files and optional session transcript included, as a publicly listed gist)
