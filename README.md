...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.4.0 — "Janitor"** (Smoke-findings cleanup from the 2026-05-01 Linear + Jira smoke runs against v2.3.0. STE-179 closes the `??` toolkit-files trap on the canonical chain by adding a `/spec-write` Step 7a commit and widening probe #22 (`setup-bootstrap-committed` → `toolkit-bootstrap-committed`) to cover the spec-files scope. STE-180 widens `/gate-check` probe #14's STE-151 carve-out to the fully-checked-single-FR state with an advisory pointing operators at `/spec-archive M<N>` or `/implement M<N>`. STE-181 adds a `## Invocation forms` section to `/implement` SKILL.md documenting the `<FR-id>` vs `M<N>` Phase-5 divergence + new doc-existence probe. STE-182 sharpens `/spec-write`'s `filename_policy_override` prose to a conditional shape (`no user override` vs `overrode user-proposed`). STE-183 drops the misleading `(v0.1.0)` suffix from `/setup`'s bootstrap commit subject and adds a `Toolkit:` body footer with the real plugin version. Test count: 1438 (+80).
