...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.86.0 — "Right Tree"** (M_f1f569, the two guards that block a commit now decide what to check by looking at the repository the commit will write to, not the directory the session happens to sit in. A commit-bearing command is recognised whether it is bare, prefixed with a directory change, routed through `-C`, wrapped in a subshell or chained — and where the target genuinely cannot be determined the guard says so and stands aside rather than answering confidently about a repository it cannot see. The same recognition closes the matching hole in the sibling gate, which gains no path classification it did not already have, and a heredoc body is read as the data it is rather than as a command, so writing a file that merely mentions a commit is no longer refused. Alongside it, the test-discipline requirement stops being unsatisfiable for a fix with no feature request behind it: a recorded red-before proof naming the staged test paths it covers is accepted beside the orchestrator's own evidence, found through the same reader, and the refusal an operator meets names both doors and the paths that raised it. Neither door present still refuses exactly as before. What this release does not buy is tamper-resistance — the second door is a claim a person records, the threat model is self-discipline rather than an adversary, and that is written into the feature rather than assumed away)
