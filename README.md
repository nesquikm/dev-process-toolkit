...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.83.0 — "Knock First"** (M_79b1f6, a milestone that spans two repositories can no longer be released by one of them while the other still holds work. The release refuses on the one sibling predicate — a sibling holding active FRs, never "the sibling has not shipped", which would deadlock two repos each waiting on the other — and --partial is the only way through, recorded on the plan as a deliberate half-release. Every door that offers the release now knocks first: the close prompt, the archival hint, the ship-debt offer and the pull-request pre-flight all run the same front door, and a resume over an archived plan waits for its sibling instead of ordering the ship. Each release commit names what its sibling actually shipped, read from the sibling's own plan. What this release does not do is written down rather than implied by a green gate: no smoke leg drives two roots, the archival hint's rendering is pinned only by presence checks until that fixture exists, and three single test failures seen before the last FR's commit were never named and stay on the record as an open question)
