...
- **Deterministic quality gates** — 83 numbered `/gate-check` probes (typecheck + lint + test + spec/plan/frontmatter/branch hygiene) override LLM judgment
...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.80.1 — "Notary"** (M_dc2ecb, release ceremony integrity: the ceremony that writes a release did not write every surface it declared. The README banner's codename and the requirements file's latest-release line had no writer at all, so both went stale on every release and a person patched them by hand. An optional entry whose pattern stopped matching aborted the whole release; a second run of the same release doubled the CHANGELOG and reported success both times; only the first occurrence of a version was bumped; and a project's replacement template was expanded as a substitution pattern instead of written out. The probe grading the requirements line swallowed the codename it was there to compare, two documents promised a bug class could not happen while naming the surface where it did, and the minor-bump rationale counted every FR as additive. Found while shipping this milestone, and blocking it: the guard grading the release surfaces knew numeric milestone ids only, so the first milestone minted from a tracker milestone parsed as no milestone and aborted its own ceremony.)
