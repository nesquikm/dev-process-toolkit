...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.22.3 — "Wired"** (M75 — STE-290 hook contract fix. STE-285's byte-checkable enforcement layer was inert since v2.21.0 — `_lib/session.sh` read `$CLAUDE_SESSION_FILE`, an env var the Claude Code harness never sets, so `require_skill_tool_use` always hit fail-open. The 2026-05-14 empirical probe (`/tmp/dpt-hook-probe` plugin × 3 launchers × 2 terminal hosts) confirmed the harness passes session info as stdin JSON carrying `transcript_path`. M75 rewrites `_lib/session.sh` as a Bun TS lib (`parseHookPayload` / `findSkillToolUse` / `requireSkillToolUse` / `emitNFR10`) plus 4 per-hook TS modules under `_lib/hooks/`. Bash shims at `process/*.sh` shrink to canonical 2-line `exec bun run "${CLAUDE_PLUGIN_ROOT}/.../<name>.ts"` wrappers; filenames + `.sh` preserved so STE-289's bundled-hooks-shape AC.6 stays GREEN. NFR-10 byte-stable substrings (`Refusing:` / `Reminder:` / `Context:`) preserved per STE-286 §104. Four integration tests rewritten to inject stdin via `Bun.spawn({ stdin, env: { CLAUDE_PLUGIN_ROOT } })`; 5 new dedicated unit test files for failure isolation. Behavioral change: hooks now actually enforce. Forward-only — zero shipped marketplace users; no migration logic. Test count: 2336.
