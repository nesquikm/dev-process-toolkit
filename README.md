...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.85.0 — "Legible"** (M_0c14d0, the design images an author pastes now reach the step that verifies the build. The verification phase takes the feature's own cited references — each with its repo-root-relative path and the caption its author wrote — and hands them to the project's resolved check skill through one shared renderer, printing the same block beneath the reminder for a project whose check does not run itself, so the path most projects are actually on is not the untested half. An image that no longer resolves is named and marked skipped rather than failing the step, because the gate already fails that condition earlier and failing it twice would block a commit on something already owned; a feature citing nothing is vacuous, invoking the check exactly as before, and one of two tokens records which case fired. The lineage is corrected in the same release: the storage half shipped two milestones earlier and deferred the consuming half, the next milestone claimed that deferral while shipping the verification hook instead, and all three records now say what each actually did. What this release does not do is look at the images — the toolkit hands the references over, and the app-driving visual comparison remains the project-authored check skill's own job)
