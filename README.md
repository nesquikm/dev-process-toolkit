...
See [`CHANGELOG.md`](./CHANGELOG.md) for the full release history. Latest: **v2.27.0 — "Forked"** (M80 — codifies the canonical audit-fix loop pattern, lands a /gate-check probe enforcing its frontmatter invariants, migrates /spec-review to the pattern as the first conformance example, and removes a Flutter/Dart placeholder cruft from /setup. STE-306 ships Pattern 29 in `docs/patterns.md` with stable anchor `{#pattern-audit-fix-loop}`. STE-307 adds /gate-check probe #54 `audit_fix_loop_pattern_invariants` (severity: error) with const allowlist `AUDIT_FIX_LOOP_CANONICAL_LOOPS` of audit-fork pairs. STE-308 introduces the read-only `spec-reviewer` subagent + `spec-review-audit` forked child skill + `parseSpecReviewResultBlock` parser; `/spec-review` rewritten to dispatch into the fork. STE-309 tightens `/setup`'s Flutter/Dart bullet to forbid the `test/empty_test.dart` placeholder + ships a regression test + cleanup callout for downstream packages. /simplify (M81+) and /implement Phase 3 Stage B (M82+) follow in later milestones. Test count: 2097. Forward-only per `project_no_users_yet`.)
