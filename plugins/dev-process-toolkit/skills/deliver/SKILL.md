...
- **This FR is the last active FR bound to its milestone** ⇒ the chain auto-extends to `/implement <FR-id>` → `/spec-archive M<N>` → `/ship-milestone M<N>` → `/pr` — unless the plan declares `spans_repos` and a sibling repo still holds active FRs, when it stops at `/pr`. `/spec-archive` is an explicit step and runs strictly before `/ship-milestone`, because a single-FR `/implement` run leaves the FR at `status: active` and archives nothing — shipping a milestone whose FRs are still active is exactly what that ordering prevents.
...
- **No `target_repo:` — or the `null` sentinel — means the invoking repo.** Every plan on disk says this by saying nothing, and it behaves exactly as it always has: both phases inline here, then the three-stage chain in one worker. The undeclared path never goes looking for another tree. A YAML list or any other value that is not one path — a block list, an empty map, a bare `target_repo:` with nothing after it — is refused rather than read as undeclared (a flow list is refused too, where its path fails to locate); one milestone routes to one tree, so a milestone living in two repos declares `spans_repos:` instead, which gates ship-readiness only — each repo's half is still delivered from that repo.
