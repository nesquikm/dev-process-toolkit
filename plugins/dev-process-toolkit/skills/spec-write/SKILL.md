...
| `requires_input_refused` | `step 7b refused — Auto Mode invocation lacked --tracker=<mode> pre-bake; remedy: re-invoke with the flag, or run interactively` |
...
| `branch_gate_*` (`created` / `edited` / `declined` / `default_applied` / `remote_probe_skipped`) | created: `gate created branch <branchName> — commit will land there`. edited: `gate accepted operator-supplied branch name <branchName>`. declined: `gate declined — skill exited non-zero with zero side effects (no commit, no staging mutation)`. default_applied: `gate auto-created branch <branchName> (auto-mode / -p) — verify before push`. remote_probe_skipped: `gate could not probe remote branches (offline / no remote / fetch timeout) — local-only collision check used; verify name is unique on remote before push` |
