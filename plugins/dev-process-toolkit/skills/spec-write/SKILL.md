...
**Draft + commit acceptance gates — Auto-mode / `-p` default-apply (STE-213, STE-220).** Two gates live in this skill: the FR-draft `Approve and proceed?` gate at § 0b step 4 (gates tracker create + FR file write), and the commit `Apply commit?` gate at § 7a (gates the spec commit). Both **default-apply `y` when the conversation includes `Auto Mode Active` in any `<system-reminder>` block, OR the invocation is `claude -p` non-interactive** — emitting `spec_write_draft_default_applied` and `spec_write_commit_default_applied` respectively (the two rows are co-located in the static map at § 7). Interactive `n` decline at either gate emits the corresponding `_declined` row. The detection contract (paste, not paraphrase) is byte-repeated at all three gate sites — § 0b step 4, here, and § 7a — so the LLM reading any single site has the full contract in view; STE-220 closes the read-order gap that left STE-213 unenforced at runtime.
