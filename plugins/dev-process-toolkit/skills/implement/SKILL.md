...
0. **Tracker-mode probes** — Before any other action:
   ...
   - **0.b Provider resolution** — Resolve `Provider` once per invocation: `LocalProvider` if `mode: none`, `TrackerProvider` wrapping the configured tracker adapter otherwise. No re-resolution mid-execution. ACs come from `specs/frs/<ulid>.md`; Phase 4 archives via `git mv`; `Provider.claimLock`/`releaseLock` gates entry/exit per STE-28.
   - **0.b′ Resolver entry** — Call `buildResolverConfig(claudeMdPath, adaptersDir)` from `adapters/_shared/src/resolver_config.ts` once at entry, then pass the result to `resolveFRArgument($ARGUMENTS, config)` from `adapters/_shared/src/resolve.ts`. Never hand-assemble the config inline; malformed adapter metadata surfaces as `MalformedAdapterMetadataError` → NFR-10 canonical refusal. Route by `kind`: `ulid` → proceed to 0.c with that ULID; `tracker-id` / `url` → branch on mode: tracker mode uses `findFRPathByTrackerRef(specsDir, trackerKey, trackerId)`, `mode: none` uses `findFRByTrackerRef(specsDir, trackerKey, trackerId)`; on hit, proceed to 0.c; on miss, run `importFromTracker(...)` then proceed. `fallthrough` → continue to step 2 for free-form argument handling. Branch-name interop: if the branch name contains a ticket ID that disagrees with the argument's resolved ticket, **the argument wins** with an NFR-10-shape warning. Full decision table: `docs/resolver-entry.md`.
   - **0.b″ Branch proposal** — Between 0.b′ and 0.c, if Schema L carries `branch_template:`, call `isCurrentBranchAcceptable(currentBranch, scope)` from `adapters/_shared/src/branch_proposal.ts`. Unacceptable ⇒ run a single LLM pass for `{type, slug}`, render via `buildBranchProposal`, prompt `[Y] accept / [e] edit / [n] abort`. `Y` → `git checkout -b`; `n` → clean exit, zero side effects. Absent `branch_template:` ⇒ skip entirely. Full decision logic in `docs/implement-reference.md` § Branch Proposal.
   - **0.c Claim** — Entry gate. Tracker mode: run the per-FR claim sequence in `docs/implement-tracker-mode.md` § Claim runbook. Four-way routing (`claimed` / `already-ours` / `taken-elsewhere` / `already-released`) per the runbook's decision steps (AC-STE-28.1/2). `mode: none`: `LocalProvider.claimLock` writes `.dpt-locks/<id>`.
   - **0.e Claim verification (Phase 1-exit self-check, tracker mode only)** — Before entering Phase 2, re-fetch the ticket via `mcp__<tracker>__get_issue(<id>)` and assert (1) `status == status_mapping[in_progress]` AND (2) `assignee == currentUser`. Mismatch ⇒ NFR-10 canonical refusal naming the ticket + observed status/assignee; hard-refuse to enter Phase 2. `mode: none` skips this step. Complements `/gate-check` probe #14 (active-side ticket-state drift, M24 STE-87) at gate time.
   - **0.f Project-milestone attach (STE-118 AC-STE-118.5, Linear-only, idempotent)** — In tracker mode with `project_milestone: true`, after 0.e succeeds, read the FR's `milestone:` frontmatter and call `planFileHeadingToMilestoneName(specs/plan/<milestone>.md)` from `adapters/_shared/src/attach_project_milestone.ts`. Then call `attachProjectMilestone(provider, project, canonicalName, ticketId)`. Idempotent on already-bound tickets. Vacuous on archived FRs, `mode: none`, and adapters with `project_milestone: false`. `MilestoneAttachmentError` surfaces per NFR-10.
   - **0.d Tracker-mode probe** — Run the Schema L probe (see `docs/patterns.md` § Tracker Mode Probe). If `CLAUDE.md` has no `## Task Tracking` section, mode is `none` and tracker hooks below skip. If a tracker mode is active:
     - **Ticket-binding pre-flight** — 3-tier resolver + confirmation prompt per `docs/ticket-binding.md`. Branch-regex mismatch fails loudly; decline exits cleanly.
     ...
     - **STE-17 diff/resolve** — Run the bidirectional AC sync loop before proceeding past Phase 1.
...
2. **Resolve the target** — Determine what to implement based on `$ARGUMENTS`:
   - `next` ⇒ first milestone in `specs/plan.md` with unchecked ACs (report `All milestones complete.` if none).
   ...
   - Single milestone name (`M1`, `M2`) ⇒ read that milestone from `specs/plan.md`.
...
Four labeled sub-steps in order: **Phase 4a** (gate-check passed — no new logic), **Phase 4b** (doc fragment, STE-74; writes `docs/.pending/<fr-id>.md` when docs generation is enabled), **Phase 4c** (report at step 14 + human approval at step 15), **Phase 4d** (Close procedure: commit → `Provider.releaseLock` → `Provider.getTicketStatus`).
...
**Procedure summary.** Run `git mv specs/frs/<name> specs/frs/archive/<name>` for each FR (stem preserved per NFR-15) + flip frontmatter `status: active → archived` + set `archived_at: <ISO now>`. **`archived_at` precision: full ISO-8601 with date + time + Z (e.g., `2026-04-30T17:23:11Z`); not date-only with zeroed time (`2026-04-30T00:00:00Z` is the regression shape).** Render the wall-clock instant of the archive commit via `date -u +%Y-%m-%dT%H:%M:%SZ`, never the shorter `date +%Y-%m-%d` form — the latter rounds to midnight UTC. Smoke #6 finding F1 caught the LLM-rendered midnight value; the prose is now pedantic so the prompt is unambiguous. **Plan-status flip:** `git mv specs/plan/<M#>.md specs/plan/archive/<M#>.md` and apply the same `status: active → archived` flip + `archived_at` to the plan frontmatter — same atomic commit, same full-ISO-8601 timestamp (date + time + Z). Before staging, call `rewriteArchiveLinks(repoRoot, frId)` per FR (`adapters/_shared/src/spec_archive/rewrite_links.ts`) to rewrite traceability references in `specs/requirements.md`, active plans, archive plans, and CHANGELOG's unreleased prefix. Call `cleanupPlanVerifyLines(projectRoot, deletedFiles, addedTestFiles)` (`adapters/_shared/src/spec_archive/cleanup_plan_verify_lines.ts`) to update `verify:` lines that reference deleted paths. All moves + flips + rewrites land in **one atomic commit** (AC-STE-22.2, AC-STE-22.6, AC-STE-125.1, AC-STE-126.1). On any helper throw, abort cleanly — do not commit, do not call `Provider.releaseLock`; surface NFR-10 canonical refusal naming the offending plan `file:line:column`. Then call `Provider.releaseLock(id)` for each released FR. **Advisory-note persistence (STE-148 AC-STE-148.2):** before staging the archive moves, for each FR, append a `## Implementation notes` body section to the FR markdown **after** `## Notes` — body is the FR's slice of `advisoryNote[]` rendered via the same shared formatter as Phase 4 step 14 (byte-identical bullet bodies, AC-STE-148.4); empty slice ⇒ heading plus the literal line `No advisory notes.` — never absent, so the archived FR carries the audit trail past the session window. Full procedure detail: `docs/implement-reference.md` § Phase 4 Milestone Archival.
...
14. **Report** — Present: AC checklist with pass/fail status; files created/modified; test coverage (which cases tested; flag modules without direct tests); self-review findings (caught + fixed; what remains); spec changes (edge cases added, deviations resolved, provisional decisions needing confirmation); drift findings (when `specs/` exists); gate check result citing actual output (e.g., `0 failures, 0 errors`); number of review rounds used. **Archival hygiene (AC-STE-125.4 / AC-STE-126.3)** — when § Milestone Archival ran, list `rewrote N traceability links in M plan files` AND `updated N plan task-list verify lines`. **Advisory notes section (STE-148 AC-STE-148.1 / AC-STE-148.3):** after the items above, append a `## Advisory notes` section rendering the `advisoryNote[]` list captured in Phase 3 Stage B as **one bullet per advisory entry**, in capture order, body shape `<concern> — <rationale>`. Zero entries ⇒ heading plus the literal line `No advisory notes.` — never absent, so the operator never confuses "no concerns" with "concerns hidden". Shared formatter with the archived-FR write below: bullet bodies are byte-identical (AC-STE-148.4).
...
15. **Wait for approval, then run the Close procedure** — Ask the user to review before committing. Do NOT commit until the user explicitly says so.
    ...
    **(b) Release** — for every claimed ticket, run `docs/implement-tracker-mode.md` § Release runbook. **No exit path skips this step.** On a full-milestone run where § Milestone Archival already released each archived FR, skip the per-ticket call here for those same FRs (AC-STE-47.6 double-call avoidance).
...
Silent skip cases: single-FR arg (e.g., `STE-42`, ULID, URL), `'all'` / `'remaining'` / no arg / empty invocation, any FR in `specs/plan/M<N>.md` still `status: active`, or any gate-check failed this run (partial success). Non-TTY stdin (CI / piped) ⇒ print the hint, do not prompt.
