...
- **Token Stats re-render.** Per in-scope FR, **after** the `git mv` / frontmatter flip and **before** the archive commit, re-render the FR's `## Token Stats` block at its new archive path from the token ledger (`.dev-process/token-ledger.jsonl`): select rows via `claimRowsForFR(projectRoot, { branch, sessionLineage, brainstormClaim: <tracker-id> })`, then call `upsertTokenStatsBlock(frBody, rows)` (both from `adapters/_shared/src/token_stats_render.ts`) so the block now carries the FR's `implement` + `tdd` rows, and `git add` the archive path so the refresh lands in the **same archive commit** — no separate commit, no post-commit dirty tree, never a standalone working-tree mutation. **Vacuous** when the ledger is absent or has no rows for the FR: skip the upsert, write nothing.
