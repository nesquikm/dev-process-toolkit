...
Silent skip cases: single-FR arg (a tracker ID, ULID, or URL), `'all'` / `'remaining'` / no arg / empty invocation, any FR in `specs/plan/M<N>.md` still `status: active`, or any gate-check failed this run (partial success). Non-TTY stdin (CI / piped) ⇒ print the hint, do not prompt.
