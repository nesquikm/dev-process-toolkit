...
3. **Prompt only when both hold** (archive moves present, no release marker). First run the sibling release gate on each affected milestone's archived plan: `bun run ${CLAUDE_PLUGIN_ROOT}/adapters/_shared/src/sibling_release.ts <projectRoot> <planFile> M<N> --offer`, `<planFile>` being `specs/plan/archive/M<N>.md` once archived, else `specs/plan/M<N>.md`. Then print the affected milestone(s) and prompt exactly:
...
The `pre-pr-spec-review` hook refuses a pull-request creation unless `/dev-process-toolkit:spec-review` has already run in this session as a `Skill` `tool_use`. It is not a prefix match on the command line: the hook reads the command through the same shell recogniser the commit gates use, so `gh pr create` and gh's own `pr new` alias are **recognised** in every shape a shell can carry them — behind a `cd`, inside a parenthesised subshell, after a `&&` chain or a newline, under a prefix assignment, `env`, `command`, a nested `bash -c`, or spliced out of a `$(…)` substitution — and `--dry-run` is recognised with them. Commands that merely read a request (`gh pr list`, `gh pr view`), that open one through a different door (`gh api …/pulls`, `hub pull-request`, GitLab's `git push -o merge_request.create`), or that only ask for help (`gh pr create --help`) open nothing and are out of scope. The hook grades the current session's transcript, so a review that ran in an earlier session does not count, and neither does auditing the specs by hand or summarising them in prose — the invocation itself is the token, not the reading. The refusal is `exit 2` at `PreToolUse`, which lands before the command, so nothing is created.

The evidence a blocking gate reads is the Skill call **plus** its receipt in the checkout the request **is opened from**: the transcript says only that the skill ran somewhere in this session, and the receipt says which checkout it ran against. Run `/dev-process-toolkit:spec-review` in that checkout, not in a sibling checkout of the same session — a review recorded against another repository is evidence about that one, and the commit gates on this checkout will say so. Running the gate is also the only thing that mints the receipt.

When the command names a repository outright — `-R` / `--repo`, else a `GH_REPO=` binding, else `GH_REPO` in the hook's own environment — that slug is matched against the remotes of the checkout the command runs in. A slug no remote matches is a **known-foreign** target and is refused even with the evidence in hand, because a review of this checkout cannot vouch for another repository; open the request from that repository's own checkout and review it there. A target the hook cannot place at all — an unexpanded word, an unbalanced subshell, a `git remote` listing that cannot be run — is not a refusal once the evidence holds: the request is opened and the hook exits 1 with a `Reminder:` naming what it could not resolve.
...
5. Create the PR using `gh pr create`:
   ...
   - **Title**: always derived from the dominant commit's [Conventional Commits v1.0.0](https://www.conventionalcommits.org/en/v1.0.0/) subject — `<type>(<scope>): <title>`, ≤ 72 characters — with no user-supplied override path. Use `!` for breaking changes (`feat(api)!: drop legacy endpoint`). When the branch carries multiple commits, the dominant commit is the branch's own `chore(release):` commit when the branch carries one, otherwise the primary feature commit. Where the host enables squash merging with the title source set to the commit-or-PR title — the setting takes the single commit's subject when the branch carries exactly one, and the PR title once it carries two or more — a squash on a multi-commit branch promotes this title to a commit subject on the trunk. The local commit-msg hook does not enforce that: it runs on commits authored in a checkout, and a merge authored on the host never passes through it. Check the host's own merge settings rather than assuming this one.
   - Body format:
