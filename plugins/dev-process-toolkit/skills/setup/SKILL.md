...
**Available-migration hint.** After the gitignore write, walk the shared migration registry (`adapters/_shared/src/migrations/`) and run each entry's detector against this tree; when any detector fires, append exactly one `## /setup audit` row via `appendAuditRow` recommending `/dev-process-toolkit:upgrade` and **MUST emit `upgrade_available`** (literal backticked token). The detection walk is **best-effort** on the same precedent step 6b follows — a detector that throws, or a registry that is absent or unreadable, is swallowed into its audit row and the run continues; the hint never blocks or fails `/setup`, and `/setup` never applies a migration itself (the runner owns every write, behind its own approval).
