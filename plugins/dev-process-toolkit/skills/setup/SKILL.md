...
> **FIRST ACTION (under non-interactive stdin) — STE-251 AC-STE-251.1.** When `process.stdin.isTTY === false` (e.g., `claude -p`), the first tool call this skill emits MUST be `AskUserQuestion` `tool_use` OR `RequiresInputRefusedError` raise (via `requireOrRefuse(...)` from `adapters/_shared/src/requires_input.ts`). `Write` / `Edit` / `NotebookEdit` are forbidden before that ask/refusal — they produce silent scaffolding that bypasses the Socratic Loop Contract. **Which of the two fires is not a judgement call: it is decided by the single-call arbiter `evaluateGateMarkerRefusal({promptBody, isTty, gateSite: "setup-socratic"})` from `adapters/_shared/src/gate_marker_refusal.ts`, whose SOLE byte-checkable evaluation path is the marker grep at `adapters/_shared/src/check_marker_runtime.ts` — outcome `apply` (literal marker present) ⇒ proceed non-interactively on the sanctioned answers named in the step contract below; outcome `prompt` (tty) ⇒ ask; every other case raises `RequiresInputRefusedError`, and that refusal MUST be surfaced verbatim so the `<dpt:requires-input-refused>v1</dpt:requires-input-refused>` marker it carries reaches the stream, because a prose-only refusal reads as `vacuous` (a non-pass) — byte-indistinguishable from a run that simply did nothing.** Read-only orientation (`Read` / `Grep` / `Glob` / `Bash`-read-only) is allowed. Interactive (tty) sessions are byte-identical to v2.17.0 — non-tty stdin only. See `docs/auto-mode-protocol.md § Socratic Loop Contract`.
...
- `requires-input: <reason>` — refuses to proceed without an answer. The canonical helper `requireOrRefuse(spec, key, sentinel)` from `adapters/_shared/src/requires_input.ts` consolidates the four-outcome decision (user-supplied / pre-baked / default-applied / refused) and throws `RequiresInputRefusedError` (NFR-10 shape) when no answer is supplied. The auto-approve marker `<dpt:auto-approve>v1</dpt:auto-approve>` does NOT relax `requires-input:` — it is informational for these gates. Full cross-skill contract: `docs/auto-mode-protocol.md`. **Sanctioned interview-answer source — the same mechanism `/spec-write` names, not a second shape.** Under non-tty the one legitimate answer to a `requires-input:` question is an operator-authored block that `resolveInterviewAnswer(promptBody, key)` from `adapters/_shared/src/auto_answers.ts` lifts out of the `<dpt:answers>v1` … `</dpt:answers>` region and hands to this same `requireOrRefuse(...)` call in its `preBakedValue` slot: a hit resolves as `pre-baked`, so the interview is ANSWERED rather than bypassed and never reaches the `default-applied` slot reserved for gates with a documented safe default; a miss returns `undefined` and leaves the refusal above to fire untouched. The auto-approve marker is a hard precondition (`extractAutoAnswers` byte-greps for it first), so a block sitting in an unmarked body — or one whose delimiters are absent or out of order — is inert and answers nothing, and the marker on its own answers nothing either.
...
Before any detection or setup, run the Schema L probe (see `docs/patterns.md` § Tracker Mode Probe). When `$ARGUMENTS` contains `--resume-tracker-binding`, route to the workspace-binding resume/repoint contract in `docs/setup-reference.md` § 0c — `bun run ${CLAUDE_PLUGIN_ROOT}/adapters/_shared/src/repoint_tracker_binding.ts`, then step 7f, then that command's `--verify` — whose preconditions follow it — regardless of `## Task Tracking` presence and regardless of the mode the probe reports; that flag re-points an existing binding at a new tracker container, and it is a different instrument from the mode-switch flags of § 0b, which change tracker MODE, refuse a same-mode no-op, and skip steps 1–8. If `CLAUDE.md` already exists and contains a `## Task Tracking` section, this is an existing tracker-mode project — § 0b is the entry point for changing MODE, and the repoint flag above is the entry point for changing CONTAINER. If `CLAUDE.md` is absent, empty of `## Task Tracking`, or `$ARGUMENTS` contains `new` — and `$ARGUMENTS` contains neither mode-switch flag named in the § 0b heading — run the normal fresh-setup flow below. When either § 0b mode-switch flag is present, section 0b overrides this routing regardless of `## Task Tracking` presence. When `$ARGUMENTS` contains `--template <path>`, dispatch to the `setup-template` child skill at `skills/setup-template/SKILL.md` (template-source reuse flow) before the normal fresh-setup steps.
...
4. If it exists: read + parse it, merge via `mergeAllowList(existing, canonical)`, write back. Preserves user additions; never strips entries. Malformed pre-existing JSON → abort with NFR-10 canonical shape. **Report the merge loudly on BOTH paths (3 and 4).** Call `mergeAllowListReport(existing, canonical)` from the same module instead of the bare merge — it returns the byte-identical `settings` (the merge is unchanged: a caller-supplied list is merged, never replaced) plus `callerCount` / `addedCount` / `mergedCount` and a pre-rendered `row` — and **MUST emit `setup_allowlist_entries_added`** (literal backticked token) in the Step 11 closing summary, naming how many entries the merge added beyond the caller-supplied list. The list a reviewer reads in the diff is the opening posture, not the effective policy, which is why silence here keeps getting re-filed as a defect. Emit on every path, including the two quiet ones — no caller list ⇒ the added count IS the full scaffolded count; zero added ⇒ a row saying so. A skipped row and a broken one read identically, so never skip.
...
**Available-migration hint.** After the gitignore write, walk the shared migration registry (`adapters/_shared/src/migrations/`) and run each entry's detector against this tree; when any detector fires, append exactly one `## /setup audit` row via `appendAuditRow` recommending `/dev-process-toolkit:gate-check` — whose probe #69 (`upgrade_staleness`) reports the drift and hands Claude the remedy, since the migration runner is model-invocable only and is not on the slash menu — and **MUST emit `upgrade_available`** (literal backticked token). The detection walk is **best-effort** on the same precedent step 6b follows — a detector that throws, or a registry that is absent or unreadable, is swallowed into its audit row and the run continues; the hint never blocks or fails `/setup`, and `/setup` never applies a migration itself (the runner owns every write, behind its own approval).
...
a. **Query the active adapter for the project's full status list.** Invoke the adapter's `list_project_statuses` capability via MCP; the returned list is the **verbatim** tracker vocabulary (casing, whitespace, special characters preserved). When the active adapter declares `list_project_statuses=false` (a custom adapter without the capability), the step short-circuits with outcome `skipped_adapter_limit` and **MUST emit `tracker_config_write_skipped_adapter_limit`** (literal backticked token) in the Step 11 closing summary. When the MCP call throws (server unreachable, timeout, auth failure), the step refuses with NFR-10 canonical shape (`Refusing:` / `Remedy:` / `Context:`) and **MUST emit `tracker_config_write_mcp_unavailable`** (literal backticked token); no partial write fires. **Jira note:** the `allowedValues` introspection behind this fetch is company-managed-only — for team-managed (`"simplified": true`) projects the adapter falls back to the transitions-derived path (`getTransitionsForJiraIssue` → `to.name`) as primary, per `adapters/jira.md` § `list_project_statuses`; both paths return the same ordered verbatim string array.
...
### 7g. Token Stats (opt-in)

`default: false` — `requires-input: false` (safe default). Ask one `AskUserQuestion` (per the Socratic Loop Contract — never bare prose): *"Enable per-skill token-usage stats? (records token usage per skill into a git-ignored ledger + renders `## Token Stats` tables at commit boundaries)"* `[y/n]`. The safe default `false` applies under non-tty stdin with no pre-baked answer — this is not a `requires-input` gate.

**Always emit** the `## Token Stats` section (`enabled: false` when declined or unanswered) as Schema L (lowercase `true`/`false`). Re-runs are atomic — splice the **full block**, write once (mirrors step 7d's `## Docs`). Over a project that already carries the section, show `[current: <bool>]`; empty input keeps the current value. Each resolved value appends a `## /setup audit` row `step:7g (token_stats.enabled) value:<bool> reason:"<user-supplied|default applied>"`.

### 8. Create specs (optional)

If the user wants the full SDD workflow (or `$ARGUMENTS` contains "new"):

- **Monolith guard — runs BEFORE the create/seed step below.** Run the read-only detector `monolithSplit.detect(projectRoot)` — a deterministic `bun` one-liner importing `monolithSplit` from `adapters/_shared/src/migrations/monolith_split` and printing the `{ applies, evidence }` verdict as JSON; it reads only and never mutates. When it APPLIES (live `### FR-<digits>:` sections in `specs/requirements.md` + no per-FR content), /setup SKIPS the `specs/frs/` mkdir + `.gitkeep` seeding entirely — including the `specs/frs/archive/` subdir; the `specs/plan/{,archive/}` scaffolding and the technical/testing template copies are UNAFFECTED (only the `specs/frs/` scaffold is withheld).
  - When the guard fires, /setup prints a **one-line explanation** naming the detected monolithic layout and the follow-up — run `/dev-process-toolkit:gate-check`, whose probe #69 (`upgrade_staleness`) surfaces the monolith-split migration on this legacy tree once bootstrap completes (mirrors the step-6c `upgrade_available` → gate-check hint) — and appends a `## /setup audit` row `step:8 (monolith_guard) value:frs-scaffold-skipped reason:"monolithic requirements.md layout — split via gate-check probe #69"`.
  - **Non-monolithic trees are byte-identical.** The guard keys on the live-section pattern `### FR-<digits>:` (both limbs of `monolithSplit.detect` require real `FR-<digits>` headings), so the requirements template's OWN placeholder `### FR-N: [Feature Name]` (`FR-N` — a literal letter, not digits) does NOT match; a fresh / non-monolithic scaffold never trips the guard and stays byte-identical to today.
...
- **Never-overwrite rule.** An existing `specs/requirements.md` is NEVER overwritten by the template copy — pre-existing spec files are the user's; the template copy fills only gaps (absent files), and never clobbers live content already on disk.
...
- **Bootstrap-milestone routing (STE-197 AC-STE-197.3).** When writing the bootstrap plan — `specs/plan/M1.md` in tracker mode, `specs/plan/M_<tail>.md` under `mode: none` (both named by `scaffoldPlanIdentity(specsDir, mode)` from `adapters/_shared/src/setup_scaffold_plan_identity.ts`, which also returns the minted `id:` to record in the plan frontmatter) — add `kind: scaffolding` to the plan frontmatter and write a single FR row marked `<scaffolding>` (e.g. `| <scaffolding> | Bootstrap (barrel + primary feature shipped pre-toolkit) | n/a |`). Do **not** invoke `Provider.sync` for bootstrap FRs. Downstream skills route the marker through plan-only closure (STE-200). Never emit a literal `<tracker-id>` row in a committed bootstrap plan.

**Scaffold deliverables (canonical inventory, STE-189).** /setup produces six artifact classes — each a **/setup deliverable, emitted unconditionally** when its step runs: (1) `CLAUDE.md` (step 5), (2) `.claude/settings.json` (step 6), (3) `.mcp.json` (step 7, tracker mode only), (4) `specs/{requirements,technical-spec,testing-spec}.md` (step 8), (5) the bootstrap plan — `specs/plan/M1.md` in tracker mode, `specs/plan/M_<tail>.md` under `mode: none` — plus `.gitkeep` stubs in `specs/frs/{,archive/}` and `specs/plan/{,archive/}` (step 8), (6) `src/.placeholder.test.ts` (Bun, step 2c). **Class-(5) carve-out:** the `.gitkeep` stubs are the *sole* exception to the emitted-unconditionally posture — the `specs/frs/{,archive/}` stubs are skipped when the monolith guard fires (a detected legacy monolithic `requirements.md` layout); the guard is narrow, so on every non-monolithic tree and for every OTHER artifact class the unconditional posture stays intact. These are emitted **regardless of any phrasing in the operator's invocation context** — "only /setup completes here", "do not run /spec-write", "minimal mode" all constrain which **downstream skills** run after /setup, not which artifacts /setup itself produces (smoke #9 / run 2 F2). `/spec-write` step 1 ("Assess current state") expects these on disk; absence is a recovery-path failure mode, not the canonical contract. The scaffold list is non-negotiable; the operator can populate spec bodies later via `/spec-write`, but the empty templates ship from /setup.
...
Procedure: pre-flight `git status --porcelain` check, stage the canonical set (CLAUDE.md, settings.json, .mcp.json when written, specs files, .gitkeep stubs, plus `commitlint.config.js` when `--commitlint` was used, plus `src/.placeholder.test.ts` when the Bun scaffold-verify branch wrote it), diff preview + prompt, commit (subject: `chore: bootstrap dev-process-toolkit` — no parenthesized version suffix; the historic `(v<plugin-version>)` form referenced the project's `bun init` default rather than the toolkit version and was misleading), handle declines/edits/failures.

**Bun placeholder rides the bootstrap commit.** The scaffolded `pre-commit-tdd-orchestrator` hook exempts the staged placeholder from the /tdd-evidence check via its verbatim `Bun zero-match workaround` marker comment (marker grep primary, basename guard secondary; staged deletions are also exempt per the first-real-test-lands lifecycle) — do not leave it staged for a manual operator commit. Mixed commits (placeholder + any tdd-required file) still require evidence; the exemption subtracts only the placeholder.
...
On accept, call `scaffoldCheckSkill({ projectRoot, stack, slug })` from `adapters/_shared/src/scaffold_check_skill.ts`. It writes a **stack-appropriate** stub — Flutter → dart/Flutter-MCP drive; web/JS/TS → Chrome-MCP or `/dev-process-toolkit:visual-check`; Python/API → HTTP e2e; other/unknown → a documented generic TODO — to `.claude/skills/<slug>-drive/SKILL.md` with `disable-model-invocation: true` (inert until you fill its TODOs), then writes `verify_skill: <slug>-drive` into the CLAUDE.md `## Verification` block. Collision-safe: an existing skill is never overwritten (de-duplicated to `<slug>-drive-2`). The same accept path also settles `run_cmd`: alongside the scaffold, offer a run command seeded from the stack detected in step 1 — a web project's dev server, a Flutter run command, a Python service's entry point — and adopt the operator's confirmed answer into the same `## Verification` block, adding no step of its own. Key the quiet branch on detection, never on which stack was matched: when `detectRunnability(projectRoot)` reports the project is not runnable — no source in its closed set fires — write `run_cmd: none`, which is an answer and not a skip. Neither path leaves `run_cmd` absent: an absent key parses to `null`, no answer at all rather than a declaration that the project cannot be run, and the moment the repo later grows a `dev` script or a `Running` heading that silence reds probe #80 where `none` would already have answered it.
...
- Steps 7b, 7c, 7d, 7e, 7g are five separate prompt sites — ask one question per turn, wait for the answer, then ask the next. Do not bundle them into a single turn even at phase transitions. See `plugins/dev-process-toolkit/docs/patterns.md § Pattern 26: Socratic Prompting {#pattern-socratic-prompting}` for the canonical rule + rationalization-prevention table.
- **Socratic Loop Contract (STE-237).** Every clarifying Q in this skill — Steps 1–6 stack-detection clarifiers AND Steps 7b–7e and 7g Schema-L resolution prompts — MUST be emitted as an `AskUserQuestion` tool call (closed-form options OR open-ended with the always-on `"Other"` free-form fallback), regardless of the autonomous-mode reminder, the auto-approve marker, or pre-baked `<command-args>` prose. Bare-prose Qs are forbidden. The first-turn contract additionally forbids `Write` / `Edit` / `NotebookEdit` tool calls before the first `AskUserQuestion` `tool_use` OR `RequiresInputRefusedError` raise; `Read` / `Grep` / `Glob` / `Bash`-read-only orientation is allowed. See `docs/auto-mode-protocol.md § Socratic Loop Contract` for the full contract. **Audit row `loop_entered:` column** — `appendAuditRow(...)` accepts `loopEntered: true` when Steps 1–6 emitted at least one `AskUserQuestion` clarifier, `false` otherwise; the column pairs with `imputed:` for two-axis loop visibility. **Scaffold-write guard — the concrete fence behind that forbidden-tools clause.** Every write-producing step is deferred until the first-turn gate has been decided: `src/.placeholder.test.ts` (§ 2c), `.claude/settings.json` (§ 6), `.dpt/.gitignore` (§ 6c) and `.mcp.json` (§ 7) MUST NOT be written before the first `AskUserQuestion` `tool_use` or `RequiresInputRefusedError` raise, and on the refusing branch none of the four fires at all — a run that refuses leaves the working tree byte-unchanged. A `verification:` annotation narrows to *within* a step: it makes that step's checks unconditional once the step is legitimately reached, and it never licenses reaching the step, or landing its write, ahead of the first ask/refusal.
