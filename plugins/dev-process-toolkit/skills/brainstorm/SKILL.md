...
Immediately after Step 1.5's spec-research seed and **before** Step 2 proposes approaches, check whether `specs/deps.yaml` exists and contains ≥ 1 entry. When both conditions hold, invoke `/dev-process-toolkit:deps-research <topic>` (forked) where `<topic>` is the clarified problem statement. The forked skill returns a small block of dependency hits (constrained shape, banner-fenced) drawn from the project's curated `specs/deps.yaml` manifest. Inject the returned block into this skill's context — Step 2's proposed approaches reference returned dependency candidates alongside the model's own analysis. After the fork returns, persist the raw returned block to `.dpt-locks/<ulid>/deps-research-result.txt` (the deterministic gate reads it — the `deps_research_result_shape` probe, not the LLM, validates its shape).
