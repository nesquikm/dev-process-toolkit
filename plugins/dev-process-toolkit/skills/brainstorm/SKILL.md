...
> **FIRST ACTION (under non-interactive stdin) — STE-251 AC-STE-251.1.** When `process.stdin.isTTY === false` (e.g., `claude -p`), the first tool call this skill emits MUST be `AskUserQuestion` `tool_use` OR `RequiresInputRefusedError` raise (via `requireOrRefuse(...)` from `adapters/_shared/src/requires_input.ts`). `Write` / `Edit` / `NotebookEdit` are forbidden before that ask/refusal — they produce silent scaffolding that bypasses the Socratic Loop Contract. Read-only orientation (`Read` / `Grep` / `Glob` / `Bash`-read-only) is allowed. Interactive (tty) sessions are byte-identical to v2.17.0 — non-tty stdin only. See `docs/auto-mode-protocol.md § Socratic Loop Contract`. **Under non-tty the refusal MUST be surfaced verbatim so the `<dpt:requires-input-refused>v1</dpt:requires-input-refused>` marker it carries reaches the stream, because a prose-only refusal reads as `vacuous` (a non-pass) — byte-indistinguishable from a run that simply did nothing. Two evasions are eliminated by name: asking the clarifying question in prose and then ending the turn is NOT an ask (the question MUST be an `AskUserQuestion` `tool_use`), and proceeding under stated assumptions is NOT an answer (state no assumption in place of the ask — refuse instead). Both are contract breaches, not shortcuts.**

**Refusal disposition (MUST-emit).** A refusal is a reportable outcome, not a silent exit. When the first-turn gate above refuses, this skill **MUST emit `brainstorm_socratic_refused`** as a literal, backticked token in its closing summary, beside the verbatim refusal marker. The token is registered in the canonical capability-key set that `/gate-check`'s closing-summary probe greps, and rendered in plain language by `/spec-write` § 7's static map; narrative paraphrase is insufficient. The token fires only on the refusal path — a run that emitted the `AskUserQuestion` `tool_use` never emits it.
...
Immediately after Step 1.5's spec-research seed and **before** Step 2 proposes approaches, check whether `specs/deps.yaml` exists and contains ≥ 1 entry. When both conditions hold, invoke `/dev-process-toolkit:deps-research <topic>` (forked) where `<topic>` is the clarified problem statement. The forked skill returns a small block of dependency hits (constrained shape, banner-fenced) drawn from the project's curated `specs/deps.yaml` manifest. Inject the returned block into this skill's context — Step 2's proposed approaches reference returned dependency candidates alongside the model's own analysis. After the fork returns, persist the raw returned block to `.dpt/scratch/<ulid>/deps-research-result.txt` (the deterministic gate reads it — the `deps_research_result_shape` probe, not the LLM, validates its shape).

**Vacuous path.** When `specs/deps.yaml` is absent or empty (zero entries), skip the invocation entirely — no fork is spawned, no tokens are spent, Step 2 proceeds as today. The manifest is the gate; without curated deps there is nothing to research. This is the legal `deps_research_skipped_no_manifest` disposition.

**Skipped under `--no-tech`.** Step 2 itself is skipped under the flag, so this seeding step is also skipped — there is no consumer for the dependency candidates and the call would only burn tokens. Vacuous under `--no-tech` regardless of manifest state. This is the legal `deps_research_skipped_no_tech` disposition.
...
**Complete disposition set (MUST-emit).** The deps-research step of any invocation resolves to EXACTLY ONE literal, backticked token. When the fork ran: ≥ 1 dependency match ⇒ **MUST emit `deps_research_invoked`**; zero matches across the manifest entries ⇒ **MUST emit `deps_research_no_matches`**; the fork fired but its output failed the shape check ⇒ **MUST emit `deps_research_shape_violation`**. When the fork was legally not invoked: `specs/deps.yaml` absent or empty ⇒ **MUST emit `deps_research_skipped_no_manifest`**; a `--no-tech` run ⇒ **MUST emit `deps_research_skipped_no_tech`**. When `specs/deps.yaml` carries ≥ 1 entry and the run is not `--no-tech`, the fork MUST be invoked — one of the first three tokens. These five are the whole legal set; there is NO legal token for "previously believed compromised," "injected," or "disabled."

**Anti-cascade.** A deps-research shape violation drops THIS seed and continues; it never disables the fork for future invocations. This skill holds no "fork compromised" state and carries no cross-invocation belief about fork health — each invocation re-invokes the fork per the manifest gate, independent of any prior run's outcome.
...
**Driven runs.** Omit this offer when the invocation body carries the driven-run marker: the step it offers was already fixed by the orchestrator that named it. Do not proceed into step 3 from here either — the orchestrator that named this stage invokes `/dev-process-toolkit:spec-write` itself as its own phase, so starting now would write the specs twice. What is dropped is this surface's claim on the spec-writing phase, never the phase: the chain still runs it, once, under its own gates. No gate is skipped; only the question goes.
