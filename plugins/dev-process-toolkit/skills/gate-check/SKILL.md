...
`tests/gate-check-tracker-project-milestone-attached.test.ts`, and
probe #27 (STE-119 frontmatter-milestone-not-archived) by
`tests/gate-check-frontmatter-milestone-not-archived.test.ts`.
Contributors adding probe 28+ must ship the matching test file in the
same commit.
