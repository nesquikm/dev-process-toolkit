...
// STE-47 AC-STE-47.5 — doc-conformance test asserts that skills/implement/SKILL.md
...
//
// These markers stop the drift observed during /implement STE-36 STE-37 where
...
describe("STE-47 AC-STE-47.1 / AC-STE-47.5 — SKILL.md Phase 4 step 15 releaseLock marker", () => {
  ...
  test("step 15 instruction is scoped to 'after commit lands' — not before", () => {
    ...
    const end = body.indexOf("## Rules", start);
    const step15 = body.slice(start, end);
    // The phrasing matters for AC-STE-47.3: release happens only after the commit
    // lands, so aborted runs do not release.
    expect(step15).toMatch(/after.*commit|commit.*succeed|commit.*land/i);
  });
});

describe("STE-47 AC-STE-47.3 — abort boundary (no releaseLock on abort)", () => {
  test("step 15 names the abort cases that skip releaseLock", () => {
    ...
    // is the core of AC-STE-47.3.
    expect(step15).toMatch(/not.*releaseLock|releaseLock.*only/i);
  });
});

describe("STE-47 AC-STE-47.6 — double-call avoidance when archival runs", () => {
...
describe("STE-47 AC-STE-47.2 — docs/implement-tracker-mode.md mirrors the instruction", () => {
