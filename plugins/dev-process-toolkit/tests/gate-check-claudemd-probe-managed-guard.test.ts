...
describe("AC-STE-434.6 — README documents 85 probes", () => {
  // Recalibrated 82 → 83: M140 added #83 external_link_verdicts on top
  // of M137's #82 stage_block_adoption.
  test("the Features bullet documents 85 numbered /gate-check probes (README.md:14)", () => {
    const readme = readFileSync(join(repoRoot, "README.md"), "utf-8");
    expect(onlyLine(readme, /numbered `\/gate-check` probes/)).toMatch(/\b85\b.*numbered/);
  });

  test("the /implement-invokes-/tdd aside documents 85 probes (README.md:109)", () => {
    const readme = readFileSync(join(repoRoot, "README.md"), "utf-8");
