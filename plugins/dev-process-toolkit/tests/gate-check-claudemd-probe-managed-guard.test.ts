...
describe("AC-STE-434.6 — README documents 81 probes", () => {
  // Recalibrated 80 → 81: M133 added #81 module_reachability on top of
  // M131's #80 runnability_declared.
  test("the Features bullet documents 81 numbered /gate-check probes (README.md:14)", () => {
    const readme = readFileSync(join(repoRoot, "README.md"), "utf-8");
    expect(onlyLine(readme, /numbered `\/gate-check` probes/)).toMatch(/\b81\b.*numbered/);
  });

  test("the /implement-invokes-/tdd aside documents 81 probes (README.md:109)", () => {
    const readme = readFileSync(join(repoRoot, "README.md"), "utf-8");
