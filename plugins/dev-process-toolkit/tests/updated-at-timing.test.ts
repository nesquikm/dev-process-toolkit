...
// STE-45 conformance — updatedAt recording must happen AFTER claimLock,
// not "at skill start" (the ambiguous original AC-STE-11.2 wording).
//
...
describe("STE-45 — updatedAt recorded after claimLock (not at skill start)", () => {
  test("AC-STE-45.1/2 — implement SKILL.md step 0.d labels updatedAt recording as post-claimLock", () => {
    ...
    expect(body).not.toContain("store the ticket's `updatedAt` in-session for `/gate-check` to compare later (AC-STE-11.2).");
  });

  test("AC-STE-45.7 — docs/implement-tracker-mode.md carries the one-line AFTER-claimLock call-out", () => {
    const body = read("docs/implement-tracker-mode.md");
    // The one-line callout from AC-STE-45.7 (exact wording).
    expect(body).toContain("Record `updatedAt` AFTER `claimLock` — the claim itself mutates the ticket");
  });

  test("AC-STE-45.5/STE-45.6 — docs/implement-tracker-mode.md covers the general rule + already-ours edge", () => {
    const body = read("docs/implement-tracker-mode.md");
    // AC-STE-45.5: the rule generalizes to any tracker-writing pre-flight step,
    ...
    // AC-STE-45.6: if claimLock returns `already-ours` and an older updatedAt
