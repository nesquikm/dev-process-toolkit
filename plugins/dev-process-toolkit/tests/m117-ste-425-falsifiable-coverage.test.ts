...
describe("AC-STE-425.2 — the outcome vocabulary", () => {
  ...
  test("every group's roster matches the audited table, row by row", () => {
    ...
    const EXPECTED: Readonly<Record<number, readonly string[]>> = {
      ...
      11: ["jira", "linear", "none"],
      // Group 12 (M124) is on the full alias too: the best-practices lens
      // disposition is decided by the manifest's presence on disk, never by a
      // tracker surface.
      12: ["jira", "linear", "none"],
      // Group 13 (M125) rides the full alias for the same structural reason as
      // group 11: the setup-template headless refusal reads stdin tty-ness
      // only, before any tracker surface could be consulted.
      13: ["jira", "linear", "none"],
      // Group 14 (STE-492) rides the full alias for a THIRD reason, distinct
      // from group 11's and 13's tty-ness one: its subject is a captured worker
      // stage report read off disk, and the `deliver-stage-result` fence shape
      // is the same artifact whatever tracker the leg configures.
      14: ["jira", "linear", "none"],
      // Group 15 (STE-533) rides the full alias for the same off-disk reason
      // group 14 does: its subject is a captured stage report, and the prose
      // lead-in cap, the one-fence rule and block-comes-last are properties of
      // the rendered text whatever tracker the leg configures.
      15: ["jira", "linear", "none"],
    };
  ...
  test("every group carries a non-empty, non-placeholder rationale", () => {
    const m = mod();
    expect(m.CANONICAL_FIXTURE_GROUPS).toHaveLength(15);
...
D("AC-STE-425.2 — the roster agrees with the SKILL it models", () => {
  ...
  test("every group heading enumerates exactly its own roster", () => {
    const m = mod();
    const slices = fixtureGroupSlices(SKILL!);
    expect(slices.size).toBe(15); // non-vacuity: the parse found every block
...
describe("AC-STE-425.2 — a group that did not execute is never rendered as a pass", () => {
  ...
  test("an observed failure survives reconciliation unchanged", () => {
    ...
    expect(records.filter((r) => r.outcome === "passed")).toHaveLength(12);
  });
  ...
  test("observing nothing yields one record per group and zero passes", () => {
    ...
    // TWELVE, and the gap is the point: on the `linear` leg groups 9 and 10 are
    ...
    // (STE-450, STE-451). It moved to 9 when group 11 (STE-464) rostered the
    // full alias again, to 10 when group 12 (M124) did the same, to 11 with
    // group 13 (M125), to 12 with group 14 (M129) and to 13 with group 15
    // (STE-533); the delta from the length above stays 2, the two n/a-by-roster
    // groups.
    expect(records.filter((r) => r.outcome === "not-reached")).toHaveLength(13);
  });
});
...
describe("AC-STE-425.2 — rendering", () => {
  ...
  test("the summary head counts every outcome separately", () => {
    ...
    expect(head).toMatch(/\b12 passed\b/);
...
describe("AC-STE-425.2 — the CLI carries the aggregate in its exit status", () => {
  test("a complete Jira roster (group 2 n/a) renders PASS and exits 0", () => {
    const r = runGroupsCli(["render", "--leg", "jira", "--passed", "1 3 4 5 6 7 8 11 12 13 14 15"]);
  ...
  test("a group number outside the roster is named, not silently dropped", () => {
    const r = runGroupsCli([
      ...
      "jira",
      "--passed",
      "1 3 4 5 6 7 8 11 12 13 14 15 99",
    ]);
...
describe("Phase 3 round-2 hardening — closing check covers every persist class", () => {
  ...
  test("the closing check names a pathspec for every fixture class the run writes", () => {
    ...
    // `deliver-stage-capture/` (M129/STE-492) is the same class: three
    // committed, hand-authored capture models that pin `verifyDeliverStageCapture`
    // in unit tests. Fixture group 14's RUNTIME subject is
    // `/tmp/dpt-smoke-<tracker>-ste492-stage-report.txt`, not this directory —
    // the run reads the models never, and writes them never — so there is
    // nothing here for the closing check to clean up or stage.
    documented.delete("deliver-stage-capture");
    // `stage-block-adoption/` (M137/STE-533) is that same class again: two
    // committed, hand-authored capture models pinning `verifyStageReportAdoption`
    // in unit tests. Fixture group 15's RUNTIME subject is
    // `/tmp/dpt-smoke-<tracker>-ste533-stage-report.txt`, never this directory.
    documented.delete("stage-block-adoption");
