...
describe("AC-STE-394.7 — README probe-count pins move 68 → 69", () => {
  ...
  test("the Features bullet counts 71 numbered probes", () => {
    expect(readme()).toMatch(/\b71\b\s+numbered `\/gate-check` probes/);
  });

  test("the /implement-invokes-/tdd aside counts 71 probes", () => {
    expect(readme()).toMatch(/layers 71 probes/);
  });
...
describe("AC-STE-394.7 — gate-check SKILL.md gains the #69 entry", () => {
  ...
  test("the highest numbered probe is now 71", () => {
    const numbers = [...skill().matchAll(/^(\d+)\. \*\*/gm)].map((m) => Number(m[1]));
    expect(numbers.length).toBeGreaterThan(0);
    expect(Math.max(...numbers)).toBe(71);
  });
...
describe("AC-STE-394.7 — the three pinned test files carry the 71-form pin", () => {
  const testFile = (name: string): string => read(join(PLUGIN_ROOT, "tests", name));

  test("tests/gate-check-spec-write-next-line-doc.test.ts pins 71", () => {
    const body = testFile("gate-check-spec-write-next-line-doc.test.ts");
    expect(body).toContain('"71 numbered"');
    expect(body).toContain("layers 71 probes");
    expect(body).toContain("toBe(71)");
  });

  test("tests/gate-check-public-surface-count-drift.test.ts pins 71", () => {
    const body = testFile("gate-check-public-surface-count-drift.test.ts");
    expect(body).toContain("\\b71\\b.*numbered");
    expect(body).toContain("\\b71\\b\\s+probes");
  });

  test("tests/m108-ste-393-docs-pins.test.ts pins 71", () => {
    const body = testFile("m108-ste-393-docs-pins.test.ts");
    expect(body).toContain("\\b71\\b\\s+numbered");
    expect(body).toContain("layers 71 probes");
  });
});
