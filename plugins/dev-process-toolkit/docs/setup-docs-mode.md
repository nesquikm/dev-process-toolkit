...
`SKILL.md` under the NFR-1 358-line budget.
...
- **Both-false is a hard refusal.** If the user declines both
  ...
  first pass is preserved across the re-ask.
- **Atomic re-run writes.** Re-running `/setup` on a project that already
  ...
  full block in a single file write — no per-line edits.
...
Missing file or missing section ≡ `{ userFacingMode: false, packagesMode: false, changelogCiOwned: false }`. A key whose value is anything other than literal `true` / `false` throws `MalformedDocsConfigError`, which the calling skill re-renders as NFR-10 canonical shape.
