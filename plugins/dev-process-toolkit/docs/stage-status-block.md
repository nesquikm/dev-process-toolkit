...
4. **The block is the LAST thing in the report, other than the cap-exempt sections below.** Nothing non-blank follows the closing marker except a section named in `CAP_EXEMPT_SECTIONS` for that stage, **and no more of it than that section's own renderer emits** — its budget, below. Three shapes are admitted inside a section: its heading, its list rows, and the bare `name:` keys the shipped renderers emit (`gate:` / `drive:` / `e2e:`, three of them from the evidence renderer alone) — plus the mandated literal sentence an empty section carries, which the entry's renderer states. Prose under a correctly-headed section is still narration. **And the rule is about a REPORT.** A stage running as an inline step of an orchestrated run — its invocation body carrying the driven-run marker, read by `isDrivenRun` in `adapters/_shared/src/driven_run_signal.ts` — produces no report of its own: it emits no block, continues to the next step in the same turn, and the orchestrator's own report carries what the operator needs. "Last thing in the report" holds vacuously where there is no report to be last in, which is why suppressing the block inline is not an exception to this rule but an instance of its subject being absent. Under a fork the distinction never arose, because ending the turn returns control; inline, the identical instruction ends the run. The authoring contract is unchanged on both paths: the fence stays documented in every adopting `SKILL.md` and is still required there by `scanStageBlockAdoption`, since suppression is a property of a driven invocation rather than a removal of the contract.
