...
| `tracker-id` or `url`, find-by-tracker-ref hit | Open that existing FR for editing. **No network call.** Single-pattern direct-filename lookup at `specs/frs/<tracker-id>.md` (+ `archive/` when `includeArchive`). Filename ↔ frontmatter disagreement returns null. **Mode-aware (STE-135):** tracker mode uses `findFRPathByTrackerRef` (path-returning; STE-76 removed the `id:` line so `findFRByTrackerRef` cannot match). `mode: none` uses `findFRByTrackerRef` (ULID-returning). | Proceed to `Provider.claimLock(<id>, branch)` on the resolved ID — tracker ID in tracker mode, ULID in `mode: none`. | Archive via `git mv` + frontmatter flip on the resolved FR (O(1) direct-filename lookup). |
