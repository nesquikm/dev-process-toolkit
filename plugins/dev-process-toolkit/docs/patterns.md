...
**Branch automation.** `branch_template:` is an additive Schema L key consumed **only** by `/implement` Phase 1 (via `buildBranchProposal` in `adapters/_shared/src/branch_proposal.ts`). Default seeded by `/setup` step 7c: `{type}/m{N}-{slug}` — the single canonical default in every mode; milestone-less FRs fall back to the ticket-keyed form at derivation time. Absent key ⇒ branch automation disabled — legacy projects continue to run on whatever branch they're invoked from. Placeholders: `{type}` (deterministic `feat`/`fix`/`chore` — derived via `branchTypeFor` from the FR's `changelog_category`; unknown values clamp to `feat`), `{N}` (the BRANCH token derived from the `milestone:` frontmatter: bare digits for a numeric milestone, or the full `M_<epic-key>` token for an Epic-keyed one — `M_GF_78` ⇒ `m_gf_78`, never `m78`), `{ticket-id}` (tracker ID or lowercased short-ULID tail), `{slug}` (LLM-inferred 2–4 word kebab). Sanitization clamps LLM output to `[a-z0-9-]` before `git checkout -b` (defense in depth). No other mode-aware skill reads `branch_template:`.
...
## Pattern 30: Project-Authored Verification Skills {#pattern-verification-skills}

**Problem**: A green gate is not a working app. The compiler is satisfied, the linter is quiet, every unit test passes — and the feature is still broken end-to-end, because nothing exercised the real runtime surface (the app boots, the HTTP endpoint answers, the screen actually renders). Gate checks assert *the code is well-formed*; they do not assert *the built thing works*.

**Solution**: Each consuming project authors its own **check skill** — a project-local Claude Code skill under `.claude/skills/<name>/SKILL.md` that drives the built artifact and reports pass/fail — and wires it to `/implement` through a `## Verification` block in CLAUDE.md (the `verify_skill` config key). `/implement`'s **Phase 4b″ — Project Verification** resolves that skill (declared → discover → scaffold-offer), runs it after the gate is GREEN but before the commit-approval gate, and reports the outcome. Because the check is *project-authored*, it can speak the project's own runtime language — boot a Flutter app over the dart MCP, curl an HTTP progress endpoint, smoke a CLI — rather than a one-size-fits-all harness. The check skill carries `disable-model-invocation: true` so it never fires opportunistically; it runs only when the verification flow invokes it. Author your own via `docs/verification-skills.md` (the authoring guide) or the `/setup` scaffold generator.

**Relation to the other verification patterns**:

- **Pattern 12 (Verification-Before-Completion)** is the *principle* — forbid "done" claims without fresh command output. Pattern 30 is the *mechanism* that lets a project satisfy Pattern 12 for the runtime layer the gate can't reach: the check skill produces the fresh, cited evidence that the built app actually works, not just that its tests pass.
- **Pattern 10 (Visual Verification via MCP)** is the *web-UI special case* of this idea — an MCP-driven visual inspection. Pattern 30 generalizes it: a visual check is one *kind* of project-authored verification skill, and for a small web project `/implement`'s Phase 4b″ can adopt the generic `/dev-process-toolkit:visual-check` (Pattern 10's shipped form) as the check outright, while a Flutter or API project authors a runtime-appropriate check instead.

**Cross-refs**: `docs/verification-skills.md` (authoring guide), `plugins/dev-process-toolkit/skills/implement/SKILL.md` § Phase 4b″ (the resolve-and-run flow), Pattern 10 (Visual Verification via MCP), Pattern 12 (Verification-Before-Completion).

## Pattern 31: Self-Documenting-Codebase Pin Collision {#pattern-pin-collision}

**Problem**: A meta-test pins a hazard by banning an identifier across a whole file — `expect(doc).not.toContain("X")`, `grep -c X == 0`. The codebase then documents its own mechanisms in the very files those meta-tests scan, so the paragraph *explaining* why `X` must not appear contains `X` and turns the pin RED. The failure looks like a regression and is not: the prose is correct, the pin is correct, and only their scope is wrong. The tempting repair — weaken or delete the pin — trades a real guard for a documentation convenience, and the pin is usually protecting something that was measured.

**Observed ten times in M121 alone**, in both directions, across four FRs: a header comment *satisfying* a source pin (M120); `SMOKE_LEGS.join` banned and then quoted by the module documenting why it never builds the pattern that way; a blanket `not.toContain("SMOKE_LEGS")` firing on two comments saying the module never imports it; a `× 2 trackers` multiplier ban firing on the paragraph correcting that multiplier; a comment naming the identifier a predecessor's AC greps to zero; a verification script asserting `'set -f' not in stripped` and firing on the comment explaining `set -f`; a note explaining why a per-leg literal may not appear writing that literal; a ban on a superseded `rm -rf` expansion firing on the paragraph documenting it as superseded; a ban on the bare word `JQL` firing on the sentence saying no JQL runs. It is no longer a recurrence worth noting per instance — it is a property of the codebase.

**Solution**: **Pin the operative shape, never the identifier.** Ask what a real regression would look like as *code* — an import statement, a call expression, a shell directive on a line of its own, an instruction line — and match that, not every mention of a name. Where the hazard genuinely is textual, scope it: to a section, to the assertion's own region, or to lines that do **not** carry a superseded/historical label. When a pin blocks a correct prose fix, the house remedy has held every single time: **reword the prose or narrow the assertion; never weaken the pin** — least of all a shipped predecessor's acceptance criterion, because relaxing one of those to flatter a successor's phrasing is how spec history stops being trustworthy.

Two riders, both learned the hard way. Prefer replacing a narrowed ban with something **strictly stronger** rather than merely narrower — a ban says the bad form is gone, which "does nothing at all" also satisfies; assert the good form is present and count it. And check the narrowed pin still bites: narrowing is the same edit as blinding it, and only running the mutation tells the two apart.
