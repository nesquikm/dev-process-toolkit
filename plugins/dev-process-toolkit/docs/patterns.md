...
**Full ULIDs are internal-only (STE-67).** In tracker mode, user-facing prose (Linear/Jira ticket descriptions, plan-file FR lists, the current CHANGELOG release section, README) references FRs by their **tracker ID** — never by the full 26-char `fr_<ULID>`. The ULID lives only in frontmatter `id:` and in code-internal references (`Provider.getMetadata`, `findFRByTrackerRef`, `getFrPath`, resolver). Archived content keeps whatever form it had at archival time — the `/gate-check` probe scopes to active content only. `mode: none` is symmetric: the short-ULID tail (6 chars, lowercased for branches) is the human-facing form, never the full 26-char ULID. Rationale: M18 moved the ULID out of filenames; STE-67 finishes the job by moving it out of user-facing prose.
