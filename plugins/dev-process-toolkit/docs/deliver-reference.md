...
| `true` — this FR is the last active FR bound to its milestone | `/implement <FR-id>` → `/spec-archive M<N>` → `/ship-milestone M<N>` → `/pr` | `/spec-archive` is an explicit step and runs strictly before `/ship-milestone`: a single-FR `/implement` run leaves the FR at `status: active` and archives nothing, and shipping a milestone whose FRs are still active is exactly what that ordering prevents. A plan declaring `spans_repos` whose sibling repo still holds active FRs stops at `/pr` instead: nothing ships while it waits. |
...
- **No `target_repo:` key — or the `null` sentinel — means the invoking session's repo.** This is what every existing plan says by saying nothing, and it behaves exactly as it always has: design and spec-writing inline in the invoking session, then the shipped three-stage chain in one worker. `/deliver` does not go looking for another tree for an undeclared milestone; there is nothing to look for. A `target_repo:` value that is not one path — a YAML list, an empty map, or a bare `target_repo:` with nothing after it — is refused, never read as undeclared: a block list, an empty map or a bare key is refused as the plan is read, and a flow list when its path fails to locate. One milestone routes to exactly one tree, so a milestone that lives in two repos declares `spans_repos:` instead, which gates ship-readiness only — each repo's half is still delivered from that repo.
...
All spawn mechanics belong to the `agent-toolkit:spawn-agent` skill: surface/pane placement, session wiring, lifecycle, teardown. `/deliver` hands it the kickoff task text, the milestone identity, the worker's NAME, and — separately — whether to bridge it, and consumes exactly one thing back: a visible worker session it can watch and relay gates from. `/deliver` authors **no** `claude -p` heredoc spawn fences of its own; spawn mechanics are prose-delegated, never inlined. (This is also why the auto-approve marker never appears in `/deliver`'s body: there is no headless fence for it to mark.)

### Worker naming — the third thing, and every way it degrades

The skill file is the operative surface for the naming rules; this section is the debugging view of the same rules — which name a given run should have handed the spawning skill, and how to read a run that came up nameless. On a **resume** the name is whatever the operator confirmed at the gate, read straight off that record and never re-derived at the spawn, so a resumed worker answering to something nobody confirmed means the spawn re-derived instead of reading.

**A fresh idea gets a name too, derived rather than confirmed.** An idea arriving as prose has no decision record behind it and no confirm gate at all, so on that path the name is built at the spawn instead — by the same derivation the record would have run, over the milestone Phase 2 has just produced — and that is what goes to the spawning skill. The operator cannot retype it here, which is a far smaller thing than not having one: a fresh run never spawns an unnamed worker.

**Fail closed there, degrade cleanly here.** The pre-flight halt above and the naming degradations below look inconsistent until you read what each protects. The halt guards a safety property — the work has to run in a visible session the operator can watch and take over, which is why an invisible background agent is an explicit non-substitute for one and why nothing short of stopping will do. A worker's remote-control name is legibility instead, and an unnamed worker still completes the whole ceremony correctly on its own surface. So every naming path below takes the lesser outcome and reports it; none of them stops the run.

**An underivable name renders `none`, and the run proceeds to the gate anyway.** When the repository basename and the run's identity do not compose into something the naming grammar admits, the decision record renders `remote_control: none` and never refuses for it — a naming problem stopping a run before its gate is the hardest outcome for the mildest cause, and the operator was about to be shown a field they could drop by hand regardless. Exactly one advisory row rides with the record naming which rule was broken and which half broke it: an identity too long to leave room under the length cap, a basename whose first character is outside the grammar, a basename that sanitizes away to nothing, or an identity that does the same — the mirror case, where the half that tells two runs apart folds away and the name would discriminate nothing. That row is how a `none` nobody typed is told apart from a bridge the operator dropped on purpose — and the difference matters beyond bookkeeping, because a dropped bridge still leaves the worker NAMED while an underivable name does not — and it is what says whether the fix is to shorten the identity or to rename the repository, neither of which an operator can guess from the bare fact that no name was composed.

**A spawning skill too old to understand the extra argument degrades.** An installation predating this contract does not recognize the argument at all, so the worker comes up unnamed and unbridged, the ceremony proceeds through every stage it would otherwise have run, and exactly one advisory row reports that the installed spawning skill cannot carry a worker name yet. It is never a halt: upgrading that plugin is the whole remedy, and until someone does, the run is worse only in how easy the worker is to find on screen.

**A bridged launch that hard-exits before the session ever starts is retried exactly once, without the argument.** Where an organization has switched remote control off, or a subscription does not reach it, the launch dies at the command line and no worker exists yet — nothing is half-started, so the same spawn is reissued unbridged and the ceremony proceeds from there. Exactly one advisory row reports that this account will not accept a bridged worker; an entitlement on the account is the whole remedy, and a single retry is the whole budget, since a launch turned away for an entitlement will be turned away again.

Those last two are separate rows on purpose. A stale installation and a refused command line are different faults with different fixes — an upgrade answers one, an account entitlement the other — so a run that reported only "the worker came up unbridged" has lost the half that says which fix to reach for.
