...
All spawn mechanics belong to the `agent-toolkit:spawn-agent` skill: surface/pane placement, session wiring, lifecycle, teardown. `/deliver` hands it three things — the kickoff task text, the milestone identity, and the `remote_control` name the worker will be reachable under — and consumes exactly one thing back: a visible worker session it can watch and relay gates from. `/deliver` authors **no** `claude -p` heredoc spawn fences of its own; spawn mechanics are prose-delegated, never inlined. (This is also why the auto-approve marker never appears in `/deliver`'s body: there is no headless fence for it to mark.)
