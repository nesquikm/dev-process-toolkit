...
2. Extract the run-scope identifier from the resolver result:
   - Milestone run (`fallthrough` + arg matches a milestone token) ⇒ `RunScope.kind = "milestone"`, `number` = the BRANCH token derived from the `milestone:` frontmatter: bare digits for a numeric milestone (`19` for `M19`), or the full `M_<epic-key>` token for an Epic-keyed one (`M_GF_78`, never its digits `78`). Passing the frontmatter token for a NUMERIC milestone (`M19`) is not the same thing and does not work — it selects the ticket-keyed template and renders `mM19`.
   - Tracker-mode FR run ⇒ `RunScope.kind = "fr-tracker"`, `trackerId = <resolved tracker ID>`, plus optional `milestoneNumber` = the BRANCH token derived from the FR's `milestone:` frontmatter when the FR is milestone-bound — bare digits for a numeric milestone (`19`), or the full `M_<epic-key>` token for an Epic-keyed one (`M_GF_78`, never its digits `78`).
...
5. Resolve the effective template via `canonicalBranchTemplate({ milestone })` from `adapters/_shared/src/branch_proposal.ts`, where `milestone` is the resolved FR's BRANCH token derived from the `milestone:` frontmatter for FR-scoped runs (bare digits for a numeric milestone, or the full `M_<epic-key>` token for an Epic-keyed one — `M_GF_78`, never its digits `78`; milestone runs use the run-arg `<N>`, unchanged) — milestone-bound ⇒ `{type}/m{N}-{slug}`, milestone-less ⇒ `{type}/{ticket-id}-{slug}` fallback. Precedence: an explicit Schema L `branch_template:` value (anything other than the canonical seeded default) wins over the derived form — operator override preserved; the absent-key skip in Guard step 1 is unaffected. Then call `buildBranchProposal({template, type, slug, milestone, trackerId, shortUlid})`. The function:
...
**The run is bounded.** A gate run is cut after `GATE_RUN_TIMEOUT_MS` (20 minutes, raised or lowered for a slower suite through `DPT_GATE_TIMEOUT_MS`), because a runner really does hang: `bun test --reporter=junit` was measured stalling at 0% CPU with no children, twice, and an unbounded run turns that into a gate that never returns — no commit, no release ceremony, no live proof. A cut run is a VERDICT, not a silence: the marker `dpt-gate-run: the gate command did not finish` is appended to the captured bytes, so a count parsed from a half-written summary cannot read as merely unparseable, and the identities half answers `unavailable` rather than reporting the skips of a report that was still being written.
