...
**Auditable evidence.** A receipt file under the target repository's receipts directory for this session, announced by a `dpt-receipt: <absolute path> sha256:<digest>` line inside the non-error result of a Bash `tool_use` whose command is one plain `bun [run] <module>` invocation of one of this plugin's deciding modules listed in `RECEIPT_ANNOUNCING_MODULES` (`create_idempotency_probe.ts`, `container_ownership.ts`, `ticket_ownership.ts`), with the file still hashing to that digest. A receipt written any other way — the Write tool, an echoed announcement (even one chained after the real command or carrying the module's name in a comment), quoted assistant text, a file rewritten after its announcement — authorises nothing. Ownership of a ticket is also shown by a tracked FR file binding it or a create of it earlier in the same session.
