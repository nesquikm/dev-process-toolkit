...
**Auditable evidence.** A receipt file under the target repository's receipts directory for this session, announced by a `dpt-receipt: <absolute path>` line inside the non-error result of a Bash `tool_use` that ran one of the deciding modules listed in `RECEIPT_ANNOUNCING_MODULES` (`create_idempotency_probe.ts`, `container_ownership.ts`, `ticket_ownership.ts`). A receipt written any other way — the Write tool, an echoed announcement, quoted assistant text — authorises nothing. Ownership of a ticket is also shown by a tracked FR file binding it or a create of it earlier in the same session.
