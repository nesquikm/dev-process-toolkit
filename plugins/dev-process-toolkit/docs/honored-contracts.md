...
**Mandate.** Before a `git commit` Bash call, the session MUST have invoked `/dev-process-toolkit:gate-check` as a skill, **and** the gate MUST have run against the checkout being committed to. Running the project's gate command by hand, reading a green test run, or reporting the gate's result in prose does not discharge the mandate — and neither, since STE-614, does a `/gate-check` run in a *different* checkout of the same session. The `pre-commit-gate-check` `PreToolUse` hook grades the transcript, not the working tree, and refuses the commit with exit 2, so the `git commit` tool call never runs at all.

**Violation name.** Unannounced Gate — running the gate checks by hand and expecting the `pre-commit-gate-check` hook to see it, instead of invoking the skill that leaves the mark the hook reads. Its STE-614 sibling is the Borrowed Gate — one `/gate-check` run in repository A standing in as evidence for a commit into repository B.

**Auditable evidence.** Two legs, both demanded. (1) The transcript leg: one JSONL record in the current session carrying a `tool_use` block named `Skill` whose `input.skill` is `dev-process-toolkit:gate-check`, on one parsed line per the STE-285 atomic-line invariant — structural since STE-614, so a quoted mention of those bytes in assistant text is no longer accepted, and a call whose paired `tool_result` carries `is_error: true` is retired rather than counted. (2) The repository leg: a `/gate-check` receipt of this session, in the receipt store of the repository being committed to, naming that same checkout by realpath — and vouched for by one of that session's Skill calls, since one gate run vouches for one checkout. The leg applies in every toolkit-managed checkout, shared or not, `repo_tag` or not. Implemented in `templates/hooks/_lib/session.ts` (`findSkillToolUse`, `requireSkillToolUse`, reading through `readTranscriptLines`) with the receipt verdict owned by `adapters/_shared/src/gate_receipt.ts`, which is also what writes the receipt. The predicate cannot see a Bash `tool_use`, a passing test run, or any command output; it fails open only when it cannot see the session at all (no transcript path, an unreadable transcript, or unparseable stdin).
...
**Auditable evidence.** One JSONL record in the current session transcript carrying a `tool_use` block named `Skill` whose `input.skill` is `dev-process-toolkit:spec-review`, on one parsed line per the STE-285 atomic-line invariant — structural since STE-614, and a call retired by an `is_error: true` `tool_result` is not evidence. Implemented in `templates/hooks/_lib/session.ts` (`findSkillToolUse`, reading through `readTranscriptLines`). Nothing else counts: not a Bash `tool_use`, not a green audit, not command output. The house rule is the same two-legged one the commit gates enforce — the Skill call plus the review's receipt in the repository the PR is raised from — and `/spec-review` already mints that receipt in the checkout it runs against; wiring this hook to resolve the `gh pr create` target repository and grade the receipt leg against it is STE-615's. The hook fails open only when the session itself is invisible to it (no transcript path, an unreadable transcript, or unparseable stdin).
...
**Auditable evidence.** Either of two lines in the current session transcript, and since STE-614 each is graded against the checkout being committed to. (1) One JSONL record carrying a `tool_use` block named `Skill` whose `input.skill` is `dev-process-toolkit:tdd`, on one parsed line per the STE-285 atomic-line invariant — structural since STE-614, and a call retired by an `is_error: true` `tool_result` is not evidence — **plus** a `/tdd` receipt of this session in that repository's receipt store, naming that same checkout by realpath and vouched for by one of the session's Skill calls. A `/tdd` run in a sibling checkout is evidence about the sibling. (2) One line carrying the marker `dpt-red-before-proof:` followed, on that same line, by `repo=<absolute checkout root>` and every staged path that raised the requirement — the atomic-line invariant re-applied, plus a position rule so a path merely mentioned earlier in the message is not swept into the claim. Door two carries its own scope because a proof is a line an operator typed rather than a receipt a gate minted: `repo=` is matched by realpath, a non-absolute or unresolvable value covers nothing, and a claim naming no repository at all keeps the older reading and covers only the session's own checkout. Coverage of (2) is a union across marker lines; a proof naming an unrelated file covers nothing, and an empty required set is never covered. Both are implemented in `templates/hooks/_lib/session.ts` (`findSkillToolUse`, `findRedBeforeProof`, joined by `requireTddEvidence`) and both read the one transcript reader, `readTranscriptLines`; the receipt half is owned by `adapters/_shared/src/gate_receipt.ts`. The predicate is still blind to Bash `tool_use` entries and to test output, so an unrecorded hand-run suite is invisible to it; it fails open only on an invisible session (no transcript path, an unreadable transcript, or unparseable stdin).
