...
- Each FR has these required top-level sections, in order: `## Requirement`, `## Acceptance Criteria`, `## Technical Design`, `## Testing`, `## Notes`. An FR may optionally carry a `## Summary` section — plain-language prose — placed first in the FR body, immediately after the H1 title and before `## Requirement` (see `/spec-write` § 0b for the authoring rule). An FR may optionally carry one additional `## Design References` section placed immediately after `## Acceptance Criteria` — purely additive, leaving the five required sections' order and required status unchanged (see `/spec-write` § 0b for its shape and a worked example). An FR may optionally carry a `## External References` section — informational (non-blocking) external links — placed immediately after `## Design References`, created only when such a link is first recorded (see `/spec-write` § 0b step 6b). An FR may also optionally carry a `## Token Stats` section — fenced between `<!-- token-stats:begin -->` / `<!-- token-stats:end -->` sentinel markers, machine-managed (never hand-edited), and placed after `## Notes`, always last in the FR body. These four are the only optional sections; all are purely additive. No `/gate-check` probe enforces this FR body section set or order, so an FR carrying the optional `## Summary`, `## Design References`, `## External References`, or `## Token Stats` section never trips a false-positive GATE FAILED.
...
- Milestone id is governed by mode. Tracker mode with Jira derives `M_<epic-key>` from the milestone Epic; Linear is tracker-first too (STE-541) — it creates the project milestone and derives `M_<id>` from the identifier the tracker allocates, and no longer consults the sequential `M<N>` scan. In `mode: none` there is no tracker key to claim from, so the identity is minted locally — since STE-537 `/setup` step 8 mints it when it writes the bootstrap plan and `/spec-write` ADOPTS that recorded value, minting only when there is nothing to adopt; either way `Provider.mintId()` returns the full `fr_<26-char ULID>`, and `milestoneIdFromUlid` derives the plan id `M_<short-ULID>` from `ulid.slice(23, 29)` (the same tail offsets the FR filename and AC prefix use). The minted value is preserved verbatim in plan frontmatter `id:` in `mode: none` only; tracker mode has no `id:` line. Existing sequential `M<N>` plans in a tracker-less project keep resolving, archiving and shipping unchanged alongside minted ones — coexistence, no migration.
...
Toolkit-owned state lives in one folder at the project root. Every path under it is composed by `adapters/_shared/src/dpt_paths.ts` (`dptRoot`, `locksDir`, `ledgerPath`, `scratchRoot`, `scratchDir`, `skipBaselinePath`, `checkoutIdPath`, `advisoryNotesPath`, `smokeRunLedgerPath`, `receiptsRoot`, `receiptsDir`) — no other module composes a `.dpt` literal.

```
.dpt/
├── .gitignore          ← tracked — ships the rules itself ("ledger/" + "scratch/" + "skip-baseline.json")
...
├── ledger/receipts/<session>/ ← ignored — one session's shared-container tracker decisions, a JSON receipt per decision (STE-602)
...
- Conformance probes:
  ...
  2. **Required frontmatter fields** present for every FR file — the mode-invariant Schema Q keys `title`, `milestone`, `status`, `archived_at`, `created_at`. BOTH `id:` and `tracker:` are **mode-conditional mirrors**: `id` is required in `mode: none` and absent in tracker mode (where the tracker ID is the canonical identity), and `tracker` is required in tracker mode and absent in `mode: none`. Mode-conditional enforcement lives in probe #13 `identity_mode_conditional`; see `skills/gate-check/SKILL.md:26` for the full contract. Missing a mode-invariant field = fail. The plan-side twin of the same bimodal rule — a minted `M_<short-ULID>` plan carries `id:` in `mode: none`, no plan carries it in tracker mode — is a separate module enforced by probe #73 `plan_identity_mode_conditional`.
