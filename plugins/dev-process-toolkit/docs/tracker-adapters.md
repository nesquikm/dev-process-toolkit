...
| Jira    | `true`              | Epic-first: `adapters/jira.md` declares `milestone_binding: epic`, so the milestone is the parent **Epic** and the ticket binds to it via its `parent` key. It reaches the legacy binding by TWO routes — always for a grandfathered numeric `M<N>` token, whose reader looks there, and for an Epic-keyed one where the Epic-availability probe reports the project cannot hold an Epic — the `milestone-<M-token>` label (e.g. `milestone-M86`), attached via `editJiraIssue` read-merge-write (fetch the issue's current `labels`, union in the milestone label, write the merged set back) so no existing label is clobbered. | Epic path: **binding never creates.** An Epic-keyed milestone names an Epic that must already exist — the token was derived from that Epic's key — so a miss refuses and names the key it looked for. Minting is a separate, named step (`mintMilestoneEpic`), and it runs the arrow this row once disclaimed: create the Epic with the human title, read the allocated key back, derive `M_<KEY>` from it. There is no flow in which an Epic is named from a milestone token that already exists locally; an Epic so named could never be found, because the token encodes a DIFFERENT key. A grandfathered numeric `M<N>` milestone never reaches this path at all — it routes to the label surface. Legacy label path: N/A (create-on-write) — the label is created implicitly the first time it is attached; no pre-existing tracker milestone object is required. |
