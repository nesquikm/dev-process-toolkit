...
| Jira    | `true`              | Epic-first: `adapters/jira.md` declares `milestone_binding: epic`, so the milestone is the parent **Epic** and the ticket binds to it via its `parent` key. Only where the Epic-availability probe reports the project cannot hold one does it degrade to the legacy binding — the `milestone-<M-token>` label (e.g. `milestone-M86`), attached via `editJiraIssue` read-merge-write (fetch the issue's current `labels`, union in the milestone label, write the merged set back) so no existing label is clobbered. | Epic path: the Epic is created if absent, **named from the milestone that already exists locally** — at migration time the token is the input and the Epic is derived from it, not the reverse. (The opposite arrow — key → `M_<KEY>` token via `milestoneIdFromEpicKey` — belongs to fresh allocation in `/spec-write`, a different call site; migration never rewrites an existing local token.) Legacy label path: N/A (create-on-write) — the label is created implicitly the first time it is attached; no pre-existing tracker milestone object is required. |
