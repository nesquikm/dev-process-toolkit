...
7. **Seed `run_cmd` on the same pass.** The offer rides this step; it is never a step of its own, and its seed comes from the very stack detection step 3 already used to pick the stub. Propose the command that stack implies — web / JS / TS / Node → the dev server (`npm run dev`, `bun run dev`); Flutter / Dart → `fvm flutter run` (or `flutter run`); Python / API → the service entry point (`uvicorn app:app --reload`, `python manage.py runserver`); other / unknown → nothing to seed from, so ask outright. The operator confirms or edits the proposal, and the confirmed value lands as `run_cmd: <command>`. When the stack declares no way to run the thing — a library, not an application, so the project is not runnable — write `run_cmd: none` instead. That is the quiet half, and it is the half most likely to be dropped: skipping it writes nothing and reads exactly like a correct run. `none` is an ANSWER ("this project cannot be run"), distinct from an absent key, which parses to `null` and is no answer at all — so **neither path leaves `run_cmd` absent**. The distinction is load-bearing downstream: `resolveVerifyMode` keys its `blocking` default off a real `run_cmd`, and probe #80 (`runnability_declared`) treats `none` as answered but an absent key as undeclared the moment run instructions appear in the repo.
