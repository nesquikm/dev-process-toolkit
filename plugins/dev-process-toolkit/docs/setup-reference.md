...
6. **Wire it in.** On a successful scaffold, write `verify_skill: <written-slug>` into the CLAUDE.md `## Verification` block (creating the block if absent). This is the only write the accept path makes to CLAUDE.md.
