...
Answer all seven before the flip. Each row carries a check that returns yes or no, not a caution to bear in mind. The deltas marked MEASURED were taken from the two repos this contract was written against — `glacy-app-be` (Jira project `GB`) and `glacy-app-fe` (Jira project `GF`). They are the SOURCE of these measurements, not a forecast: whether those two repos should share a project is an open question their own survey argued against, and nothing here decides it. The rows below apply to any repo pair considering a shared container.

1. **Target project key visible to the MCP.** Step 7b already refuses on an invisible key, so this is the cheapest failure to find and it should be found first. **Check:** the target project key appears in the project list the tracker MCP returns for the operator's own credentials.
2. **Probe #25 is green in every participating repository: the repo tag is declared in every participating repo, and present in the forwarded default labels.** `repo_tag:` is a free-form field of the `### Jira` / `### Linear` sub-section, not a `## Task Tracking` top-level key. The conjunct that separates one repo's work from another's on a shared board is inert without it — and on a shared board an inert conjunct reads as "no prior run" on the create-idempotency probe, which is where the shipped contract scopes it, which is a silent false negative rather than a loud failure. **Check:** probe #25 reports green in every participating repository — every one declares a `repo_tag:` value, and that same value is present in the forwarded default labels.
3. **Issue type reconciled across the participating repos.** MEASURED: `GB` declares Story (id 11108); `GF` declares Task. `jira_issue_type:` is the override that settles which one the shared project gets. **Check:** one declared issue type, and the target project's create screen accepts it.
4. **Status vocabulary re-derived against the target project.** MEASURED: `GB` carries a `specs/tracker-config.yaml` (To Do / In Progress / In Review / Done); `GF` carries none. Repointing without a status re-fetch carries `GB`'s vocabulary onto `GF`. Hand-write the union of BOTH containers' status names BEFORE the flip, then re-check that file AFTER the flip and hand-merge back anything it lost — the flag's own step 7f run rewrites it from the newly-bound container alone, so it will clobber a hand-written union without saying so. Whether the two vocabularies actually collide is empirical per repo pair; this row orders the check and does not assert the answer. **Check:** the file's status list is the union of both containers before the flip, and still the union after it.
5. **One reconciled tracker site and MCP server.** MEASURED: `GB` says `mcp_server: atlassian`; `GF` says `mcp_server: claude.ai Atlassian`. Two spellings of a server are two bindings at the resolver, which requires `mcp_server:` to match the exact string `claude mcp list` prints. **Check:** one site, and one identical `mcp_server:` value in every participating repo.
6. **Milestone-label collision surveyed.** MEASURED on `GF`: labels span the numeric series `milestone-M23`..`milestone-M46` AND the Epic-keyed `milestone-M_GF_80` / `milestone-M_GF_83` / `milestone-M_GF_85`, while `GB` (MEASURED) brings its own numeric series into that same namespace. This row RECORDS that the milestone-label collision must be answered before the repoint; it does not solve it. **Check:** the two repos' milestone-label sets have been listed side by side and the overlap is either empty or has a written resolution.
7. **No active FRs mid-flight.** A repoint landing between an FR's ticket create and its next sync strands that binding: the ticket lives in the old container while the config names the new one. **Check:** no FR is in flight — every active FR is either not yet created in the tracker or already synced.
